...
def _get_historical_price_and_ma20(stock_code, issue_date_str, force_regenerate=False):
    ...
    try:
        ...
        # DB中无数据或需要重新生成
        from ml_training.data.update_market_data import generate_market_data, fetch_latest_data
        ...
        try:
            from ml_training.data.update_indices_data import rebuild_locked_indices_data
...
def generate_report(stock_code='300735.SZ', stock_name='光弘科技', issue_date=None, force=False, debug_mode=False, target_chapter=None):
    ...
    if not is_fresh:
        ...
        try:
            ...
            if scripts_dir not in sys.path:
                sys.path.insert(0, scripts_dir)
            from ml_training.data.update_market_data import generate_market_data
                    ...
                    if data_age > 7:
                        ...
                        if auto_generate:
                            ...
                            try:
                                ...
                                if scripts_dir not in sys.path:
                                    sys.path.insert(0, scripts_dir)
                                from ml_training.data.update_market_data import generate_industry_data
