...
def rebuild_indices_data():
    ...
    for index_info in indices:
        ...
        try:
            ...
            # 年化收益率（所有窗口期）
            ann_20d, _ = calculate_annual_return_v2(df, 20)
            ann_60d, _ = calculate_annual_return_v2(df, 60)
            ann_120d, _ = calculate_annual_return_v2(df, 120)
            ann_250d, _ = calculate_annual_return_v2(df, 250)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
使用示例:
  python update_indices_data.py              # 默认：重新获取数据
  python update_indices_data.py --mode rebuild # 重新获取数据
  python update_indices_data.py --mode update  # 更新现有文件
        '''
    )
    ...
    parser.add_argument(
        ...
        default='rebuild',
        help='运行模式: rebuild(重新获取API数据) 或 update(更新现有文件)'
    )
