...
def generate_sensitivity_chart(volatilities, profit_probs, save_path):
    """生成敏感性分析图表"""
    plt.figure(figsize=(10, 6))
    colors = ['#2ecc71' if p >= 60 else '#f39c12' if p >= 40 else '#e74c3c' for p in profit_probs]
    bars = plt.bar(range(len(volatilities)), profit_probs, color=colors, alpha=0.7)
    plt.xlabel('波动率', fontproperties=font_prop, fontsize=12)
    plt.ylabel('盈利概率 (%)', fontproperties=font_prop, fontsize=12)
    plt.title('不同波动率下的盈利概率', fontproperties=font_prop, fontsize=14, fontweight='bold')
    plt.xticks(range(len(volatilities)), [f'{v*100:.0f}%' for v in volatilities], fontproperties=font_prop)
...
def generate_monte_carlo_chart(final_prices, issue_price, save_path):
    """生成蒙特卡洛模拟图表"""
    plt.figure(figsize=(12, 6))

    # 价格分布直方图
    plt.subplot(1, 2, 1)
    plt.hist(final_prices, bins=50, alpha=0.7, color='steelblue', edgecolor='black')
    plt.axvline(x=issue_price, color='red', linestyle='--', linewidth=2, label=f'发行价: {issue_price:.2f}')
    plt.xlabel('未来价格', fontproperties=font_prop, fontsize=12)
    plt.ylabel('频数', fontproperties=font_prop, fontsize=12)
    plt.title('锁定期后价格分布 (5000次模拟)', fontproperties=font_prop, fontsize=12)
    ...
    plt.hist(returns, bins=50, alpha=0.7, color='coral', edgecolor='black')
    plt.axvline(x=0, color='red', linestyle='--', linewidth=2, label='盈亏平衡')
    plt.xlabel('收益率 (%)', fontproperties=font_prop, fontsize=12)
    plt.ylabel('频数', fontproperties=font_prop, fontsize=12)
    plt.title('收益率分布', fontproperties=font_prop, fontsize=12)
    plt.legend(prop=font_prop)
    plt.grid(True, alpha=0.3)
    for label in plt.gca().get_xticklabels():
        label.set_fontproperties(font_prop)
...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    generate_sensitivity_chart(volatilities, prob_results, sensitivity_chart_path)
    ...
    add_image(document, sensitivity_chart_path)

    add_paragraph(document, '分析结论：')
    add_paragraph(document, f'• 在当前市场波动率（{market_data["volatility"]*100:.1f}%）下，项目盈利概率约 {prob_results[3]:.1f}%')
    ...
    add_title(document, '4.1 估值参数', level=2)
    ...
    generate_monte_carlo_chart(final_prices, project_params['issue_price'], mc_chart_path)
    ...
    add_paragraph(document, '图表 3: 蒙特卡洛模拟结果')
    ...
    # 保存图表
    var_chart_path = os.path.join(IMAGES_DIR, '04_var_analysis.png')
    ...
    add_paragraph(document, '图表 4: VaR风险度量')
    ...
    # 保存雷达图
    radar_chart_path = os.path.join(IMAGES_DIR, '05_radar_chart.png')
    ...
    add_paragraph(document, '图表 5: 风险评分雷达图')
