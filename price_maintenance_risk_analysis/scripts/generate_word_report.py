...
def generate_dcf_valuation_heatmap(save_path, current_price, net_income, total_shares):
    """生成DCF估值热力图
    ...
    # 估值参数（基于真实数据）
    # 假设自由现金流 = 净利润 × 60%（考虑资本支出和营运资本）
    base_fcf = net_income * 0.6
    # 假设债务基于数据中的总债务
    total_debt = 4.5  # 45.1亿元
    # 股数（亿股）
    ...
    for g in growth_range:
        ...
        for w in wacc_range:
            ...
            equity_value = enterprise_value - total_debt  # 减债务（亿元）
...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    total_shares = project_params.get('issue_shares', 5000000)  # 总股数
    ...
    generate_dcf_valuation_heatmap(dcf_chart_path, project_params['current_price'], net_income, total_shares)
    ...
    # 计算基础FCF
    base_fcf = net_income * 0.6  # 假设FCF为净利润的60%

    dcf_process_data = [
        ['财务数据', ''],
        ['净利润', f'{net_income/100000000:.2f} 亿元'],
        ['净资产', f'{net_assets/100000000:.2f} 亿元'],
        ['总债务', f'{total_debt/100000000:.2f} 亿元'],
