            ...
            if prices_list and len(prices_list) >= 100:
                ...
                if arima_result['model_fitted']:
                    ...
                    try:
                        ...
                        # 正确的累积预测：从历史终点继续
                        cumulative_forecast_values = np.concatenate([
                            [last_cumulative],  # 预测起点值（等于历史终点）
                            forecast_series.cumsum().values + last_cumulative  # 后续累积预测
                        ])

                        # 预测部分的x轴从历史终点后开始
                        forecast_x_start = len(log_returns)
                        forecast_x = range(forecast_x_start, forecast_x_start + len(cumulative_forecast_values))

                        ax2.plot(historical_index, cumulative_returns.values, '-',
                                label='历史累积收益率', color='#3498db', linewidth=2, alpha=0.7)
                        ax2.plot(forecast_x, cumulative_forecast_values,
                                '--', label='ARIMA预测累积收益率', color='#27ae60', linewidth=2, alpha=0.8)
                        ...
                        ax2.set_title('ARIMA模型 - 累积收益率拟合与预测',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
