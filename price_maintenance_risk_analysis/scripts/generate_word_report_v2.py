...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    add_title(document, '2.1 波动率敏感性分析', level=2)
    ...
    # ==================== 2.3 不同时间窗口风险指标分析 ====================
    add_paragraph(document, '')
    add_title(document, '2.3 不同时间窗口风险指标分析', level=2)
    ...
    try:
        ...
        if ts_token:
            ...
            if not df_prices.empty and len(df_prices) > 180:
                ...
                add_table_data(document, ['时间窗口', '年化波动率', '年化收益率', '最大回撤', '夏普比率'], window_data)
    ...
    add_paragraph(document, '本章节分析定增项目在不同预设情景下的风险表现，包括发行价折扣情景、不同时间窗口下的风险指标对比以及破发概率情景分析。')

    # ==================== 3.1 发票价折扣情景分析 ====================
    add_title(document, '3.1 发票价折扣情景分析', level=2)
    ...
    try:
        ...
        if ts_token:
            ...
            if not df_prices.empty and len(df_prices) > 180:
                ...
                # 添加图表
                add_paragraph(document, '')
                add_paragraph(document, '图表 3.2.1: 不同时间窗口的波动率对比')
                add_image(document, chart_paths[0], width=Inches(6.5))

                add_paragraph(document, '')
                add_paragraph(document, '图表 3.2.2: 不同时间窗口的收益率对比')
                add_image(document, chart_paths[1], width=Inches(6.5))

                add_paragraph(document, '')
                add_paragraph(document, '图表 3.2.3: 不同时间窗口的风险指标对比')
                add_image(document, chart_paths[2], width=Inches(6.5))
    ...
    add_title(document, '7.1 模拟参数', level=2)
