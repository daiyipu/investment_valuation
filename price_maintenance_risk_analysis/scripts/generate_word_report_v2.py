...
def generate_tornado_chart(issue_price, current_price, lockup_period, volatility, drift, save_path):
    ...
    敏感性分析原则：
    - 所有参数使用标准化的单位变化：±10个百分点（绝对变化）
    - 溢价率：±10个百分点（例如从-5%变为-15%或+5%）
    - 波动率：±10个百分点（例如从30%变为20%或40%）
    - 漂移率：±10个百分点（例如从8%变为-2%或18%）
    ...
    # 计算基准溢价率
    base_premium_rate = (base_params['issue_price'] - base_params['current_price']) / base_params['current_price']
    ...
    param_definitions = [
        {
            'name': 'premium_rate',
            'name_cn': '溢价率',
            'changes': [-0.10, 0.10],  # ±10个百分点
            'change_type': 'absolute',
            'apply_change': lambda base_val, change: base_val + change,  # 绝对变化
            'recalc_issue_price': True  # 需要重新计算发行价
        },
        {
            'name': 'volatility',
            'name_cn': '波动率',
    ...
    for param_def in param_definitions:
        ...
        for change in param_def['changes']:
            ...
            # 特殊处理溢价率：需要重新计算发行价
            if param_def.get('recalc_issue_price'):
                new_premium_rate = base_premium_rate + change  # 溢价率绝对变化
                params['issue_price'] = params['current_price'] * (1 + new_premium_rate)
            else:
    ...
    # 预期收益率按收益率影响从大到小排序
    param_changes_by_return = sorted(param_changes, key=lambda x: x['return_impact'], reverse=True)
...
def generate_relative_valuation_charts_split(current_metrics, industry_avg, peer_companies_data, save_dir):
    ...
    bars2 = ax.bar(x + width/2, industry_vals, width, label='行业中位数', color='#e74c3c', alpha=0.8)
...
def generate_industry_index_charts(industry_data, save_dir):
    ...
    try:
        ...
        # 获取最近500天的数据（优先DB缓存 → AKShare降级 → sw_daily兜底）
        ...
        if df is None or df.empty:
            try:
                ...
                ind_name = (industry_data.get('sw_l3_name') or industry_data.get('sw_l2_name') or
                           industry_data.get('sw_l1_name') or industry_data.get('industry_name', ''))
                if ind_name:
                    clean = ind_name.replace('Ⅱ','').replace('Ⅲ','').strip()
                    ths = ak.stock_board_industry_name_ths()
                    m = ths[ths['name'].str.contains(clean, na=False)]
                    if m.empty:
                        m = ths[ths['name'].str.contains(ind_name, na=False)]
                    if not m.empty:
                        s_fmt = f"{start_date[:4]}-{start_date[4:6]}-{start_date[6:8]}"
                        e_fmt = f"{end_date[:4]}-{end_date[4:6]}-{end_date[6:8]}"
                        df = ak.stock_board_industry_index_ths(symbol=m.iloc[0]['name'], start_date=s_fmt, end_date=e_fmt)
                        if df is not None and not df.empty:
                            df = df.rename(columns={'日期':'trade_date','开盘价':'open','最高价':'high',
...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    pricing_ma20 = project_params.get('pricing_ma20', market_data.get('ma_20', 0))
    ma20 = market_data.get('ma_20', pricing_ma20)  # 用于显示的MA20
    ma120 = market_data.get('ma_120', 0)
    issue_price = project_params['issue_price']

    # 计算折价/溢价率（负值表示折价，正值表示溢价）
    # 始终使用pricing_ma20作为基准，确保与发行价计算一致
    discount_premium = (issue_price - pricing_ma20) / pricing_ma20 * 100
    if issue_price < pricing_ma20:
        issue_type = "折价发行"
    else:
        issue_type = "溢价发行"
    ...
    # 计算实际溢价率
    actual_premium_rate = (project_params["issue_price"] / project_params["current_price"]) - 1
    ...
    info_data = [
        ...
        ['实际溢价率', f'{actual_premium_rate*100:+.2f}%'],
        ['发行类型', f'{issue_type}（名义溢价率: {discount_premium:.2f}%）']
    ]
    ...
    project_data = [
        ...
        ['当前价格', f'{project_params["current_price"]:.2f} 元/股'],
        ['MA20价格（定价基准）', f'{pricing_ma20:.2f} 元/股'],
        ['MA20价格（当前）', f'{ma20:.2f} 元/股'],
        ['名义溢价率', f'{discount_premium:.2f}%'],
        ['实际溢价率', f'{actual_premium_rate*100:+.2f}%'],
        ['发行类型', issue_type]
    ]
    ...
    try:
        ...
        if ts_token:
            ...
            # 获取申万行业指数的PE数据（DB缓存 → AKShare降级 → sw_daily兜底）
            sw_index_pe = None
            sw_index_pb = None
            sw_index_ps = None
            try:
                ...
                if df_index is None or df_index.empty:
                    try:
                        ...
                        if not matched.empty:
                            ...
                            if sw_index_pe:
                                print(f"  使用AKShare申万PE数据: PE={sw_index_pe:.2f}, PB={sw_index_pb:.2f}")
                                df_index = 'akshare_skip'
                    except Exception:
                        pass
                # 3. sw_daily兜底
                ...
                if not sw_index_pe:
                    print(f"⚠️ 申万行业指数PE数据为空，将跳过行业PE对比")
            except Exception as e:
    ...
    industry_stats_val = {
        'pe': {
            ...
            'p05': peer_companies_val['pe'].quantile(0.05),  # 5%分位数（用于压力测试）
            'min': peer_companies_val['pe'].min(),
            'max': peer_companies_val['pe'].max(),
            'std': peer_companies_val['pe'].std(),
            'iqr': peer_companies_val['pe'].quantile(0.75) - peer_companies_val['pe'].quantile(0.25)  # 四分位距
        },
        'ps': {
            ...
            'iqr': peer_companies_val['ps'].quantile(0.75) - peer_companies_val['ps'].quantile(0.25)  # 四分位距
        },
        'pb': {
            ...
            'iqr': peer_companies_val['pb'].quantile(0.75) - peer_companies_val['pb'].quantile(0.25)  # 四分位距
        }
    }

    # 使用行业中位数（更稳健，不受极端值影响）
    industry_avg_val = {
        'pe': industry_stats_val['pe']['median'],
        'ps': industry_stats_val['ps']['median'],
        'pb': industry_stats_val['pb']['median']
    }

    # 估值指标对比表（增强版）
    valuation_headers = ['指标', '光弘科技', '行业中位数', '平均值', 'Q1(25分位)', 'Q3(75分位)', '最小值', '最大值', '偏离度']
    valuation_data = [
        ['PE (TTM)',
         ...
         f"{(current_metrics_val['pe']-industry_stats_val['pe']['median'])/industry_stats_val['pe']['median']*100:+.1f}%"],
        ['PB',
         ...
         f"{(current_metrics_val['pb']-industry_stats_val['pb']['median'])/industry_stats_val['pb']['median']*100:+.1f}%"],
        ['PS (TTM)',
         ...
         f"{(current_metrics_val['ps']-industry_stats_val['ps']['median'])/industry_stats_val['ps']['median']*100:+.1f}%"]
    ]
    ...
    add_paragraph(document, '• 行业中位数：行业50%的公司估值低于此水平，抗极端值干扰（推荐参考）')
    add_paragraph(document, '• 平均值：所有同行公司的算术平均值（受极端值影响较大）')
    add_paragraph(document, '• 标准差：衡量数据离散程度，受极端值影响显著')
    add_paragraph(document, '• 四分位距(IQR)：Q3-Q1，衡量中间50%数据的离散程度，更稳健')
    ...
    add_paragraph(document, '• 标准差受极端值（如高PE公司）影响较大，建议参考四分位距(IQR)或中位数作为更稳健的估值基准')
    ...
    add_paragraph(document, f'• PE: 平均{industry_stats_val["pe"]["mean"]:.2f}倍，中位数{peer_companies_val["pe"].median():.2f}倍，标准差{industry_stats_val["pe"]["std"]:.2f}倍，四分位距{industry_stats_val["pe"]["iqr"]:.2f}倍')
    ...
    add_paragraph(document, f'• PB: 平均{industry_stats_val["pb"]["mean"]:.2f}倍，中位数{peer_companies_val["pb"].median():.2f}倍，标准差{industry_stats_val["pb"]["std"]:.2f}倍，四分位距{industry_stats_val["pb"]["iqr"]:.2f}倍')
    ...
    add_paragraph(document, f'• PS: 平均{industry_stats_val["ps"]["mean"]:.2f}倍，中位数{peer_companies_val["ps"].median():.2f}倍，标准差{industry_stats_val["ps"]["std"]:.2f}倍，四分位距{industry_stats_val["ps"]["iqr"]:.2f}倍')
    ...
    try:
        ...
        if stock_pe_data is not None and industry_pe_data is not None:
            ...
            # 计算个股历史分位数统计
            stock_pe_current = float(stock_pe_data.iloc[-1]['pe_ttm'])
            stock_pe_min = float(stock_pe_data['pe_ttm'].min())
            stock_pe_max = float(stock_pe_data['pe_ttm'].max())
            stock_pe_median = float(stock_pe_data['pe_ttm'].median())
            # 确保percentile是标量值
            stock_pe_percentile_sum = (stock_pe_data['pe_ttm'] < stock_pe_current).sum()
            stock_pe_percentile = float(stock_pe_percentile_sum.iloc[0] if isinstance(stock_pe_percentile_sum, pd.Series) else stock_pe_percentile_sum) / len(stock_pe_data) * 100

            # 计算申万行业指数历史分位数统计
            sw_index_pe_current = float(industry_pe_data.iloc[-1]['pe_ttm'])
            sw_index_pe_min = float(industry_pe_data['pe_ttm'].min())
            sw_index_pe_max = float(industry_pe_data['pe_ttm'].max())
            sw_index_pe_median = float(industry_pe_data['pe_ttm'].median())
            # 确保percentile是标量值
            sw_index_pe_percentile_sum = (industry_pe_data['pe_ttm'] < sw_index_pe_current).sum()
            sw_index_pe_percentile = float(sw_index_pe_percentile_sum.iloc[0] if isinstance(sw_index_pe_percentile_sum, pd.Series) else sw_index_pe_percentile_sum) / len(industry_pe_data) * 100
        ...
        if market_data and 'total_days' in market_data:
            ...
            if len(price_series_list) >= 250:
                ...
                for window in [20, 60, 120, 250]:
                    ...
                    # 计算夏普比率
                    volatility = time_window_results['volatility'][[20, 60, 120, 250].index(window)]
                    annual_return = time_window_results['annual_return'][[20, 60, 120, 250].index(window)]
                    excess_return = annual_return - risk_free_rate
                    sharpe = excess_return / volatility if volatility > 0 else 0
                    sharpe_list.append(sharpe)

                time_window_results['max_drawdown'] = max_drawdown_list
                time_window_results['sharpe'] = sharpe_list
            else:
                # 数据不足时使用0值
                time_window_results['max_drawdown'] = [0] * 4
                time_window_results['sharpe'] = [0] * 4
    ...
    add_paragraph(document, '  - 百分比参数（波动率、漂移率）：±5个百分点')
    ...
    # 默认使用250日窗口参数（最稳定的长期参数）
    mc_volatility = mc_volatility_250d
    mc_drift = mc_drift_250d

    # ==================== 模拟结果 ====================
    add_paragraph(document, '2. 模拟结果（基于锁定期180天）', bold=True)
    add_paragraph(document, '（注：使用250日窗口参数进行模拟，模拟期数=锁定期180天）')
    ...
    for window_name, config in windows_mc.items():
        ...
        try:
            ...
            if time_steps == 60:
                ...
                years = time_steps / 252.0  # 其他情况用实际计算

            # 使用复利公式：(1 + 期收益率)^(1/年数) - 1
            # 注意：当期收益率接近-100%时，年化收益率会趋于-∞，需要限制
            annualized_returns = np.where(
                returns > -0.99,  # 避免除以0或负数
                (1 + returns) ** (1 / years) - 1,
                -0.99  # 极端情况限制为-99%
            )
            ...
            print(f"    调试: 模拟期数={time_steps}日(年数={years}), 锁定期={project_params['lockup_period']}月")
            ...
            # 统计分析
            # 计算分位数
            percentile_1 = np.percentile(annualized_returns, 1)   # 1%分位数（左侧尾部最坏1%）
            percentile_5 = np.percentile(annualized_returns, 5)   # 5%分位数（左侧尾部最坏5%）
            percentile_10 = np.percentile(annualized_returns, 10) # 10%分位数（左侧尾部最坏10%）
            percentile_25 = np.percentile(annualized_returns, 25)
            percentile_75 = np.percentile(annualized_returns, 75)
            percentile_95 = np.percentile(annualized_returns, 95)  # 95%分位数（右侧尾部最好5%）

            # 计算CVaR（条件风险价值）= 尾部平均值
            # CVaR 90% = 10%最坏情况的平均值
            # CVaR 95% = 5%最坏情况的平均值
            # CVaR 99% = 1%最坏情况的平均值
            cvar_90 = annualized_returns[annualized_returns <= percentile_10].mean()
            cvar_95 = annualized_returns[annualized_returns <= percentile_5].mean()
            cvar_99 = annualized_returns[annualized_returns <= percentile_1].mean()

            # 计算期收益率的VaR和CVaR（用于VaR章节，更直观）
            # 期收益率即锁定期的实际收益率，不是年化收益率
            percentile_1_period = np.percentile(returns, 1)
            percentile_5_period = np.percentile(returns, 5)
            percentile_10_period = np.percentile(returns, 10)
            cvar_95_period = returns[returns <= percentile_5_period].mean()
            cvar_99_period = returns[returns <= percentile_1_period].mean()
            ...
            multi_window_mc_results[window_name] = {
                ...
                'profit_prob': (returns > 0).mean() * 100,
                'percentile_1': percentile_1,   # 1%分位数（左侧尾部最坏1%）
                'percentile_5': percentile_5,   # 5%分位数（左侧尾部最坏5%）
                'percentile_10': percentile_10, # 10%分位数（左侧尾部最坏10%）
                'percentile_25': percentile_25,
                'percentile_75': percentile_75,
                'percentile_95': percentile_95,  # 95%分位数（右侧尾部最好5%）
                'cvar_90': cvar_90,  # CVaR 90% (10%尾部平均)
                'cvar_95': cvar_95,  # CVaR 95% (5%尾部平均)
                'cvar_99': cvar_99,  # CVaR 99% (1%尾部平均)
                # 期收益率的VaR和CVaR（用于VaR章节）
                'var_95_period': abs(percentile_5_period),  # 期收益率VaR 95%
                'var_99_period': abs(percentile_1_period),  # 期收益率VaR 99%
                'cvar_95_period': abs(cvar_95_period),      # 期收益率CVaR 95%
                'cvar_99_period': abs(cvar_99_period),      # 期收益率CVaR 99%
            }
    ...
    window_headers = ['窗口期', '波动率', '历史年化收益率', '模拟预期年化收益', '中位数收益', '盈利概率', '5% VaR\n(最坏5%)', '1% VaR\n(最坏1%)']
    window_table_data = []
    ...
    for window_name in ['60日', '120日', '250日']:
        if window_name in multi_window_mc_results:
            ...
            window_table_data.append([
                ...
                f"{r['profit_prob']:.1f}%",
                f"{r['percentile_5']*100:.2f}%",     # 5%分位数（左侧尾部）
                f"{r['percentile_1']*100:.2f}%"      # 1%分位数（左侧尾部）
            ])
    ...
    add_paragraph(document, '')
    add_paragraph(document, '🔬 本节分析流程：')
    add_paragraph(document, '• 5.3.1 平稳性检验（ADF检验）- 确定差分阶数d')
    add_paragraph(document, '• 5.3.2 ACF/PACF分析 - 初步判断p和q的范围')
    add_paragraph(document, '• 5.3.3 最优阶数选择 - 使用AIC/BIC准则自动寻优')
    add_paragraph(document, '• 5.3.4 模型拟合与预测 - 进行120日预测')
    add_paragraph(document, '• 5.3.5 拟合曲线可视化 - 展示拟合效果和预测趋势')
    add_paragraph(document, '')
    ...
    if not skip_time_series:
        try:
            if prices_list and len(prices_list) >= 100:
                ...
                # ==================== 5.3.1 平稳性检验（ADF检验） ====================
                add_title(document, '5.3.1 平稳性检验（ADF检验）', level=3)

                add_paragraph(document, '平稳性是建立ARIMA模型的前提条件。ADF检验（Augmented Dickey-Fuller Test）用于检验时间序列是否存在单位根。')
                add_paragraph(document, '')
                add_paragraph(document, '📊 原假设与备择假设：')
                add_paragraph(document, '• H0（原假设）：序列存在单位根，非平稳')
                add_paragraph(document, '• H1（备择假设）：序列不存在单位根，平稳')
                add_paragraph(document, '• 判断标准：p值 < 0.05 时拒绝原假设，认为序列平稳')
                add_paragraph(document, '')

                # 执行ADF检验
                print("\n执行ADF平稳性检验...")
                adf_result = forecaster.adf_test()

                if 'error' not in adf_result:
                    # 添加ADF检验结果表格
                    adf_table_data = [
                        ['ADF统计量', f"{adf_result['adf_statistic']:.4f}"],
                        ['p值', f"{adf_result['p_value']:.4f}"],
                        ['平稳性判断', '✅ 序列平稳' if adf_result['is_stationary'] else '❌ 序列非平稳'],
                        [''],
                        ['1% 临界值', f"{adf_result['critical_values']['1%']:.4f}"],
                        ['5% 临界值', f"{adf_result['critical_values']['5%']:.4f}"],
                        ['10% 临界值', f"{adf_result['critical_values']['10%']:.4f}"]
                    ]
                    add_table_data(document, ['指标', '值'], adf_table_data)

                    add_paragraph(document, '')
                    add_paragraph(document, '📝 检验结论：', bold=True)
                    add_paragraph(document, f'• {adf_result["interpretation"]}')

                    if adf_result['is_stationary']:
                        add_paragraph(document, '• 对数收益率序列已平稳，可以直接建立ARIMA模型，d=0')
                    else:
                        add_paragraph(document, '• 序列非平稳，需要差分处理，建议d=1')
                else:
                    add_paragraph(document, '⚠️ ADF检验执行失败')

                # ==================== 5.3.2 ACF/PACF分析 ====================
                add_paragraph(document, '')
                add_title(document, '5.3.2 ACF/PACF分析', level=3)

                add_paragraph(document, '自相关函数（ACF）和偏自相关函数（PACF）用于初步判断ARIMA模型的阶数p和q。')
                add_paragraph(document, '')
                add_paragraph(document, '📊 判断原则：')
                add_paragraph(document, '• AR(p)模型：ACF拖尾（缓慢衰减），PACF在p阶后截尾（突然变为0）')
                add_paragraph(document, '• MA(q)模型：ACF在q阶后截尾，PACF拖尾')
                add_paragraph(document, '• ARMA(p,q)模型：ACF和PACF都拖尾')
                add_paragraph(document, '')
                ...
                try:
                    ...
                    log_returns = forecaster._calculate_returns()

                    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

                    # ACF图
                    plot_acf(log_returns, lags=40, ax=ax1, alpha=0.05)
                    ax1.set_title('自相关函数（ACF）', fontproperties=font_prop, fontsize=14, fontweight='bold')
                    ax1.set_xlabel('滞后期数', fontproperties=font_prop, fontsize=12)
                    ax1.set_ylabel('自相关系数', fontproperties=font_prop, fontsize=12)
                    for label in ax1.get_xticklabels():
                        label.set_fontproperties(font_prop)
                    for label in ax1.get_yticklabels():
                        label.set_fontproperties(font_prop)
                    ...
                    # PACF图
                    plot_pacf(log_returns, lags=40, ax=ax2, alpha=0.05)
                    ax2.set_title('偏自相关函数（PACF）', fontproperties=font_prop, fontsize=14, fontweight='bold')
                    ax2.set_xlabel('滞后期数', fontproperties=font_prop, fontsize=12)
                    ax2.set_ylabel('偏自相关系数', fontproperties=font_prop, fontsize=12)
                    for label in ax2.get_xticklabels():
                        label.set_fontproperties(font_prop)
                    for label in ax2.get_yticklabels():
                        label.set_fontproperties(font_prop)
                    ...
                    acf_pacf_chart_path = os.path.join(IMAGES_DIR, '05_03_02_acf_pacf.png')
                    plt.savefig(acf_pacf_chart_path, dpi=150, bbox_inches='tight')
                    plt.close()

                    add_paragraph(document, '图表 5.3.1: ACF与PACF图')
                    add_image(document, acf_pacf_chart_path, width=Inches(6))

                    add_paragraph(document, '')
                    add_paragraph(document, '📝 图形解读：')
                    add_paragraph(document, '• 蓝色阴影区域：95%置信区间，超出此范围表示统计显著')
                    add_paragraph(document, '• 观察ACF和PACF的截尾或拖尾特征，初步判断p和q的范围')
                    add_paragraph(document, '• 如果ACF和PACF都快速衰减到0，说明p和q都不大（建议0-3）')

                    print(f"✅ 已生成ACF/PACF图: {acf_pacf_chart_path}")

                except Exception as e:
                    print(f"⚠️ 生成ACF/PACF图失败: {e}")
                    add_paragraph(document, f'⚠️ ACF/PACF图生成失败: {e}')

                # ==================== 5.3.3 最优阶数选择 ====================
                add_paragraph(document, '')
                add_title(document, '5.3.3 最优阶数选择', level=3)

                add_paragraph(document, '基于AIC（赤池信息准则）自动搜索最优的(p,d,q)组合。AIC值越小，模型拟合越好且不过拟合。')
                add_paragraph(document, '')
                add_paragraph(document, '🔍 搜索范围：')
                add_paragraph(document, '• p（AR阶数）：0-3')
                add_paragraph(document, '• d（差分阶数）：0-2')
                add_paragraph(document, '• q（MA阶数）：0-3')
                add_paragraph(document, '• 总计测试组合数：最多4×3×4=48种')
                add_paragraph(document, '')

                # ARIMA预测（包含自动寻优）
                print("\n运行ARIMA模型预测漂移率...")
                arima_result = forecaster.forecast_drift_with_arima(
                    horizon=120,
                    auto_find_order=True,
                    max_p=3,
                    max_d=2,
                    max_q=3
                )
                ...
                # 显示最优阶数
                if 'order_selection' in arima_result and arima_result['order_selection']:
                    order_selection = arima_result['order_selection']
                    optimal_order = arima_result['order_used']

                    add_paragraph(document, '📊 最优模型选择结果：', bold=True)
                    add_paragraph(document, f'• 最优阶数：ARIMA{optimal_order}')
                    add_paragraph(document, f'• AIC值：{order_selection["best_ic"]:.2f}')

                    # 显示前5个最优模型对比
                    if order_selection.get('top_models'):
                        top_models = order_selection['top_models']
                        comparison_table = [['排名', '阶数(p,d,q)', 'AIC', 'BIC']]
                        ...
                        add_paragraph(document, '')
                        add_paragraph(document, '前5个最优模型对比：', bold=True)
                        add_table_data(document, ['排名', '阶数(p,d,q)', 'AIC', 'BIC'], comparison_table[1:])

                        add_paragraph(document, '')
                        add_paragraph(document, '说明：')
                        add_paragraph(document, '• AIC和BIC都越小越好，但AIC更侧重拟合优度，BIC更侧重模型简洁性')
                        add_paragraph(document, '• 通常选择AIC最小的模型，如果AIC相近，选择BIC更小的模型（阶数更简单）')
                else:
                    add_paragraph(document, f'• 使用阶数：ARIMA{arima_result["order_used"]}（固定或降级）')

                # ==================== 5.3.4 模型拟合与预测 ====================
                add_paragraph(document, '')
                add_title(document, '5.3.4 模型拟合与预测', level=3)

                add_paragraph(document, '基于最优阶数建立ARIMA模型，对未来120日进行预测。')
                add_paragraph(document, '')

                # 添加模型设置说明
                add_paragraph(document, '模型设置：', bold=True)
                arima_params = [
                    ['最优阶数', f'ARIMA{arima_result["order_used"]}'],
                    ['预测期数', '120日（半年）'],
                ...
                forecast_drift_pct = arima_result["forecast_drift"] * 100

                add_paragraph(document, f'• 预测漂移率（年化）：{forecast_drift_pct:.2f}%')
                add_paragraph(document, f'• 预测期间：未来120个交易日（约半年）')
                add_paragraph(document, f'• 含义：如果模型预测准确，未来半年股价预期年化收益率为{forecast_drift_pct:.2f}%')

                # 如果预测值偏低，添加说明
                if forecast_drift_pct < 5.0:
                    add_paragraph(document, '')
                    add_paragraph(document, '关于ARIMA预测值的说明：', bold=True)
                    add_paragraph(document, f'• ARIMA预测年化收益率为{forecast_drift_pct:.2f}%，{"低于" if forecast_drift_pct < 3 else "接近"}历史平均水平')
                    add_paragraph(document, '• ARIMA模型特性：倾向于预测"均值回归"，对持续趋势的预测能力有限')
                    add_paragraph(document, '• 预测基于过去{len(prices_series)}个交易日的数据，反映了近期市场表现')
                    add_paragraph(document, '• 建议：结合历史数据（60日/120日/250日窗口）综合判断，不单独依赖ARIMA预测')
                    add_paragraph(document, f'• 历史对比：60日{mc_drift_60d*100:.2f}%、120日{mc_drift_120d*100:.2f}%、250日{mc_drift_250d*100:.2f}%')
                ...
                if arima_result['model_fitted']:
                    add_paragraph(document, '')
                    add_title(document, '5.3.5 拟合曲线可视化', level=3)

                    add_paragraph(document, '通过可视化图表直观展示ARIMA模型的拟合效果和预测趋势。')
                    add_paragraph(document, '')
                    ...
                    try:
                        ...
                        # 获取数据
                        log_returns = arima_result['log_returns']
                        fitted_model = arima_result['fitted_model']
                        forecast_series = arima_result['forecast_series']
                        fitted_values = fitted_model.fittedvalues  # 获取拟合值

                        # 历史数据的索引（从0开始）
                        historical_index = range(len(log_returns))

                        # 预测数据的索引（从历史数据末尾开始）
                        forecast_index = range(len(log_returns), len(log_returns) + len(forecast_series))

                        # 图1: 收益率百分比（左轴）+ 价格（右轴）双Y轴图表
                        # 将对数收益率转换为收益率百分比，更直观
                        returns_pct = np.exp(log_returns.values) - 1  # 对数收益率 → 简单收益率
                        fitted_values_pct = np.exp(fitted_values.values) - 1  # 拟合值转换
                        forecast_pct = np.exp(forecast_series.values) - 1  # 预测值转换

                        # 左轴：收益率百分比
                        ax1.plot(historical_index, returns_pct * 100, 'o-', alpha=0.5, markersize=3,
                                label='历史日收益率', color='#3498db', linewidth=1)

                        # 绘制拟合值（in-sample prediction）
                        fitted_index = range(len(fitted_values))
                        ax1.plot(fitted_index, fitted_values_pct * 100, '-', linewidth=1.5,
                                label='ARIMA拟合收益率', color='#e74c3c', alpha=0.8)

                        # 绘制预测值
                        ax1.plot(forecast_index, forecast_pct * 100, '--', linewidth=2,
                                label='ARIMA预测收益率', color='#27ae60', alpha=0.8)

                        # 添加历史均值参考线
                        mean_return_pct = returns_pct.mean()
                        ax1.axhline(y=mean_return_pct * 100, color='gray', linestyle=':', linewidth=1.5,
                                   label=f'历史均值: {mean_return_pct*100:.2f}%', alpha=0.6)

                        # 添加零线
                        ax1.axhline(y=0, color='black', linestyle='-', linewidth=0.8, alpha=0.3)

                        ax1.set_xlabel('交易日', fontproperties=font_prop, fontsize=12)
                        ax1.set_ylabel('日收益率（%）', fontproperties=font_prop, fontsize=12, color='#3498db')
                        ax1.set_title('ARIMA模型 - 日收益率拟合与预测（右轴：对应价格）',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ...
                        # 绘制价格曲线（半透明，作为参考）
                        # 注意：log_returns比prices_series少一个元素（因为pct_change().dropna()）
                        prices_for_plot = prices_series.iloc[1:].values  # 去掉第一个元素，与log_returns长度匹配
                        ax1_right.plot(historical_index, prices_for_plot, '-',
                                       color='gray', alpha=0.3, linewidth=1.5, label='价格参考')

                        # 绘制拟合价格
                        fitted_cumsum = fitted_values.cumsum()
                        fitted_prices = prices_series.iloc[0] * np.exp(fitted_cumsum.values)
                        ax1_right.plot(fitted_index, fitted_prices,
                                       '-', color='#e74c3c', alpha=0.3, linewidth=1)

                        # 绘制预测价格
                        forecast_cumsum = forecast_series.cumsum()
                        forecast_prices_ref = np.concatenate([
                            [prices_series.iloc[-1]],
                            prices_series.iloc[-1] * np.exp(forecast_cumsum.values)
                        ])
                        forecast_x_ref = range(len(log_returns), len(log_returns) + len(forecast_prices_ref))
                        ax1_right.plot(forecast_x_ref, forecast_prices_ref,
                                       '--', color='#27ae60', alpha=0.3, linewidth=1.5)

                        ax1_right.set_ylabel('价格（元）', fontproperties=font_prop, fontsize=12, color='gray')
                        ...
                        # 左轴图例
                        legend1 = ax1.legend(prop=font_prop, fontsize=9, loc='upper left')
                        # 右轴图例（单独显示）
                        ax1_right.legend([plt.Line2D([0], [0], color='gray', alpha=0.3, linewidth=1.5)],
                                         ['价格参考'], prop=font_prop, fontsize=9, loc='upper right')

                        # 图2: 价格拟合与预测（还原为具体价格，更直观）
                        # 获取初始价格
                        initial_price = prices_series.iloc[0]
                        last_price = prices_series.iloc[-1]

                        # 历史价格
                        # 注意：log_returns比prices_series少一个元素（因为pct_change().dropna()）
                        prices_for_plot = prices_series.iloc[1:].values  # 去掉第一个元素，与log_returns长度匹配
                        ax2.plot(historical_index, prices_for_plot, '-',
                                label='历史价格', color='#3498db', linewidth=2, alpha=0.7)

                        # ARIMA拟合价格（in-sample）
                        # fitted_values是对数收益率的拟合值，需要还原为价格
                        fitted_cumsum = fitted_values.cumsum()
                        fitted_prices = initial_price * np.exp(fitted_cumsum.values)
                        ax2.plot(fitted_index, fitted_prices,
                                '-', linewidth=1.5, label='ARIMA拟合价格', color='#e74c3c', alpha=0.8)

                        # ARIMA预测价格（out-of-sample）
                        # 从最后一个历史价格开始，用预测的对数收益率累加
                        forecast_cumsum = forecast_series.cumsum()
                        forecast_prices = np.concatenate([
                            [last_price],  # 预测起点（等于最后一个历史价格）
                            last_price * np.exp(forecast_cumsum.values)  # 后续预测价格
                        ])

                        # 预测部分的x轴从历史终点后开始
                        forecast_x_start = len(log_returns)
                        forecast_x = range(forecast_x_start, forecast_x_start + len(forecast_prices))

                        ax2.plot(forecast_x, forecast_prices,
                                '--', linewidth=2, label='ARIMA预测价格', color='#27ae60', alpha=0.8)

                        # 标注预测起点
                        ax2.axvline(x=len(log_returns), color='red', linestyle='--', linewidth=1.5,
                                   label='预测起点', alpha=0.5)

                        # 标注当前价格
                        ax2.axhline(y=last_price, color='gray', linestyle=':', linewidth=1.5,
                                   label=f'当前价格: {last_price:.2f}元', alpha=0.6)

                        ax2.set_xlabel('交易日', fontproperties=font_prop, fontsize=12)
                        ax2.set_ylabel('价格（元）', fontproperties=font_prop, fontsize=12)
                        ax2.set_title('ARIMA模型 - 价格拟合与预测（120日）',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ...
                        # 保存图表
                        arima_chart_path = os.path.join(IMAGES_DIR, '05_03_05_arima_fit_forecast.png')
                        plt.savefig(arima_chart_path, dpi=150, bbox_inches='tight')
                        plt.close()

                        add_paragraph(document, '图表 5.3.2: ARIMA模型拟合与预测曲线')
                        add_image(document, arima_chart_path, width=Inches(6))

                        add_paragraph(document, '')
                        add_paragraph(document, '📊 图表解读：')
                        add_paragraph(document, '• 上图：展示历史对数收益率、ARIMA模型拟合值（红色）和未来120日预测值（绿色虚线）')
                        add_paragraph(document, '• 下图：累积收益率曲线，更直观地展示价格趋势')
                        add_paragraph(document, '• 灰色虚线：历史平均收益率，作为参考基准')
                        add_paragraph(document, '• 如果预测曲线向上倾斜，表示预期未来股价上涨；向下倾斜表示预期下跌')
                        add_paragraph(document, f'• 未来半年预测累积收益: {forecast_series.sum():.4f}（对数收益率）')
                        add_paragraph(document, f'• 折算年化漂移率: {arima_result["forecast_drift"]*100:.2f}%')

                        print(f"✅ 已生成ARIMA拟合曲线图: {arima_chart_path}")

                    except Exception as e:
                        print(f"⚠️ 生成ARIMA拟合图失败: {e}")
                        import traceback
                        traceback.print_exc()
    ...
    add_paragraph(document, '')
    add_paragraph(document, '🔬 本节分析流程：')
    add_paragraph(document, '• 5.4.1 ARCH效应检验（残差平方ACF/PACF + ARCH-LM检验）- 确认是否需要GARCH模型')
    add_paragraph(document, '• 5.4.2 最优阶数选择 - 使用AIC/BIC准则自动寻优')
    add_paragraph(document, '• 5.4.3 模型拟合与预测 - 进行120日波动率预测')
    add_paragraph(document, '• 5.4.4 模型诊断 - 检验标准化残差是否无自相关')
    add_paragraph(document, '• 5.4.5 拟合曲线可视化 - 展示条件波动率和预测趋势')
    add_paragraph(document, '')
    ...
    if not skip_time_series:
        try:
            if 'forecaster' in locals():
                ...
                # ==================== 5.4.1 ARCH效应检验 ====================
                add_title(document, '5.4.1 ARCH效应检验', level=3)

                add_paragraph(document, 'ARCH效应（自回归条件异方差）是使用GARCH模型的前提条件。')
                add_paragraph(document, '')
                add_paragraph(document, '📊 检验步骤：')
                add_paragraph(document, '1. 绘制残差平方的ACF/PACF图 - 初步判断是否存在ARCH效应')
                add_paragraph(document, '2. ARCH-LM检验 - 统计检验确认ARCH效应的存在性')
                add_paragraph(document, '')

                # 生成残差平方的ACF/PACF图
                try:
                    returns = forecaster.prices.pct_change().dropna() * 100
                    ...
                    # 残差平方的ACF图
                    from statsmodels.graphics.tsaplots import plot_acf, plot_pacf
                    plot_acf(squared_returns, lags=40, ax=ax1, alpha=0.05)
                    ax1.set_title('残差平方的ACF（检验ARCH效应）', fontproperties=font_prop, fontsize=14, fontweight='bold')
                    ax1.set_xlabel('滞后期数', fontproperties=font_prop, fontsize=12)
                    ax1.set_ylabel('自相关系数', fontproperties=font_prop, fontsize=12)
                    for label in ax1.get_xticklabels():
                        label.set_fontproperties(font_prop)
                    ...
                    # 残差平方的PACF图
                    plot_pacf(squared_returns, lags=40, ax=ax2, alpha=0.05)
                    ax2.set_title('残差平方的PACF（初步判断p和q）', fontproperties=font_prop, fontsize=14, fontweight='bold')
                    ax2.set_xlabel('滞后期数', fontproperties=font_prop, fontsize=12)
                    ax2.set_ylabel('偏自相关系数', fontproperties=font_prop, fontsize=12)
                    for label in ax2.get_xticklabels():
                        label.set_fontproperties(font_prop)
                    ...
                    squared_acf_pacf_path = os.path.join(IMAGES_DIR, '05_04_01_squared_returns_acf_pacf.png')
                    plt.savefig(squared_acf_pacf_path, dpi=150, bbox_inches='tight')
                    plt.close()

                    add_paragraph(document, '图表 5.4.1: 残差平方的ACF与PACF图')
                    add_image(document, squared_acf_pacf_path, width=Inches(6))

                    add_paragraph(document, '')
                    add_paragraph(document, '📝 图形解读：')
                    add_paragraph(document, '• 如果ACF缓慢衰减，说明存在波动率聚集现象，需要使用GARCH模型')
                    add_paragraph(document, '• 通过PACF可以初步判断p和q的阶数（通常在1-2之间）')

                    print(f"✅ 已生成残差平方ACF/PACF图: {squared_acf_pacf_path}")

                except Exception as e:
                    print(f"⚠️ 生成残差平方ACF/PACF图失败: {e}")
                    add_paragraph(document, f'⚠️ ACF/PACF图生成失败: {e}')

                # ARCH-LM检验
                add_paragraph(document, '')
                add_paragraph(document, 'ARCH-LM检验结果：', bold=True)

                print("\n执行ARCH-LM检验...")
                arch_lm_result = forecaster.arch_lm_test()

                if 'error' not in arch_lm_result:
                    arch_lm_table = [
                        ['LM统计量', f"{arch_lm_result['lm_statistic']:.4f}"],
                        ['p值', f"{arch_lm_result['p_value']:.4f}"],
                        ['ARCH效应', '✅ 存在' if arch_lm_result['has_arch_effect'] else '❌ 不存在']
                    ]
                    add_table_data(document, ['指标', '值'], arch_lm_table)

                    add_paragraph(document, '')
                    add_paragraph(document, '📝 检验结论：', bold=True)
                    add_paragraph(document, f'• {arch_lm_result["interpretation"]}')
                else:
                    add_paragraph(document, '⚠️ ARCH-LM检验执行失败')

                # ==================== 5.4.2 最优阶数选择 ====================
                add_paragraph(document, '')
                add_title(document, '5.4.2 最优阶数选择', level=3)

                add_paragraph(document, '基于AIC（赤池信息准则）自动搜索最优的(p,q)组合。')
                add_paragraph(document, '')
                add_paragraph(document, '🔍 搜索范围：')
                add_paragraph(document, '• p（ARCH阶数）：1-2')
                add_paragraph(document, '• q（GARCH阶数）：1-2')
                add_paragraph(document, '• 总计测试组合数：最多2×2=4种')
                add_paragraph(document, '')

                # GARCH预测（包含自动寻优）
                print("\n运行GARCH模型预测波动率...")
                garch_result = forecaster.forecast_volatility_with_garch(
                    horizon=120,
                    auto_find_order=True,
                    max_p=2,
                    max_q=2
                ...
                # 显示最优阶数
                if 'order_selection' in garch_result and garch_result['order_selection']:
                    order_selection = garch_result['order_selection']
                    optimal_order = garch_result['order_used']

                    add_paragraph(document, '📊 最优模型选择结果：', bold=True)
                    add_paragraph(document, f'• 最优阶数：GARCH({optimal_order[0]},{optimal_order[1]})')
                    add_paragraph(document, f'• AIC值：{order_selection["best_ic"]:.2f}')

                    # 显示前5个最优模型对比
                    if order_selection.get('top_models'):
                        top_models = order_selection['top_models']
                        comparison_table = [['排名', '阶数(p,q)', 'AIC', 'BIC', 'α+β(持续性)']]
                        ...
                        for i, model in enumerate(top_models[:5], 1):
                            ...
                            comparison_table.append([
                                ...
                                f"{model['aic']:.2f}",
                                f"{model['bic']:.2f}",
                                f"{model['persistence']:.4f}"
                            ])

                        add_paragraph(document, '')
                        add_paragraph(document, '所有候选模型对比：', bold=True)
                        add_table_data(document, ['排名', '阶数(p,q)', 'AIC', 'BIC', 'α+β'], comparison_table[1:])

                        add_paragraph(document, '')
                        add_paragraph(document, '说明：')
                        add_paragraph(document, '• AIC和BIC都越小越好，同时考虑α+β的值（越接近1表示波动持续性越强）')
                        add_paragraph(document, '• 通常选择AIC最小的模型，同时确保α+β < 1（模型平稳）')
                else:
                    add_paragraph(document, f'• 使用阶数：GARCH{garch_result["order_used"]}（固定或降级）')

                # ==================== 5.4.3 模型拟合与预测 ====================
                add_paragraph(document, '')
                add_title(document, '5.4.3 模型拟合与预测', level=3)

                add_paragraph(document, '基于最优阶数建立GARCH模型，对未来120日波动率进行预测。')
                add_paragraph(document, '')

                # 添加模型设置说明
                add_paragraph(document, '模型设置：', bold=True)
                garch_params = [
                    ['最优阶数', f'GARCH{garch_result["order_used"]}'],
                    ['预测期数', '120日（半年）'],
                    ['数据量', f'{len(prices_series)}个交易日'],
                    ['拟合状态', '✅ 成功' if garch_result['model_fitted'] else '⚠️ 失败（使用历史波动率）']
                ]
                if garch_result['model_fitted']:
                    garch_params.append(['AIC准则', f"{garch_result.get('aic', 'N/A')}"])
                ...
                if garch_result['model_fitted']:
                    ...
                    model_params = [
                        ...
                        ['α+β（持续性）', f"{garch_result['persistence']:.4f}"]
                    ]
                    add_table_data(document, ['参数', '值'], model_params)

                    add_paragraph(document, '')
                    add_paragraph(document, '参数解读：', bold=True)
                    add_paragraph(document, f'• α+β = {garch_result["persistence"]:.4f}')
                    if garch_result['persistence'] < 1:
                        add_paragraph(document, '  ✅ < 1，模型平稳，波动率会逐渐回归长期均值')
                        half_life = -np.log(0.5) / (1 - garch_result['persistence'])
                        add_paragraph(document, f'• 波动率半衰期：约{half_life:.0f}日（即冲击影响衰减一半所需时间）')
                    else:
                        add_paragraph(document, '  ⚠️ ≥ 1，模型非平稳，波动率可能发散（需要检查数据）')

                    add_paragraph(document, f'• α = {garch_result["alpha"]:.4f}：新信息对波动率的冲击强度')
                    add_paragraph(document, f'• β = {garch_result["beta"]:.4f}：波动率的持续性强度')
                ...
                if garch_result['model_fitted']:
                    add_paragraph(document, '')
                    add_title(document, '5.4.4 模型诊断', level=3)

                    add_paragraph(document, '检验GARCH模型的拟合质量，主要检查标准化残差是否无自相关（即已充分提取波动率信息）。')
                    add_paragraph(document, '')
                    ...
                    try:
                        ...
                        # 获取标准化残差
                        std_residuals = garch_result['standardized_residuals'].dropna()

                        # 对标准化残差做ARCH-LM检验
                        lm_stat, p_val, _, _ = het_arch(std_residuals, nlags=10)

                        # 根据p值生成解释文字
                        if p_val >= 0.05:
                            interpretation = '✅ 标准化残差无ARCH效应，说明GARCH模型已充分提取波动率聚集效应，模型拟合充分'
                        else:
                            interpretation = '⚠️ 标准化残差仍有ARCH效应，说明GARCH模型未充分提取波动率信息，可能需要提高模型阶数'

                        diagnostic_table = [
                            ['检验项目', '统计量', 'p值', '结论'],
                            ['标准化残差ARCH-LM检验', f'{lm_stat:.4f}', f'{p_val:.4f}',
                             '✅ 无ARCH效应' if p_val >= 0.05 else '❌ 仍有ARCH效应'],
                            ['', '', '', ''],
                            ['解释', interpretation, '', '']
                        ]

                        add_table_data(document, ['检验项目', '统计量', 'p值', '结论'], diagnostic_table[1:])

                        add_paragraph(document, '')
                        add_paragraph(document, '📝 诊断结论：', bold=True)
                        if p_val >= 0.05:
                            add_paragraph(document, '• ✅ 标准化残差已无ARCH效应（p >= 0.05），模型拟合充分')
                            add_paragraph(document, '• ✅ GARCH模型成功提取了波动率中的聚集效应')
                        else:
                            add_paragraph(document, '• ⚠️ 标准化残差仍有ARCH效应，可能需要更高阶的GARCH模型')

                        # 绘制标准化残差图
                        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 8))

                        # 标准化残差时序图
                        ax1.plot(std_residuals.index, std_residuals.values, 'o-', alpha=0.5, markersize=2)
                        ax1.axhline(y=0, color='red', linestyle='--', linewidth=1, alpha=0.7)
                        ax1.axhline(y=2, color='gray', linestyle=':', linewidth=1, alpha=0.5)
                        ax1.axhline(y=-2, color='gray', linestyle=':', linewidth=1, alpha=0.5)
                        ax1.set_xlabel('时间', fontproperties=font_prop, fontsize=12)
                        ax1.set_ylabel('标准化残差', fontproperties=font_prop, fontsize=12)
                        ax1.set_title('标准化残差时序图（应在±2之间）',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ax1.grid(True, alpha=0.3)
                        for label in ax1.get_xticklabels():
                            label.set_fontproperties(font_prop)
                        ...
                        plot_acf(std_residuals, lags=40, ax=ax2, alpha=0.05)
                        ax2.set_title('标准化残差的ACF（应无显著自相关）',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ax2.set_xlabel('滞后期数', fontproperties=font_prop, fontsize=12)
                        ax2.set_ylabel('自相关系数', fontproperties=font_prop, fontsize=12)
                        for label in ax2.get_xticklabels():
                            label.set_fontproperties(font_prop)
                        for label in ax2.get_yticklabels():
                            label.set_fontproperties(font_prop)
                        ...
                        diagnostic_chart_path = os.path.join(IMAGES_DIR, '05_04_04_garch_diagnostic.png')
                        plt.savefig(diagnostic_chart_path, dpi=150, bbox_inches='tight')
                        plt.close()

                        add_paragraph(document, '图表 5.4.2: GARCH模型诊断图')
                        add_image(document, diagnostic_chart_path, width=Inches(6))

                        print(f"✅ 已生成GARCH模型诊断图: {diagnostic_chart_path}")

                    except Exception as e:
                        print(f"⚠️ 生成GARCH模型诊断失败: {e}")
                        import traceback
                        traceback.print_exc()
                ...
                if garch_result['model_fitted']:
                    add_paragraph(document, '')
                    add_title(document, '5.4.5 拟合曲线可视化', level=3)

                    add_paragraph(document, '通过可视化图表直观展示GARCH模型的条件波动率拟合效果和预测趋势。')
                    add_paragraph(document, '')
                    ...
                    try:
                        ...
                        # 统一使用百分比形式（日数据）
                        returns = forecaster.prices.pct_change().dropna() * 100  # 日收益率百分比
                        conditional_vol = (garch_result['conditional_variances'] ** 0.5) * 100  # 日波动率百分比

                        # 绘制收益率和条件波动率（都是日百分比形式，量纲一致）
                        ax.plot(returns.index, returns.values, 'o-', alpha=0.3, markersize=2,
                               label='日收益率（%）', color='#3498db', linewidth=0.5)
                        ax.plot(conditional_vol.index, conditional_vol.values, '-',
                               label='日波动率（%，GARCH拟合）', color='#e74c3c', linewidth=1.5)

                        # 计算长期平均波动率（日波动率和年化波动率）
                        long_run_vol_daily = np.sqrt(garch_result['omega'] / (1 - garch_result['persistence'])) * 100  # 转百分比
                        long_run_vol_annual = (long_run_vol_daily / 100) * np.sqrt(250) * 100  # 先转小数，年化，再转百分比

                        # 添加长期平均波动率线（日波动率）
                        ax.axhline(y=long_run_vol_daily, color='green', linestyle='--', linewidth=1.5,
                                  label=f'长期平均日波动率: {long_run_vol_daily:.2f}%', alpha=0.7)

                        # 添加±2倍条件波动率带
                        ax.fill_between(conditional_vol.index,
                                        conditional_vol.values * (-2),
                                        conditional_vol.values * 2,
                                        alpha=0.2, color='gray', label='±2倍日波动率')

                        ax.set_xlabel('时间', fontproperties=font_prop, fontsize=12)
                        ax.set_ylabel('百分比 (%)', fontproperties=font_prop, fontsize=12)
                        ax.set_title('GARCH模型 - 日波动率拟合曲线（量纲统一：均为日百分比）',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ...
                        garch_chart_path = os.path.join(IMAGES_DIR, '05_04_05_garch_fit_forecast.png')
                        plt.savefig(garch_chart_path, dpi=150, bbox_inches='tight')
                        plt.close()

                        add_paragraph(document, '图表 5.4.3: GARCH模型条件波动率拟合曲线')
                        add_image(document, garch_chart_path, width=Inches(6))

                        add_paragraph(document, '')
                        add_paragraph(document, '📊 图表解读（所有数据均为日百分比，量纲统一）：', bold=True)
                        add_paragraph(document, '• 蓝色点：实际日收益率（如+2%表示当日上涨2%）')
                        add_paragraph(document, '• 红色线：GARCH拟合的日波动率（反映每日波动程度的变化）')
                        add_paragraph(document, f'• 绿色虚线：长期平均日波动率: {long_run_vol_daily:.2f}%')
                        add_paragraph(document, f'• 灰色带：±2倍日波动率区间（约95%置信区间，大多数收益率应在此范围内）')
                        add_paragraph(document, '')
                        add_paragraph(document, '📈 年化数据（用于对比和预测）：', bold=True)
                        add_paragraph(document, f'• 长期平均年化波动率: {long_run_vol_annual:.2f}%')
                        add_paragraph(document, f'• 预测年化波动率（未来120日）: {garch_result["forecast_volatility"]*100:.2f}%')
                        add_paragraph(document, f'• 转换公式：年化波动率 = 日波动率 × √250 ≈ 日波动率 × 15.81')
                        add_paragraph(document, '')
                        add_paragraph(document, '说明：', bold=True)
                        add_paragraph(document, f'• 图表中所有曲线使用相同量纲（日百分比），可直接对比')
                        add_paragraph(document, f'• 例如：某日收益率+2%，当日波动率3%，说明该日波动超出平均水平')
                        add_paragraph(document, f'• 预测时将日波动率年化，便于与历史数据（60日/120日/250日）对比')

                        print(f"✅ 已生成GARCH拟合曲线图: {garch_chart_path}")

                    except Exception as e:
                        print(f"⚠️ 生成GARCH拟合图失败: {e}")
                        import traceback
                        traceback.print_exc()
    ...
    if not skip_time_series:
        ...
        # 使用预测参数
        # 注意：ARIMA预测的是对数漂移率，需要转换为价格漂移率才能用于GBM模拟
        # 根据伊藤引理：price_drift = log_drift + 0.5 * σ²
        log_drift_predicted = arima_result['forecast_drift']
        predicted_vol = garch_result['forecast_volatility']

        # 对数漂移率 → 价格漂移率的转换
        price_drift_predicted = log_drift_predicted + 0.5 * predicted_vol**2
        ...
        print(f"  ARIMA预测对数漂移率: {log_drift_predicted*100:.2f}%")
        print(f"  GARCH预测波动率: {predicted_vol*100:.2f}%")
        print(f"  转换后价格漂移率: {price_drift_predicted*100:.2f}% (对数漂移率 + 0.5×σ²)")

        sim_predicted = analyzer.monte_carlo_simulation(
            n_simulations=n_simulations,
            time_steps=time_steps,
            volatility=predicted_vol,
            drift=price_drift_predicted,  # 使用转换后的价格漂移率
            seed=42
        )
        ...
        mc_5_5_params = [
            ['ARIMA对数漂移率', f'{log_drift_predicted*100:.2f}%'],
            ['GARCH波动率', f'{predicted_vol*100:.2f}%'],
            ['GBM价格漂移率', f'{price_drift_predicted*100:.2f}%（对数漂移率+0.5×σ²）'],
        ...
        add_paragraph(document, f'• 预期年化收益率为{mean_return*100:.2f}%（算术平均值），{"高于" if mean_return > 0 else "低于"}无风险收益')
        add_paragraph(document, f'• 中位数收益率为{median_return*100:.2f}%（更稳健的预期）')

        # 添加技术说明（伊藤引理转换）
        add_paragraph(document, '')
        add_paragraph(document, '技术说明：', bold=True)
        add_paragraph(document, f'• ARIMA模型预测的是对数收益率的漂移率：{log_drift_predicted*100:.2f}%')
        add_paragraph(document, f'• 根据伊藤引理，GBM模拟需要价格漂移率：{price_drift_predicted*100:.2f}%')
        add_paragraph(document, f'• 转换公式：价格漂移率 = 对数漂移率 + 0.5 × σ² = {log_drift_predicted*100:.2f}% + {0.5 * predicted_vol**2 * 100:.2f}%')

        # 如果均值和中位数差距较大，添加说明
        if abs(mean_return - median_return) > 0.10:  # 差距超过10个百分点
            add_paragraph(document, '')
            add_paragraph(document, '⚠️ 均值与中位数差异较大：', bold=True)
            if mean_return > median_return:
                add_paragraph(document, f'  • 均值（{mean_return*100:.2f}%）> 中位数（{median_return*100:.2f}%）= {abs(mean_return - median_return)*100:.2f}个百分点')
                add_paragraph(document, '  • 说明：分布右偏，少数高收益场景拉高了均值')
                add_paragraph(document, f'  • 实际投资中，中位数（{median_return*100:.2f}%）更能代表典型情况')
            else:
                add_paragraph(document, f'  • 均值（{mean_return*100:.2f}%）< 中位数（{median_return*100:.2f}%）= {abs(mean_return - median_return)*100:.2f}个百分点')
                add_paragraph(document, '  • 说明：分布左偏，少数低收益场景拉低了均值')
        ...
        comparison_table = [
            ...
            ['预测参数（ARIMA+GARCH）',
             f'{price_drift_predicted*100:.2f}%',
            ...
            ['', '', '', '', ''],
            ['差异',
             f'{(price_drift_predicted - historical_drift)*100:+.2f}%',
        ...
        add_paragraph(document, '')
        add_paragraph(document, '对比分析：', bold=True)
        add_paragraph(document, f'• 漂移率：预测参数{"高于" if price_drift_predicted > historical_drift else "低于"}历史参数，差{abs(price_drift_predicted - historical_drift)*100:.2f}个百分点')
    ...
    for drift in drift_rates:
        for vol in volatilities:
            for discount in discounts:
                ...
                # 计算实际溢价率（相对当前价格）
                actual_premium_rate = (issue_price - current_price) / current_price
                ...
                all_scenarios.append({
                    ...
                    'volatility': vol,
                    'discount': discount,
                    'actual_premium_rate': actual_premium_rate,
                    'issue_price': issue_price,
    ...
    for i, s in enumerate(top_by_return, 1):
        top_return_data.append([
            ...
            f"{s['discount']*100:+.0f}%",
            f"{s['actual_premium_rate']*100:+.1f}%",
            f"{s['issue_price']:.2f} 元",
            f"{s['mean_return']*100:+.2f}%",
            f"{s['profit_prob']:.1f}%"
        ])

    return_headers = ['排名', '漂移率', '波动率', '溢价率(名义)', '溢价率(实际)', '发行价', '预期年化收益', '盈利概率']
    ...
    if best_scenarios:
        ...
        add_table_data(document, ['排名', '漂移率', '波动率', '溢价率(名义)', '溢价率(实际)', '预期年化收益', '盈利概率'], best_data)
    else:
    ...
    add_paragraph(document, '本节基于市场指数和行业的历史数据，构建6个典型情景进行蒙特卡洛模拟分析，评估不同市场环境下的定增收益。')
    add_paragraph(document, '')
    add_paragraph(document, '📊 本节包含6个情景：')
    add_paragraph(document, '• **指数情景（3个）**：基于沪深300、中证500等主要指数历史数据')
    add_paragraph(document, '  - 情景1（指数乐观）：指数历史上涨趋势明显、波动较低')
    add_paragraph(document, '  - 情景2（指数中性）：指数历史平稳、波动适中')
    add_paragraph(document, '  - 情景3（指数悲观）：指数历史下跌趋势、波动较高')
    add_paragraph(document, '')
    add_paragraph(document, '• **行业情景（3个）**：基于所属行业的历史数据')
    add_paragraph(document, '  - 情景4（行业乐观）：行业历史上涨趋势明显、波动较低')
    add_paragraph(document, '  - 情�景5（行业中性）：行业历史平稳、波动适中')
    add_paragraph(document, '  - 情景6（行业悲观）：行业历史下跌趋势、波动较高')
    add_paragraph(document, '')
    add_paragraph(document, '每个情景分别使用120日窗口的历史参数进行蒙特卡洛模拟，计算盈利概率和收益率中位数。')
    add_paragraph(document, '')
    ...
    # ===== 情景2-4: 指数情景（基于沪深300、中证500、创业板指、科创50） =====
    if indices_data_available:
        # 从指数数据中提取所有指数的波动率和收益率，用于创建乐观/中性/悲观情景
        major_indices = ['沪深300', '中证500', '创业板指', '科创50']
        index_vol_list = []
        index_return_list = []
        ...
        for index_name in major_indices:
            if index_name in indices_data_loaded:
                index_vol_list.append(indices_data_loaded[index_name]['volatility_120d'])
                index_return_list.append(indices_data_loaded[index_name]['return_120d'])

        if index_vol_list and index_return_list:
            # 情景2: 指数乐观（高漂移+低波动+深折价）
            index_opt_drift_raw = max(index_return_list)
            index_opt_vol_raw = min(index_vol_list)
            index_optimistic_drift = index_opt_drift_raw * 1.1  # 取最高漂移率并放大10%
            index_optimistic_vol = index_opt_vol_raw * 0.9      # 取最低波动率并缩小10%

            scenarios_config.append({
                'name': '情景1（指数乐观）',
                'description': f'主要指数漂移率最高值{index_opt_drift_raw*100:+.2f}%放大10%至{index_optimistic_drift*100:+.2f}%、波动率最低值{index_opt_vol_raw*100:.2f}%缩小10%至{index_optimistic_vol*100:.2f}%、溢价率-20%（深折价）',
                'volatility': index_optimistic_vol,
                'drift': index_optimistic_drift,
                'premium_rate': -0.20
            })

            # 情景3: 指数中性（中位数漂移+中位数波动+中等折价）
            index_neutral_drift = np.median(index_return_list)  # 指数中位数漂移率
            index_neutral_vol = np.median(index_vol_list)        # 指数中位数波动率

            scenarios_config.append({
                'name': '情景2（指数中性）',
                'description': f'主要指数120日窗口中位数参数（漂移率{index_neutral_drift*100:+.2f}%、波动率{index_neutral_vol*100:.2f}%）、溢价率-10%（折价）',
                'volatility': index_neutral_vol,
                'drift': index_neutral_drift,
                'premium_rate': -0.10
            })

            # 情景4: 指数悲观（低漂移+高波动+平价）
            index_pess_drift_raw = min(index_return_list)
            index_pess_vol_raw = max(index_vol_list)
            index_pessimistic_drift = index_pess_drift_raw * 0.9  # 取最低漂移率并缩小10%
            index_pessimistic_vol = index_pess_vol_raw * 1.1       # 取最高波动率并放大10%

            scenarios_config.append({
                'name': '情景3（指数悲观）',
                'description': f'主要指数漂移率最低值{index_pess_drift_raw*100:+.2f}%缩小10%至{index_pessimistic_drift*100:+.2f}%、波动率最高值{index_pess_vol_raw*100:.2f}%放大10%至{index_pessimistic_vol*100:.2f}%、溢价率0%（平价）',
                'volatility': index_pessimistic_vol,
                'drift': index_pessimistic_drift,
                'premium_rate': 0.00
            })

            print(f"✅ 已创建3个指数情景（乐观/中性/悲观）")
        else:
            print(f"⚠️ 指数数据不完整，跳过指数情景")
    else:
        print(f"⚠️ 指数数据不可用，跳过指数情景")

    # ===== 情景5-7: 行业情景（基于标的股票所属行业历史数据） =====
    if industry_data_available:
        # 情景5: 行业乐观（高漂移+低波动+深折价）
        industry_opt_drift_raw = industry_return_120d
        industry_opt_vol_raw = industry_vol_120d
        industry_optimistic_drift = industry_return_120d * 1.3  # 行业漂移率放大30%
        industry_optimistic_vol = industry_vol_120d * 0.8       # 行业波动率缩小20%

        scenarios_config.append({
            'name': '情景4（行业乐观）',
            'description': f'行业漂移率{industry_opt_drift_raw*100:+.2f}%放大30%至{industry_optimistic_drift*100:+.2f}%、波动率{industry_opt_vol_raw*100:.2f}%缩小20%至{industry_optimistic_vol*100:.2f}%、溢价率-20%（深折价）',
            'volatility': industry_optimistic_vol,
            'drift': industry_optimistic_drift,
            'premium_rate': -0.20
        })

        # 情景6: 行业中性（实际漂移+实际波动+中等折价）
        industry_neutral_drift = industry_return_120d  # 行业实际漂移率
        industry_neutral_vol = industry_vol_120d        # 行业实际波动率

        scenarios_config.append({
            'name': '情景5（行业中性）',
            'description': f'行业实际参数（漂移率{industry_neutral_drift*100:+.2f}%、波动率{industry_neutral_vol*100:.2f}%）、溢价率-10%（折价）',
            'volatility': industry_neutral_vol,
            'drift': industry_neutral_drift,
            'premium_rate': -0.10
        })

        # 情景7: 行业悲观（低漂移+高波动+平价）
        industry_pess_drift_raw = industry_return_120d
        industry_pess_vol_raw = industry_vol_120d
        industry_pessimistic_drift = industry_return_120d * 0.7  # 行业漂移率缩小30%
        industry_pessimistic_vol = industry_vol_120d * 1.2       # 行业波动率放大20%

        scenarios_config.append({
            'name': '情景6（行业悲观）',
            'description': f'行业漂移率{industry_pess_drift_raw*100:+.2f}%缩小30%至{industry_pessimistic_drift*100:+.2f}%、波动率{industry_pess_vol_raw*100:.2f}%放大20%至{industry_pessimistic_vol*100:.2f}%、溢价率0%（平价）',
            'volatility': industry_pessimistic_vol,
            'drift': industry_pessimistic_drift,
            'premium_rate': 0.00
        })

        print(f"✅ 已创建3个行业情景（乐观/中性/悲观）")
    else:
        print(f"⚠️ 行业数据不可用，跳过行业情景")
    ...
    if 'stock_pe_data' in locals() and 'industry_pe_data' in locals() and stock_pe_data is not None and industry_pe_data is not None:
        try:
            ...
            if 'intrinsic_value' in locals():
                ...
                # 不限制漂移率范围，使用真实DCF估值结果
                # 注：DCF估值可能非常极端，但这是基于估值的真实预期
    ...
    # 筛选6.2节相关情景（当前情景、指数情景1-3、行业情景4-6）
    index_industry_results = [r for r in comprehensive_results
                              ...
                              (r['scenario']['name'] == '当前情景' or
                               ...
                               r['scenario']['name'].startswith('情景3（') or
                               r['scenario']['name'].startswith('情景4（') or
                               r['scenario']['name'].startswith('情景5（') or
                               r['scenario']['name'].startswith('情景6（'))]
    ...
    add_paragraph(document, '通过模拟PE回归到行业5%分位数（极端低估情况），评估最坏情况下估值回调对投资收益的影响。')
    add_paragraph(document, '')
    ...
    # 获取行业PE分位数数据（使用5%分位数，代表极端情况）
    pe_p05 = industry_stats_val['pe']['p05']  # 5%分位数（极端低估水平）

    # 计算PE回归到行业5%分位数的极端情景
    target_price_p05 = eps_rel * pe_p05
    return_p05 = (target_price_p05 - current_price_rel) / current_price_rel * 100
    ...
    scenario_data_stress = [
        ...
        [f'PE→行业5%分位({pe_p05:.2f}倍)', f"{current_metrics_val['pe']:.2f}", f"{pe_p05:.2f}",
         f"{current_price_rel:.2f}", f"{target_price_p05:.2f}", f"{return_p05:+.2f}"],
    ]
    ...
    add_paragraph(document, f'• 行业5%分位PE({pe_p05:.2f}倍)代表行业极端低估水平（95%的公司PE高于此值）')
    add_paragraph(document, f'• 如果PE回归到行业5%分位数，目标价格为{target_price_p05:.2f}元，预期收益{return_p05:+.2f}%')
    ...
    if return_p05 > 0:
        add_paragraph(document, f'✅ 即使在PE回归到行业5%分位数的极端情况下，预期收益仍为正({return_p05:+.2f}%)，估值安全边际较高')
    elif return_p05 > -10:
        add_paragraph(document, f'⚠️ 在PE回归到行业5%分位数的极端情况下，预期收益为负({return_p05:+.2f}%)，存在一定估值回调风险，但风险可控')
    else:
        add_paragraph(document, f'❌ 在PE回归到行业5%分位数的极端情况下，预期收益大幅为负({return_p05:+.2f}%)，估值回调风险较高，需谨慎投资')

    add_paragraph(document, '')

    # 计算定增收益影响
    issue_price = project_params['issue_price']
    return_pe_stress = (target_price_p05 - issue_price) / issue_price * 100
    add_paragraph(document, '对定增收益的影响：')
    add_paragraph(document, f'• 发行价：{issue_price:.2f}元')
    add_paragraph(document, f'• PE回归至5%分位数后的定增收益率：{return_pe_stress:+.2f}%')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '本节定义了6种极端市场情景，模拟不同危机事件对定增项目的影响。')
    ...
    # 将情景定义添加到文档中
    scenario_definition_data = []
    for name, params in stress_scenarios.items():
        scenario_definition_data.append([
            name,
            params['description'],
            f"-{int(params['price_drop']*100)}%",
            f"{params['volatility_spike']:.1f}x"
        ])

    add_table_data(document,
        ['情景名称', '情景描述', '股价跌幅', '波动率倍数'],
        scenario_definition_data)

    add_paragraph(document, '')
    add_paragraph(document, '情景说明：')
    add_paragraph(document, '• **2008金融危机**：全球金融海啸，市场暴跌60%，波动率翻倍')
    add_paragraph(document, '• **2020疫情**：新冠疫情冲击，市场下跌40%，波动率上升50%')
    add_paragraph(document, '• **行业政策收紧**：监管政策变化，行业估值下挫25%')
    add_paragraph(document, '• **个股重大利空**：公司业绩暴雷、财务造假等个股风险')
    add_paragraph(document, '• **流动性危机**：市场流动性枯竭，波动率飙升至2.5倍')
    add_paragraph(document, '• **极端熊市**：系统性风险，市场整体下跌50%')
    add_paragraph(document, '')
    ...
    # 情景1：深度溢价 + 高波动 + 更负的漂移率（三重打击）
    premium_rate_extreme = 0.20  # 20%溢价
    vol_multiplier_extreme = 1.5  # 波动率放大1.5倍

    # 极端漂移率逻辑：确保比正常情况更差
    # 如果正常漂移率已经是负数，极端漂移率应该更负（至少1.5倍）
    # 如果正常漂移率是正数，极端漂移率应该为负（-10%）
    if current_drift_120d < 0:
        drift_extreme = current_drift_120d * 1.5  # 更负（1.5倍）
        print(f"   正常漂移率已是负数({current_drift_120d*100:.2f}%)，极端漂移率调整为{drift_extreme*100:.2f}%（更负）")
    else:
        drift_extreme = -0.10  # 固定-10%
        print(f"   正常漂移率为正({current_drift_120d*100:.2f}%)，极端漂移率设置为{drift_extreme*100:.2f}%")
    ...
    extreme_scenario_data = [
        ...
        ['漂移率', f'{current_drift_120d*100:+.2f}%', f'{drift_rate_extreme*100:+.2f}%', f'×{drift_rate_extreme/current_drift_120d if current_drift_120d != 0 else 1.0:.1f}' if current_drift_120d < 0 else '极端悲观'],
        ['窗口期', '120日', '120日', '保持不变'],
    ...
    extreme_results_headers = ['指标', '极端情景（三重打击）数值', '风险评估']
    extreme_results_data = [
        ['预期年化收益', f'{mean_return_extreme:+.2f}%', '极端亏损' if mean_return_extreme < -30 else '严重亏损' if mean_return_extreme < -10 else '亏损'],
        ['中位数收益', f'{median_return_extreme:+.2f}%', '极端亏损' if median_return_extreme < -30 else '严重亏损' if median_return_extreme < -10 else '亏损'],
        ['盈利概率', f'{profit_prob_extreme:.1f}%', '极高风险' if profit_prob_extreme < 20 else '高风险' if profit_prob_extreme < 40 else '中等风险'],
        ['95% VaR', f'{var_95_extreme:+.2f}%', '95%置信度下最大损失'],
        ['99% VaR', f'{var_99_extreme:+.2f}%', '99%置信度下最大损失'],
        ['最差情况', f'{worst_loss_extreme:+.2f}%', '极端情景下最差损失']
    ]
    ...
    summary_data = [
        ['7.1 PE回归压力测试', f'PE回归至行业5%分位({pe_p05:.2f}倍)', f'{return_pe_stress:+.2f}%',
    ...
    # PE回归结论
    if return_pe_stress > 0:
        conclusions.append(f'✅ 估值回归：即使PE回归至行业5%分位数，定增收益仍为正({return_pe_stress:+.2f}%)，估值安全边际充足')
    else:
        conclusions.append(f'⚠️ 估值回归：PE回归至行业5%分位数将导致亏损({return_pe_stress:+.2f}%)，需警惕估值回调风险')
    ...
    add_paragraph(document, 'VaR含义说明：', bold=True)
    add_paragraph(document, '• 95% VaR：在95%的情况下，最大损失不超过此值')
    add_paragraph(document, '  对应分布左侧5%分位数（即5%最坏情况）')
    add_paragraph(document, '• 99% VaR：在99%的情况下，最大损失不超过此值')
    add_paragraph(document, '  对应分布左侧1%分位数（即1%最坏情况）')
    add_paragraph(document, '• 本报告VaR基于锁定期实际收益率，非年化收益率')
    add_paragraph(document, '• 最大损失不超过100%（本金全部亏完）')
    add_paragraph(document, '')

    add_paragraph(document, 'VaR计算参数说明（多窗口期对比）：', bold=True)

    # 直接使用5.2节已计算的多窗口期蒙特卡洛结果
    # multi_window_mc_results 已包含60日、120日、250日的分位数和CVaR
    # 使用期收益率（非年化）的VaR，更直观反映实际损失
    var_results = {}
    for window_name in ['60日', '120日', '250日']:
        if window_name in multi_window_mc_results:
            mc_result = multi_window_mc_results[window_name]

            # 使用期收益率的VaR（锁定期实际收益率）
            # var_95_period, var_99_period 是基于期收益率计算的
            var_95 = mc_result.get('var_95_period', abs(mc_result['percentile_5']))
            var_99 = mc_result.get('var_99_period', abs(mc_result['percentile_1']))
            cvar_95 = mc_result.get('cvar_95_period', abs(mc_result['cvar_95']))
            cvar_99 = mc_result.get('cvar_99_period', abs(mc_result['cvar_99']))

            # 计算var_90用于后续分析
            var_90 = abs(mc_result.get('percentile_10', 0))

            var_results[window_name] = {
                'vol': mc_result['volatility'],
                'drift': mc_result['drift'],
                'var_90': var_90,
                'var_95': var_95,
                'var_99': var_99,
                'cvar_90': cvar_90,
    ...
    var_params_data.extend([
        ...
        ['名义折价/溢价率', f'{discount_var:+.2f}% {discount_note}', f'{discount_var:+.2f}% {discount_note}', f'{discount_var:+.2f}% {discount_note}']
    ])

    # 计算实际折价率（相对当前价格）
    actual_discount_var = (project_params["issue_price"] - project_params["current_price"]) / project_params["current_price"] * 100
    actual_discount_note = f"（公式：(发行价-当前价)/当前价）"

    var_params_data.extend([
        ['实际折价/溢价率', f'{actual_discount_var:+.2f}% {actual_discount_note}', f'{actual_discount_var:+.2f}% {actual_discount_note}', f'{actual_discount_var:+.2f}% {actual_discount_note}']
    ])
    ...
    # 创建VaR对比表格
    var_comparison_headers = ['窗口期', '95% VaR\n(5%分位数)', '99% VaR\n(1%分位数)', '95% CVaR\n(5%尾部平均)', '99% CVaR\n(1%尾部平均)', '风险等级']
    ...
    cvar_90 = var_results[recommended_window]['cvar_90']
    ...
    # 增量风险分析
    add_paragraph(document, 'VaR指标解读：')
    add_paragraph(document, '• 95% VaR：在95%的情况下，最大损失不超过此值（对应分布左侧5%分位数）')
    add_paragraph(document, '• 99% VaR：在99%的情况下，最大损失不超过此值（对应分布左侧1%分位数）')
    add_paragraph(document, '• CVaR：超过VaR的极端情况下的平均损失（更保守的风险度量）')
    add_paragraph(document, '')
    add_paragraph(document, 'VaR vs CVaR 的区别：')
    add_paragraph(document, '• VaR：回答"有5%的概率损失至少多少"（分位点）')
    add_paragraph(document, '• CVaR：回答"在最差5%的情况下，平均损失多少"（尾部均值）')
    ...
    add_paragraph(document, '本节对前面章节的分析结果进行综合汇总，涵盖个股与行业对比、相对估值、DCF估值、蒙特卡洛模拟、情景分析、压力测试和VaR风险度量等核心内容。')
    add_paragraph(document, '')
    ...
    # 检查是否有历史分位数数据
    has_percentile_data = 'stock_pe_percentile' in locals() or 'stock_pe_data' in locals()

    if has_percentile_data:
        # 从2.3节计算的历史分位数数据
        if 'stock_pe_data' in locals():
            stock_pe_current = float(stock_pe_data.iloc[-1]['pe_ttm'])
            stock_pe_min = float(stock_pe_data['pe_ttm'].min())
            stock_pe_max = float(stock_pe_data['pe_ttm'].max())
            stock_pe_median = float(stock_pe_data['pe_ttm'].median())
        ...
        # 构建历史分位数对比表格
        percentile_comparison_data = [
            ['指标', '当前值', '历史最小', '历史中位数', '历史最大', '当前分位数', '评价'],
        ]

        # PE数据
        if 'stock_pe_data' in locals() and stock_pe_current > 0:
            pe_eval = "历史低位" if stock_pe_percentile <= 25 else "历史中位" if stock_pe_percentile <= 75 else "历史高位"
            percentile_comparison_data.append([
                '市盈率(PE)',
                f'{stock_pe_current:.2f}倍',
                f'{stock_pe_min:.2f}倍',
                f'{stock_pe_median:.2f}倍',
        ...
        # 添加PB数据（如果有）
        stock_pb = market_data.get('pb_ratio', 0)
        stock_pb_data = None  # PB历史数据需要通过专门接口获取，当前未实现
        if stock_pb > 0 and stock_pb_data is not None:
            stock_pb_current = float(stock_pb_data.iloc[-1]['pb'])
            stock_pb_min = float(stock_pb_data['pb'].min())
            stock_pb_max = float(stock_pb_data['pb'].max())
            stock_pb_median = float(stock_pb_data['pb'].median())
            stock_pb_percentile = float((stock_pb_data['pb'] < stock_pb_current).sum() / len(stock_pb_data) * 100)

            pb_eval = "历史低位" if stock_pb_percentile <= 25 else "历史中位" if stock_pb_percentile <= 75 else "历史高位"
            percentile_comparison_data.append([
                '市净率(PB)',
                f'{stock_pb_current:.2f}倍',
                f'{stock_pb_min:.2f}倍',
                f'{stock_pb_median:.2f}倍',
                f'{stock_pb_max:.2f}倍',
                f'{stock_pb_percentile:.1f}%',
                pb_eval
            ])

        # 添加表格
        if len(percentile_comparison_data) > 1:
            add_table_data(document, percentile_comparison_data[0], percentile_comparison_data[1:])

            # 添加解读
            add_paragraph(document, '')
            add_paragraph(document, '📊 历史分位数解读：', bold=True)

            if 'stock_pe_data' in locals() and stock_pe_current > 0:
                add_paragraph(document, f'• PE当前分位数：{stock_pe_percentile:.1f}%')
                if stock_pe_percentile <= 25:
                    add_paragraph(document, f'  ✅ 处于历史低位（前25%），历史上仅{stock_pe_percentile:.1f}%的时间估值更低')
                elif stock_pe_percentile <= 50:
                    add_paragraph(document, f'  ✓ 处于历史中位（前50%），估值相对合理')
                elif stock_pe_percentile <= 75:
                    add_paragraph(document, f'  ⚠️ 处于历史中高位（前75%），估值相对偏高')
                else:
                    add_paragraph(document, f'  ❌ 处于历史高位（前25%），估值偏高，需警惕回调风险')

        else:
            add_paragraph(document, '⚠️ 历史分位数数据不可用')
    else:
        add_paragraph(document, '⚠️ 历史分位数数据不可用，无法进行历史对比分析')
    ...
    add_paragraph(document, '本节基于行业PE分位数计算不同估值情景下的目标价和预期收益率，评估估值回归对投资收益的影响。')
    add_paragraph(document, '')

    # 检查行业PE分位数数据是否可用
    has_industry_percentile = 'industry_stats_val' in locals() and industry_stats_val is not None

    if has_industry_percentile and 'pe' in industry_stats_val:
        # 获取PE分位数数据（使用第二章的键名：q1, median, q3）
        pe_p25 = industry_stats_val['pe'].get('q1', 0)  # 25%分位数
        pe_p50 = industry_stats_val['pe'].get('median', 0)  # 50%分位数（中位数）
        pe_p75 = industry_stats_val['pe'].get('q3', 0)  # 75%分位数

        # 检查数据有效性
        if pe_p25 > 0 and pe_p50 > 0 and pe_p75 > 0:
            # 计算在不同分位数下的目标价
            eps_rel = market_data.get('eps', 0)

            if eps_rel > 0:
                target_price_p25 = eps_rel * pe_p25
                target_price_p50 = eps_rel * pe_p50
                target_price_p75 = eps_rel * pe_p75

                current_price_rel = project_params['current_price']

                relative_valuation_data = [
                    ['低估情景', f'25%分位数: {pe_p25:.2f}倍', f'{target_price_p25:.2f}元', f'{((target_price_p25/current_price_rel - 1)*100):+.2f}%', '安全边际充足' if target_price_p25 > current_price_rel else '安全边际不足'],
                    ['中性情景', f'50%分位数: {pe_p50:.2f}倍', f'{target_price_p50:.2f}元', f'{((target_price_p50/current_price_rel - 1)*100):+.2f}%', '合理估值' if target_price_p50 > current_price_rel * 0.9 else '估值偏高'],
                    ['高估情景', f'75%分位数: {pe_p75:.2f}倍', f'{target_price_p75:.2f}元', f'{((target_price_p75/current_price_rel - 1)*100):+.2f}%', '估值偏高' if target_price_p75 < current_price_rel * 1.1 else '估值合理'],
                ]

                add_table_data(document, ['估值情景', '行业PE分位数', '目标价', '收益率（相对当前价）', '评价'], relative_valuation_data)

                # 添加解读
                add_paragraph(document, '')
                add_paragraph(document, '📊 相对估值解读：', bold=True)
                add_paragraph(document, f'• 当前EPS：{eps_rel:.2f}元/股')
                add_paragraph(document, f'• 当前价格：{current_price_rel:.2f}元/股')
                add_paragraph(document, f'• 当前PE（市盈率）：{current_price_rel/eps_rel:.2f}倍' if eps_rel > 0 else '• 当前PE：数据不可用')

                if target_price_p25 > current_price_rel:
                    add_paragraph(document, f'✅ 25%分位数情景：目标价{target_price_p25:.2f}元 > 当前价，安全边际充足')
                if target_price_p50 > current_price_rel:
                    add_paragraph(document, f'✅ 50%分位数情景：目标价{target_price_p50:.2f}元 > 当前价，估值合理')
                if target_price_p75 < current_price_rel:
                    add_paragraph(document, f'⚠️ 75%分位数情景：目标价{target_price_p75:.2f}元 < 当前价，估值偏高')
            else:
                add_paragraph(document, '⚠️ EPS数据不可用，无法计算目标价')
        else:
            add_paragraph(document, '⚠️ 行业PE分位数数据不完整，无法进行相对估值分析')
            add_paragraph(document, '   需要的数据：pe_p25, pe_p50, pe_p75')
    else:
        add_paragraph(document, '⚠️ 行业PE分位数数据不可用，无法进行相对估值分析')
        add_paragraph(document, '   提示：请确保已获取行业指数数据（运行脚本时需要行业数据）')

    # ==================== 9.1.3 DCF估值分析 ====================
    add_title(document, '9.1.3 DCF估值分析', level=3)

    # DCF评价
    dcf_premium = (intrinsic_value - current_price_rel) / current_price_rel * 100 if intrinsic_value > 0 else float('inf')
    dcf_eval = "显著低估" if dcf_premium > 50 else "低估" if dcf_premium > 20 else "合理估值" if dcf_premium > -20 else "高估" if dcf_premium > -50 else "显著高估"

    dcf_data = [
        ['DCF内在价值', f'{intrinsic_value:.2f}元/股' if intrinsic_value > 0 else '负值（不可投资）', '基于自由现金流折现计算'],
        ['当前价格', f'{current_price_rel:.2f}元/股', '市场最新交易价格'],
        ['估值溢价/折价', f'{dcf_premium:+.2f}%' if intrinsic_value > 0 else 'N/A', '正数表示低估，负数表示高估'],
        ['投资建议', dcf_eval, '基于DCF估值综合判断'],
    ]

    add_table_data(document, ['估值指标', '数值', '说明'], dcf_data)
    add_paragraph(document, '')

    # ==================== 9.1.4 蒙特卡洛模拟结果 ====================
    add_title(document, '9.1.4 蒙特卡洛模拟结果', level=3)

    add_paragraph(document, '本节使用两种方法进行蒙特卡洛模拟：')
    add_paragraph(document, '1. 历史参数模拟：使用历史120日波动率和漂移率')
    add_paragraph(document, '2. 预测参数模拟：使用ARIMA预测漂移率和GARCH预测波动率')
    add_paragraph(document, '')

    # 获取120日窗口的蒙特卡洛结果
    if '120日' in multi_window_mc_results:
        mc_120_result = multi_window_mc_results['120日']
        mc_profit_prob_hist = mc_120_result['profit_prob']
        mc_mean_return_hist = mc_120_result['mean_return']
        mc_median_return_hist = mc_120_result['median_return']
        mc_percentile_5_hist = mc_120_result['percentile_5']
        mc_percentile_1_hist = mc_120_result['percentile_1']

        # 获取120日窗口的参数
        mc_volatility_120d = mc_volatility_120d  # 历史波动率
        mc_drift_120d = mc_drift_120d  # 历史漂移率
        lockup_months = project_params['lockup_period']  # 锁定期

        # 计算溢价率
        premium_rate = (project_params['issue_price'] - project_params['current_price']) / project_params['current_price']

        # ======== 方法1：历史参数模拟 ========
        add_paragraph(document, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
        add_paragraph(document, '方法1：历史参数模拟（基于历史120日数据）', bold=True)
        add_paragraph(document, '')

        mc_eval_hist = "风险较高" if mc_profit_prob_hist < 50 else "风险适中" if mc_profit_prob_hist < 70 else "风险较低"

        # 模拟结果表格
        monte_carlo_results_hist = [
            ['模拟窗口', '120日（半年）', '基于历史120日波动率和收益率'],
            ['模拟次数', '10,000次', '蒙特卡洛模拟路径数'],
            ['盈利概率', f'{mc_profit_prob_hist:.1f}%', '模拟中盈利场景占比'],
            ['预期年化收益', f'{mc_mean_return_hist*100:.2f}%', '平均年化收益率'],
            ['中位数年化收益', f'{mc_median_return_hist*100:.2f}%', '中位数年化收益率（更稳健）'],
            ['5% VaR（最坏5%）', f'{mc_percentile_5_hist*100:.2f}%', '95%置信水平下的最大损失'],
            ['1% VaR（最坏1%）', f'{mc_percentile_1_hist*100:.2f}%', '99%置信水平下的最大损失'],
            ['风险评估', mc_eval_hist, '基于盈利概率综合判断'],
        ]

        # 模拟参数表格
        monte_carlo_params_hist = [
            ['历史漂移率（120日）', f'{mc_drift_120d*100:.2f}%', '历史120日期间的年化收益率'],
            ['历史波动率（120日）', f'{mc_volatility_120d*100:.2f}%', '历史120日期间的年化波动率'],
            ['锁定期', f'{lockup_months}个月', f'定增锁定期（持股时间）'],
            ['发行价', f'{project_params["issue_price"]:.2f}元', '定增发行价格'],
            ['当前价', f'{project_params["current_price"]:.2f}元', '市场最新价格'],
            ['溢价率', f'{premium_rate*100:+.2f}%', f'发行价相对当前价的溢价/折价'],
        ]

        add_table_data(document, ['模拟指标', '数值', '说明'], monte_carlo_results_hist)

        add_paragraph(document, '')
        add_paragraph(document, '📋 模拟参数：', bold=True)
        add_table_data(document, ['参数', '数值', '说明'], monte_carlo_params_hist)

        # ======== 方法2：预测参数模拟 ========
        add_paragraph(document, '')
        add_paragraph(document, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
        add_paragraph(document, '方法2：预测参数模拟（基于ARIMA+GARCH预测）', bold=True)
        add_paragraph(document, '')

        # 使用预测参数（从时间序列预测结果获取）
        try:
            # ARIMA预测漂移率、GARCH预测波动率已在前面章节计算
            log_drift_predicted = arima_result['forecast_drift']
            predicted_vol = garch_result['forecast_volatility']
            ...
            sim_predicted = analyzer.monte_carlo_simulation(
                ...
                time_steps=time_steps,
                volatility=predicted_vol,
                drift=price_drift_predicted,
                seed=42
            )

            # 计算结果
            final_prices_pred = sim_predicted.iloc[:, -1].values
            returns_pred = (final_prices_pred - project_params['issue_price']) / project_params['issue_price']

            # 年化收益率计算：使用固定系数（单利）
            coefficient = 0.5  # 120日=半年
            annualized_returns_pred = returns_pred / coefficient

            # 统计
            mc_profit_prob_pred = (returns_pred > 0).mean() * 100
            mc_mean_return_pred = annualized_returns_pred.mean()
            mc_median_return_pred = np.median(annualized_returns_pred)
            mc_percentile_5_pred = np.percentile(annualized_returns_pred, 5)
            mc_percentile_1_pred = np.percentile(annualized_returns_pred, 1)

            mc_eval_pred = "风险较高" if mc_profit_prob_pred < 50 else "风险适中" if mc_profit_prob_pred < 70 else "风险较低"

            # 模拟结果表格
            monte_carlo_results_pred = [
                ['模拟窗口', '120日（半年）', '基于ARIMA和GARCH预测参数'],
                ['模拟次数', '10,000次', '蒙特卡洛模拟路径数'],
                ['盈利概率', f'{mc_profit_prob_pred:.1f}%', '模拟中盈利场景占比'],
                ['预期年化收益', f'{mc_mean_return_pred*100:.2f}%', '平均年化收益率'],
                ['中位数年化收益', f'{mc_median_return_pred*100:.2f}%', '中位数年化收益率（更稳健）'],
                ['5% VaR（最坏5%）', f'{mc_percentile_5_pred*100:.2f}%', '95%置信水平下的最大损失'],
                ['1% VaR（最坏1%）', f'{mc_percentile_1_pred*100:.2f}%', '99%置信水平下的最大损失'],
                ['风险评估', mc_eval_pred, '基于盈利概率综合判断'],
            ]

            # 模拟参数表格
            monte_carlo_params_pred = [
                ['ARIMA对数漂移率', f'{log_drift_predicted*100:.2f}%', 'ARIMA模型预测的对数漂移率'],
                ['GARCH波动率', f'{predicted_vol*100:.2f}%', 'GARCH模型预测的波动率'],
                ['GBM价格漂移率', f'{price_drift_predicted*100:.2f}%', f'对数漂移率+0.5×σ²={log_drift_predicted*100:.2f}%+{0.5*predicted_vol**2*100:.2f}%'],
                ['锁定期', f'{lockup_months}个月', f'定增锁定期（持股时间）'],
                ['发行价', f'{project_params["issue_price"]:.2f}元', '定增发行价格'],
                ['当前价', f'{project_params["current_price"]:.2f}元', '市场最新价格'],
                ['溢价率', f'{premium_rate*100:+.2f}%', f'发行价相对当前价的溢价/折价'],
            ...
            add_table_data(document, ['模拟指标', '数值', '说明'], monte_carlo_results_pred)

            add_paragraph(document, '')
            add_paragraph(document, '📋 模拟参数：', bold=True)
            add_table_data(document, ['参数', '数值', '说明'], monte_carlo_params_pred)

            # ======== 两种方法对比 ========
            add_paragraph(document, '')
            add_paragraph(document, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
            add_paragraph(document, '两种方法对比分析', bold=True)
            add_paragraph(document, '')

            # 对比数据
            comparison_data = [
                ['对比项目', '历史参数模拟', '预测参数模拟', '差异'],
                ['漂移率', f'{mc_drift_120d*100:.2f}%', f'{log_drift_predicted*100:.2f}%', f'{(log_drift_predicted-mc_drift_120d)*100:+.2f}个百分点'],
                ['波动率', f'{mc_volatility_120d*100:.2f}%', f'{predicted_vol*100:.2f}%', f'{(predicted_vol-mc_volatility_120d)*100:+.2f}个百分点'],
                ['盈利概率', f'{mc_profit_prob_hist:.1f}%', f'{mc_profit_prob_pred:.1f}%', f'{mc_profit_prob_pred-mc_profit_prob_hist:+.1f}个百分点'],
                ['预期年化收益', f'{mc_mean_return_hist*100:.2f}%', f'{mc_mean_return_pred*100:.2f}%', f'{(mc_mean_return_pred-mc_mean_return_hist)*100:+.2f}个百分点'],
                ['中位数年化收益', f'{mc_median_return_hist*100:.2f}%', f'{mc_median_return_pred*100:.2f}%', f'{(mc_median_return_pred-mc_median_return_hist)*100:+.2f}个百分点'],
                ['95% VaR', f'{abs(mc_percentile_5_hist)*100:.2f}%', f'{abs(mc_percentile_5_pred)*100:.2f}%', f'{(abs(mc_percentile_5_pred)-abs(mc_percentile_5_hist))*100:+.2f}个百分点'],
            ]

            # 分离表头和数据
            comparison_headers = comparison_data[0]
            comparison_table_data = comparison_data[1:]
            add_table_data(document, comparison_headers, comparison_table_data)
            ...
            # 漂移率对比
            if abs(log_drift_predicted - mc_drift_120d) > 0.05:  # 差异超过5个百分点
                if log_drift_predicted > mc_drift_120d:
                    add_paragraph(document, f'• 预测漂移率（{log_drift_predicted*100:.2f}%）显著高于历史漂移率（{mc_drift_120d*100:.2f}%），ARIMA模型预测未来收益趋势向好')
                else:
                    add_paragraph(document, f'• 预测漂移率（{log_drift_predicted*100:.2f}%）显著低于历史漂移率（{mc_drift_120d*100:.2f}%），ARIMA模型预测未来收益趋势不容乐观')
            else:
                add_paragraph(document, f'• 预测漂移率（{log_drift_predicted*100:.2f}%）与历史漂移率（{mc_drift_120d*100:.2f}%）接近，模型预测延续历史趋势')

            # 波动率对比
            if abs(predicted_vol - mc_volatility_120d) > 0.05:  # 差异超过5个百分点
                if predicted_vol > mc_volatility_120d:
                    add_paragraph(document, f'• 预测波动率（{predicted_vol*100:.2f}%）高于历史波动率（{mc_volatility_120d*100:.2f}%），GARCH模型预测未来波动加剧')
                else:
                    add_paragraph(document, f'• 预测波动率（{predicted_vol*100:.2f}%）低于历史波动率（{mc_volatility_120d*100:.2f}%），GARCH模型预测未来波动趋于平稳')
            else:
                add_paragraph(document, f'• 预测波动率（{predicted_vol*100:.2f}%）与历史波动率（{mc_volatility_120d*100:.2f}%）接近，模型预测延续历史波动特征')

            # 盈利概率对比
            if abs(mc_profit_prob_pred - mc_profit_prob_hist) > 10:  # 差异超过10个百分点
                add_paragraph(document, f'• 预测参数下盈利概率（{mc_profit_prob_pred:.1f}%）{"显著高于" if mc_profit_prob_pred > mc_profit_prob_hist else "显著低于"}历史参数下盈利概率（{mc_profit_prob_hist:.1f}%），预测模型{"看好" if mc_profit_prob_pred > mc_profit_prob_hist else "不看好"}该项目前景')
            else:
                add_paragraph(document, f'• 两种方法下盈利概率接近（历史{mc_profit_prob_hist:.1f}% vs 预测{mc_profit_prob_pred:.1f}%），结果相互验证')

            # 综合建议
            add_paragraph(document, '')
            add_paragraph(document, '📊 综合建议：', bold=True)

            # 保守建议（取较低盈利概率）
            conservative_prob = min(mc_profit_prob_hist, mc_profit_prob_pred)
            if conservative_prob < 30:
                add_paragraph(document, f'• ⚠️ 两种方法均显示盈利概率较低（历史{mc_profit_prob_hist:.1f}%，预测{mc_profit_prob_pred:.1f}%），建议谨慎投资或提高折价要求')
            elif conservative_prob > 70:
                add_paragraph(document, f'• ✅ 两种方法均显示盈利概率较高（历史{mc_profit_prob_hist:.1f}%，预测{mc_profit_prob_pred:.1f}%），项目具有较好投资价值')
            else:
                add_paragraph(document, f'• 📊 两种方法显示盈利概率中等（历史{mc_profit_prob_hist:.1f}%，预测{mc_profit_prob_pred:.1f}%），建议结合其他因素综合评估')

        except Exception as e:
            print(f"⚠️ 预测参数模拟失败: {e}")
            add_paragraph(document, '⚠️ 预测参数模拟计算失败，请检查ARIMA和GARCH模型是否拟合成功')
    ...
    add_paragraph(document, '本节汇总不同市场情景下的蒙特卡洛模拟结果，展示中等情景区间（不含极端乐观/悲观情景）。')
    add_paragraph(document, '')

    # 提取中等情景结果（去掉首尾极端情景）
    current_scenario_result = None
    index_neutral_result = None
    industry_neutral_result = None
    index_pessimistic_result = None
    industry_pessimistic_result = None

    for result in comprehensive_results:
        if 'scenario' in result:
            scenario_name = result['scenario']['name']
            if scenario_name == '当前情景':
                current_scenario_result = result
            elif '指数中性' in scenario_name:
                index_neutral_result = result
            elif '行业中性' in scenario_name:
                industry_neutral_result = result
            elif '指数悲观' in scenario_name:
                index_pessimistic_result = result
            elif '行业悲观' in scenario_name:
                industry_pessimistic_result = result

    scenario_data = []

    # 基准情景
    if current_scenario_result:
        scenario_data.append([
            '当前情景（基准）',
            f'{current_scenario_result["profit_prob"]:.1f}%',
            f'{current_scenario_result["median_return"]*100:.2f}%',
            '基于当前市场参数'
        ])

    # 中等情景区间
    if index_neutral_result:
        scenario_data.append([
            '指数中性情景',
            f'{index_neutral_result["profit_prob"]:.1f}%',
            f'{index_neutral_result["median_return"]*100:.2f}%',
            '基于主要指数历史数据'
        ])

    if industry_neutral_result:
        scenario_data.append([
            '行业中性情景',
            f'{industry_neutral_result["profit_prob"]:.1f}%',
            f'{industry_neutral_result["median_return"]*100:.2f}%',
            '基于所属行业历史数据'
        ])

    if index_pessimistic_result:
        scenario_data.append([
            '指数悲观情景',
            f'{index_pessimistic_result["profit_prob"]:.1f}%',
            f'{index_pessimistic_result["median_return"]*100:.2f}%',
            '指数表现较差情景'
        ])

    if industry_pessimistic_result:
        scenario_data.append([
            '行业悲观情景',
            f'{industry_pessimistic_result["profit_prob"]:.1f}%',
            f'{industry_pessimistic_result["median_return"]*100:.2f}%',
            '行业表现较差情景'
        ])

    if scenario_data:
        add_table_data(document, ['情景类型', '盈利概率', '中位数收益', '评价'], scenario_data)

        add_paragraph(document, '')
        add_paragraph(document, '情景分析说明：', bold=True)
        add_paragraph(document, '• 本表展示中等情景区间，不包括极端乐观/悲观情景')
        add_paragraph(document, '• 当前情景：基于当前市场参数的基准情景')
        add_paragraph(document, '• 指数情景：基于沪深300、中证500等主要指数历史数据')
        add_paragraph(document, '• 行业情景：基于所属行业的历史数据')
        add_paragraph(document, '• 完整情景分析请参见第6章"情景分析与敏感性分析"')
    else:
        add_paragraph(document, '⚠️ 未找到情景分析结果')
    ...
    for result in comprehensive_results:
        if 'scenario' in result:
            scenario_name = result['scenario']['name']
            if '极端' in scenario_name:
                extreme_stress_result = result
            elif '悲观' in scenario_name or '最坏' in scenario_name:
                pessimistic_result = result

    # 使用极端情景或悲观情景
    stress_scenario = extreme_stress_result or pessimistic_result

    if stress_scenario:
        scenario_name_display = stress_scenario['scenario']['name']
        stress_data = [
            [scenario_name_display, f'{stress_scenario["profit_prob"]:.1f}%', f'{stress_scenario["median_return"]*100:.2f}%', '压力测试情景'],
        ]

        # 与当前情景对比
        if current_scenario_result:
            profit_prob_diff = stress_scenario["profit_prob"] - current_scenario_result["profit_prob"]
            median_return_diff = stress_scenario["median_return"] - current_scenario_result["median_return"]

            stress_data.append([
                '与当前情景差异',
                f'{profit_prob_diff:+.1f}个百分点',
                f'{median_return_diff*100:+.2f}%',
                '压力情景下的额外风险'
            ])

        add_table_data(document, ['测试类型', '盈利概率', '中位数收益', '说明'], stress_data)

        add_paragraph(document, '')
        add_paragraph(document, '压力测试说明：', bold=True)
        add_paragraph(document, f'• 压力测试情景：{scenario_name_display}')
        add_paragraph(document, '• 该情景模拟极端市场条件下的投资表现')
        add_paragraph(document, '• 与当前情景对比可评估项目的抗风险能力')
    else:
        add_paragraph(document, '⚠️ 未找到压力测试情景结果')
        add_paragraph(document, '   提示：请确保情景分析中包含极端或悲观情景')
    ...
    # 重新计算VaR基于简单收益率（锁定期收益率，不年化）
    # 使用120日参数运行蒙特卡洛模拟
    try:
        mc_vol_120d = market_data.get('volatility_120d', market_data.get('volatility', 0.30))
        mc_drift_120d = market_data.get('annual_return_120d', market_data.get('drift', 0.08))
        time_steps = 120  # 120日
        n_simulations = 10000

        sim_var = analyzer.monte_carlo_simulation(
            n_simulations=n_simulations,
            time_steps=time_steps,
            volatility=mc_vol_120d,
            drift=mc_drift_120d,
            seed=42
        )

        # 计算锁定期收益率（简单收益率，不年化）
        final_prices = sim_var.iloc[:, -1].values
        simple_returns = (final_prices - project_params['issue_price']) / project_params['issue_price']

        # 计算VaR百分位数（基于简单收益率）
        var_90_simple = abs(np.percentile(simple_returns, 10))  # 90%置信VaR
        var_95_simple = abs(np.percentile(simple_returns, 5))   # 95%置信VaR
        var_99_simple = abs(np.percentile(simple_returns, 1))   # 99%置信VaR

        # 计算CVaR（尾部平均损失）
        cvar_90_simple = abs(simple_returns[simple_returns <= -var_90_simple].mean())
        cvar_95_simple = abs(simple_returns[simple_returns <= -var_95_simple].mean())
        cvar_99_simple = abs(simple_returns[simple_returns <= -var_99_simple].mean())

        # 限制最大损失为100%（股票价格不能为负）
        var_90_simple = min(var_90_simple, 1.0)
        var_95_simple = min(var_95_simple, 1.0)
        var_99_simple = min(var_99_simple, 1.0)
        cvar_90_simple = min(cvar_90_simple, 1.0)
        cvar_95_simple = min(cvar_95_simple, 1.0)
        cvar_99_simple = min(cvar_99_simple, 1.0)

        var_risk_level = "低风险" if var_95_simple < 0.15 else "中等风险" if var_95_simple < 0.30 else "高风险"

        var_summary_data = [
            ['90%置信', f'{var_90_simple*100:.2f}%', f'{cvar_90_simple*100:.2f}%', '10%概率损失超过此值'],
        ...
        add_table_data(document, ['置信水平', 'VaR（最大损失）', 'CVaR（尾部平均损失）', '说明'], var_summary_data)

        add_paragraph(document, '')
        add_paragraph(document, 'VaR说明：', bold=True)
        add_paragraph(document, '• VaR基于锁定期（120日）的简单收益率计算，未进行年化')
        add_paragraph(document, '• 最大损失上限为100%（股票价格最低为0，无杠杆情况下不会超过100%损失）')
        add_paragraph(document, '• 95% VaR表示在95%置信水平下，锁定期末的最大可能损失')
        add_paragraph(document, f'• 当前95% VaR为{var_95_simple*100:.2f}%，表示在5%的最坏情况下，锁定期末可能损失{var_95_simple*100:.2f}%')

    except Exception as e:
        print(f"⚠️ VaR计算失败: {e}")
        add_paragraph(document, '⚠️ VaR计算失败，请检查参数设置')
    ...
    if '120日' in multi_window_mc_results:
        mc_120_result = multi_window_mc_results['120日']
        profit_prob_display = mc_120_result['profit_prob']
        mean_return_display = mc_120_result['mean_return'] * 100
    else:
        profit_prob_display = 0
        mean_return_display = 0

    # 计算评估值
    risk_level_eval = "低风险" if total_score >= 80 else "中等风险" if total_score >= 60 else "高风险"
    profit_level_eval = "较高" if profit_prob_display >= 70 else "中等" if profit_prob_display >= 50 else "较低"
    issue_type_eval = "折价发行" if discount_premium < 0 else "平价发行" if discount_premium < 5 else "溢价发行"

    # 9.1.1 历史分位数评估
    historical_eval = "合理" if total_score >= 60 else "偏高"
    if current_price_rel > 0 and intrinsic_value > 0:
        historical_valuation = (intrinsic_value - current_price_rel) / current_price_rel * 100
        historical_eval = "低估" if historical_valuation > 20 else "高估" if historical_valuation < -20 else "合理"
    historical_percentile = "历史中位"  # 需要从实际数据计算

    # 9.1.2 相对估值评估
    relative_eval = "合理估值"
    if eps_rel > 0 and current_price_rel > 0:
        current_pe = current_price_rel / eps_rel
        relative_eval = "低估" if current_pe < 20 else "高估" if current_pe > 30 else "合理"

    # 9.1.3 DCF估值评估
    if intrinsic_value > 0 and current_price_rel > 0:
        dcf_premium = (intrinsic_value - current_price_rel) / current_price_rel * 100
        dcf_eval = "显著低估" if dcf_premium > 50 else "低估" if dcf_premium > 20 else "合理估值" if dcf_premium > -20 else "高估" if dcf_premium > -50 else "显著高估"
    else:
        dcf_eval = "数据不可用"

    # 9.1.4 蒙特卡洛评估
    mc_eval = "风险较高" if profit_prob_display < 50 else "风险适中" if profit_prob_display < 70 else "风险较低"

    # 9.1.5 情景分析评估
    scenario_eval = "情景敏感" if profit_prob_display < 30 else "情景稳健"

    # 9.1.6 压力测试评估
    stress_eval = "抗压能力弱" if profit_prob_display < 30 else "抗压能力强"

    # 9.1.7 VaR评估（使用简单收益率VaR）
    var_95_simple = abs(mc_120_result.get('percentile_5', 0)) if '120日' in multi_window_mc_results else 0
    var_eval = "风险可控" if var_95_simple < 0.30 else "风险较高" if var_95_simple < 0.50 else "风险极高"

    # 汇总表格
    summary_data = [
        # 综合评分
        ['综合风险评分', f'{total_score}/100', risk_level_eval, '基于多维度量化评估（9.1.1至9.1.7）'],

        # 9.1.1 历史分位数
        ['历史分位数位置', historical_percentile, historical_eval, '当前股价相对历史水平（9.1.1）'],

        # 9.1.2 相对估值
        ['相对估值', f'{current_price_rel/eps_rel:.1f}倍' if eps_rel > 0 else 'N/A', relative_eval, '当前PE相对行业水平（9.1.2）'],

        # 9.1.3 DCF估值
        ['DCF内在价值', f'{intrinsic_value:.2f}元/股' if intrinsic_value > 0 else 'N/A', dcf_eval, '基于自由现金流折现（9.1.3）'],

        # 9.1.4 蒙特卡洛
        ['蒙特卡洛盈利概率', f'{profit_prob_display:.1f}%', profit_level_eval, '历史+预测参数模拟（9.1.4）'],
        ['预期年化收益率', f'{mean_return_display:.2f}%', '较高' if mean_return_display > 10 else '中等' if mean_return_display > 0 else '负收益', '模拟平均年化收益率（9.1.4）'],

        # 9.1.5 情景分析
        ['情景分析稳健性', f'{profit_prob_display:.1f}%', scenario_eval, '多情景下的盈利概率（9.1.5）'],

        # 9.1.6 压力测试
        ['压力测试抗压性', f'{profit_prob_display:.1f}%', stress_eval, '极端情景下的表现（9.1.6）'],

        # 9.1.7 VaR
        ['95% VaR（锁定期）', f'{var_95_simple*100:.2f}%', var_eval, '锁定期最大损失（9.1.7）'],

        # 发行参数
        ['发行类型', f'{issue_type}', issue_type_eval, '相对MA20的折价/溢价情况'],
        ['溢价率（相对MA20）', f'{discount_premium:+.2f}%', '折价' if discount_premium < 0 else '溢价', '发行价相对MA20的溢价率'],

        # 市场参数
        ['波动率(120日)', f'{mc_volatility_120d*100:.2f}%', '低波动' if mc_volatility_120d < 0.3 else '高波动' if mc_volatility_120d > 0.5 else '中等波动', '年化波动率'],
        ['年化漂移率(120日)', f'{mc_drift_120d*100:+.2f}%', '正收益' if mc_drift_120d > 0 else '负收益', '历史年化收益率'],

        # 总结（人工填写）
        ['', '', '', ''],
        ['总结', '____________________', '____________________', '请根据以上分析填写综合投资建议（人工填写）'],
    ]

    add_table_data(document, ['评价类目', '分值/指标', '评估', '说明'], summary_data)

    # 添加评估说明
    add_paragraph(document, '')
    add_paragraph(document, '综合评估说明：', bold=True)
    add_paragraph(document, f'• 本表汇总了9.1.1至9.1.7节的核心分析结果')
    add_paragraph(document, f'• 综合风险评分：{total_score}/100（{risk_level_eval}）')
    add_paragraph(document, f'• 评估维度包括：历史分位数、相对估值、DCF估值、蒙特卡洛模拟、情景分析、压力测试、VaR风险度量')
    add_paragraph(document, f'• "总结"行请人工填写，建议结合以上分析给出明确的投资建议（推荐/谨慎/不推荐）')
    ...
    add_paragraph(document, '本节首先评估宏观环境，然后基于9.1.5情景分析汇总结果，按照严格筛选条件推荐报价方案。')
    add_paragraph(document, '')
    ...
    current_percentile = current_turnover_percentile  # 保存用于显示
    ...
    add_paragraph(document, '  - 市场活跃度：不活跃(20分)、较不活跃(40分)、平稳(60分)、较活跃(80分)、活跃(100分)')
    add_paragraph(document, '')
    ...
    # 生成并添加换手率曲线图
    if 'market_turnover' in market_data and market_data['market_turnover'] is not None:
        turnover_chart_path = os.path.join(IMAGES_DIR, 'market_turnover_history.png')
    ...
    add_paragraph(document, '基于上述宏观环境评估和9.1.5情景分析汇总结果，按照严格筛选条件推荐报价方案。')
    add_paragraph(document, '')
    ...
    for result in comprehensive_results:
        if 'scenario' in result and 'median_return' in result and 'profit_prob' in result:
            scenario_name = result['scenario']['name']
            median_return = result['median_return']  # 已经是百分比，不需要转换
            profit_prob = result['profit_prob']  # 已经是百分比

            # 获取情景的溢价率（从scenario对象中获取）
            scenario_premium_rate = result['scenario']['premium_rate']  # 小数形式，如-0.05表示-5%
            scenario_premium_pct = scenario_premium_rate * 100  # 转为百分比，如-5.0表示-5%

            # 获取情景的VaR（使用var_95作为95% VaR）
            var_95 = abs(result.get('var_95', median_return))  # 已经是百分比

            scenario_options.append({
                'name': scenario_name,
                'median_return': median_return,
                'profit_prob': profit_prob,
                'premium_rate': scenario_premium_pct,  # 使用百分比形式
                'var_95': var_95,
                'issue_price': current_price_eval * (1 + scenario_premium_rate)  # 使用小数形式计算
            })
    ...
    if '120日' in multi_window_mc_results:
        ...
        scenario_options.append({
            'name': '历史参数模拟（120日）',
            'median_return': mc_120_result['median_return'],  # 已经是百分比
            'profit_prob': mc_120_result['profit_prob'],  # 已经是百分比
            'premium_rate': discount_premium * 100,  # 当前溢价率（转为百分比）
            'var_95': abs(mc_120_result.get('percentile_5', mc_120_result['median_return'])),  # 已经是百分比
            'issue_price': issue_price_eval
        })

    # ==================== 第一部分：反向推算的最高报价（始终显示） ====================
    add_paragraph(document, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    add_paragraph(document, '一、反向推算最高报价', bold=True)
    add_paragraph(document, '')
    ...
    # 方法2：考虑波动率的反推（使用安全边际）
    # 在目标收益率基础上增加波动率折扣：安全收益率 = 目标收益率 + 0.5 * 波动率²
    ...
    add_paragraph(document, '')
    add_paragraph(document, '1. 设定目标收益率：8%（年化）')
    add_paragraph(document, '2. 使用120日历史波动率：{:.2f}%'.format(volatility_120d * 100))
    add_paragraph(document, '3. 锁定期：{}个月（{:.2f}年）'.format(project_params['lockup_period'], lockup_years))
    add_paragraph(document, '4. 参考价格：当前价格{:.2f}元，MA20价格{:.2f}元'.format(current_price_eval, ma20_price))
    add_paragraph(document, '')
    ...
    add_paragraph(document, '  最高发行价 = {:.2f} ÷ (1 + 8% × {:.2f}) = {:.2f}元'.format(
        current_price_eval, lockup_years, max_issue_price_simple))
    add_paragraph(document, '')

    add_paragraph(document, '• 方法2（更保守）：考虑波动率安全边际')
    add_paragraph(document, '  安全边际 = 0.5 × 波动率² = 0.5 × ({:.2f}%)² = {:.2f}%'.format(
        volatility_120d * 100, safety_margin * 100))
    add_paragraph(document, '  调整后收益率 = 目标收益率 + 安全边际 = 8% + {:.2f}% = {:.2f}%'.format(
        safety_margin * 100, adjusted_return * 100))
    add_paragraph(document, '  最高发行价 = {:.2f} ÷ (1 + {:.2f}% × {:.2f}) = {:.2f}元'.format(
        current_price_eval, adjusted_return * 100, lockup_years, max_issue_price_adjusted))
    add_paragraph(document, '')
    ...
    pricing_options = [
        ...
        [
            '方法1：简单反推',
            '{:.2f}元'.format(max_issue_price_simple),
            '{:+.2f}%'.format(premium_to_current_simple),
            '{:+.2f}%'.format(premium_to_ma20_simple),
            '不考虑波动率，相对激进'
        ],
        [
            '方法2：考虑波动率安全边际',
            '{:.2f}元'.format(max_issue_price_adjusted),
            '{:+.2f}%'.format(premium_to_current_adjusted),
            '{:+.2f}%'.format(premium_to_ma20_adjusted),
            '更保守，推荐使用'
        ],
    ]

    # 分离表头和数据
    pricing_headers = pricing_options[0]
    pricing_table_data = pricing_options[1:]
    add_table_data(document, pricing_headers, pricing_table_data)
    ...
    add_paragraph(document, '• 建议最高报价：不高于{:.2f}元/股'.format(recommended_price))
    add_paragraph(document, '• 建议最高溢价率（相对MA20）：{:+.2f}%'.format(recommended_premium))
    ...
    if recommended_premium <= -5:
        add_paragraph(document, '• ✅ 建议报价 - 溢价率{:+.2f}%提供{:.2f}%的安全边际，符合保守投资原则'.format(
            recommended_premium, abs(recommended_premium)))
    elif recommended_premium <= 0:
        add_paragraph(document, '• ⚠️ 谨慎报价 - 溢价率{:+.2f}%安全边际有限，建议折价至少5%'.format(recommended_premium))
    else:
        add_paragraph(document, '• ❌ 不建议报价 - 溢价率{:+.2f}%为溢价发行，缺乏安全边际'.format(recommended_premium))
        add_paragraph(document, '  建议：等待市场时机或选择其他投资机会')

    add_paragraph(document, '')
    add_paragraph(document, '⚠️ 注意事项：')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    add_paragraph(document, '二、基于9.1.5情景的筛选结果', bold=True)
    add_paragraph(document, '')
    ...
    if qualified_scenarios:
        ...
        add_paragraph(document, '⚠️ 9.1.5情景分析中未找到完全符合所有4个筛选条件的方案')
        add_paragraph(document, '• 建议参考上述反向推算的最高报价')
        add_paragraph(document, '• 或参考下文基于585种细化情景的筛选结果')

    # ==================== 第三部分：基于585种细化情景的筛选 ====================
    add_paragraph(document, '')
    add_paragraph(document, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    add_paragraph(document, '三、基于585种细化情景的筛选', bold=True)
    add_paragraph(document, '')
    add_paragraph(document, '本节从585种细化情景组合中筛选符合条件的方案，提供更全面的报价参考。')
    add_paragraph(document, '')

    # 从all_scenarios_for_appendix中筛选符合条件的方案
    qualified_585_scenarios = []
    for scenario in all_scenarios_for_appendix:
        # 获取实际溢价率（百分比形式）
        premium_rate_pct = scenario['actual_premium_rate'] * 100
        median_return_pct = scenario['median_return'] * 100
        var_95_pct = abs(scenario['var_5']) * 100  # var_5是5%分位数，对应VaR 95%
        profit_prob_pct = scenario['profit_prob']
        ...
        if condition_1 and condition_2 and condition_3 and condition_4:
            qualified_585_scenarios.append({
                ...
                'issue_price': scenario['issue_price'],
    ...
    if qualified_585_scenarios:
        add_paragraph(document, f'✅ 从585种情景中找到{len(qualified_585_scenarios)}个符合条件的方案')
        add_paragraph(document, '')
        ...
        recommended_585_data = [
            ['建议报价', f"{best_585_scenario['issue_price']:.2f}元/股", f"溢价率{best_585_scenario['premium_rate']:+.2f}%"],
            ['预期中位数收益率', f"{best_585_scenario['median_return']:.2f}%", '年化收益率'],
            ['盈利概率', f"{best_585_scenario['profit_prob']:.1f}%", '蒙特卡洛模拟盈利概率'],
            ['95% VaR', f"{best_585_scenario['var_95']:.2f}%", '锁定期最大损失'],
            ['', '', ''],
            ['市场参数', f"漂移率{best_585_scenario['drift']*100:+.0f}%, 波动率{best_585_scenario['volatility']*100:.0f}%", '基于历史数据'],
            ['✅ 溢价率', f"{best_585_scenario['premium_rate']:+.2f}%", '≤ -5% ✓'],
            ['✅ 中位数收益率', f"{best_585_scenario['median_return']:.2f}%", '> 8% ✓'],
            ['✅ 95% VaR', f"{best_585_scenario['var_95']:.2f}%", '< 60% ✓'],
        ...
        add_paragraph(document, '')
        add_paragraph(document, '推荐理由：', bold=True)
        add_paragraph(document, f'• 该方案从585种情景中筛选，满足所有4个筛选条件')
        add_paragraph(document, f'• 中位数收益率{best_585_scenario["median_return"]:.2f}%最接近8%目标')
        add_paragraph(document, f'• 溢价率{best_585_scenario["premium_rate"]:+.2f}%提供{abs(best_585_scenario["premium_rate"]):.2f}%的安全边际')
        add_paragraph(document, f'• 盈利概率{best_585_scenario["profit_prob"]:.1f}%，超过半数场景盈利')
        add_paragraph(document, f'• 95% VaR为{best_585_scenario["var_95"]:.2f}%，最大损失相对可控')
        ...
        if best_585_scenario['median_return'] >= 15:
            add_paragraph(document, f'• ✅ 强烈推荐 - 该方案预期收益率{best_585_scenario["median_return"]:.2f}%较高，风险可控')
        elif best_585_scenario['median_return'] >= 10:
            add_paragraph(document, f'• ✅ 推荐 - 该方案预期收益率{best_585_scenario["median_return"]:.2f}%达标，风险收益平衡')
        else:
            add_paragraph(document, f'• ⚠️ 谨慎推荐 - 该方案预期收益率{best_585_scenario["median_return"]:.2f}%勉强达标，需密切关注风险')

    else:
        add_paragraph(document, '⚠️ 585种细化情景中未找到完全符合所有4个筛选条件的方案')
        add_paragraph(document, '• 建议参考上述反向推算的最高报价')
        add_paragraph(document, '• 可适当放宽筛选条件（如降低收益率要求或提高可接受VaR）')
    ...
    # 确保变量已定义
    issue_price_eval = project_params.get('issue_price', issue_price_eval if 'issue_price_eval' in locals() else project_params['issue_price'])
    current_price_eval = project_params.get('current_price', current_price_eval if 'current_price_eval' in locals() else project_params['current_price'])
    ma20_mc = market_data.get('ma_20', 0)
    discount_premium = (issue_price_eval - ma20_mc) / ma20_mc * 100 if ma20_mc > 0 else 0
    ...
    # 判断当前发行价是否合理
    # 假设锁定期末价格回到当前价，计算20%年化收益对应的最高发行价
    expected_price = current_price_eval  # 期望锁定期末价格回到当前价
    ...
    # 从var_results中获取120日窗口的var_95，这是基于期收益率的
    ...
    # 6. 发行定价风险
    add_paragraph(document, '6. 发行定价风险')
    if current_premium < 0:
    ...
    add_paragraph(document, '')
    add_paragraph(document, '说明：本表按波动率分块展示，便于根据实际市场波动率选择参考情景。在每个波动率区间内，按中位数收益率从高到低倒序排列。建议参考创业板180日波动率（约36%）确定目标波动率区间。')
    add_paragraph(document, '')
    ...
    for vol_name, vol_min, vol_max in vol_ranges:
        ...
        # 在区间内按中位数收益率倒序排列（从高到低）
        scenarios_sorted = sorted(scenarios_in_range,
                                 key=lambda x: -x['median_return'])
        ...
        appendix_headers = ['排名', '漂移率', '波动率', '溢价率(名义)', '溢价率(实际)', '发行价(元)', '预期年化收益', '中位数收益', '盈利概率', '5% VaR', '95% VaR']
    ...
    add_paragraph(document, '  - 每个区块内按中位数收益率从高到低倒序排列')
    add_paragraph(document, '• 漂移率：年化收益率，反映股价预期趋势（正值=上涨趋势，负值=下跌趋势）')
    add_paragraph(document, '• 波动率：年化波动率，反映股价不确定性')
    add_paragraph(document, '• 溢价率（名义）：发行价相对MA20的溢价（负值=折价，正值=溢价）')
    add_paragraph(document, '• 溢价率（实际）：发行价相对当前价的溢价（负值=折价，正值=溢价），公式：(发行价-当前价)/当前价')
