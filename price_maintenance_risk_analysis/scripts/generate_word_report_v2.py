...
def generate_tornado_chart(issue_price, current_price, lockup_period, volatility, drift, save_path):
    ...
    param_definitions = [
        ...
        },
        {
            ...
            'name_cn': '波动率',
            'changes': [-0.05, 0.05],  # ±5%
            'change_type': 'absolute_percent',
            'apply_change': lambda base_val, change: base_val + change
        },
        {
            'name': 'drift',
            'name_cn': '漂移率',
    ...
    for param_def in param_definitions:
        ...
        for change in param_def['changes']:
            ...
            if param_def['change_type'] == 'percent':
                ...
                change_unit = f'每1%变化'
            elif param_def['change_type'] == 'absolute_percent':
                # 绝对百分比变化：每1%的影响
                prob_sensitivity = prob_impact / abs(change * 100)
                return_sensitivity = return_impact / abs(change * 100)
                change_unit = f'每1%变化'
            ...
            if param_def['change_type'] == 'percent':
                ...
                if change < 0:
                    change_desc = f"降低{abs(int(change*100))}%"
                else:
                    change_desc = f"提高{int(change*100)}%"
            else:  # absolute
                if change < 0:
                    change_desc = f"减少{abs(int(change))}个月"
                else:
                    change_desc = f"增加{int(change)}个月"
    ...
    ax1.set_title('A. 盈利概率敏感性', fontproperties=font_prop, fontsize=14, fontweight='bold')
    ...
    ax2.set_title('B. 预期收益率敏感性', fontproperties=font_prop, fontsize=14, fontweight='bold')
    ...
    # 总标题
    fig.suptitle('参数敏感性排序（龙卷风图）\n标准化单位变化：价格±10%、波动率/漂移率±5%、锁定期±1个月',
                 fontproperties=font_prop, fontsize=13, fontweight='bold', y=0.98)

    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()

    return top_prob, top_return
...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    top_prob, top_return = generate_tornado_chart(project_params['issue_price'], market_data['current_price'],
                                                         project_params['lockup_period'], volatility, drift, tornado_chart_path)

    add_paragraph(document, '图表 4.5: 参数敏感性排序（龙卷风图 - 双维度分析）')
    add_image(document, tornado_chart_path, width=Inches(6.5))
    add_paragraph(document, '')
    ...
    add_paragraph(document, '  - 锁定期：每1个月变化的影响')
    ...
    add_paragraph(document, '• **盈利概率敏感性（左图）**：')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '• **预期收益率敏感性（右图）**：')
    ...
    add_paragraph(document, '  - 锁定期对两个维度都有影响，但单位敏感性相对较小')
    ...
    # ==================== 4.6 敏感性分析综合排序 ====================
    add_paragraph(document, '')
    add_title(document, '4.6 敏感性分析综合排序', level=2)

    add_paragraph(document, '本节对前面4.2-4.5节的敏感性分析结果进行综合排序和对比分析。')
    ...
    # 添加排序表格
    add_paragraph(document, '')
    add_paragraph(document, '4.6.1 按盈利概率排序（保本概率）', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '📊 表4.6.1: 盈利概率排名 TOP 10')
    ...
    # 盈利幅度排序
    add_paragraph(document, '')
    add_paragraph(document, '4.6.2 按盈利幅度排序（20%收益目标达成概率）', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '📊 表4.6.2: 盈利幅度排名 TOP 10（20%收益目标）')
    ...
    # 综合结论
    add_paragraph(document, '')
    add_paragraph(document, '4.6.3 综合分析结论', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, f'   - 溢价率每降低5%（更深的折价），保本概率平均提升10-15%')
    add_paragraph(document, f'   - 溢价率每降低5%（更深的折价），保本概率平均提升10-15%')
    add_paragraph(document, f'   - 深度折价（-20%以下）的情景在盈利概率和盈利幅度排序中均位居前列')
    add_paragraph(document, '')

    add_paragraph(document, f'2. 漂移率对盈利幅度影响显著')
    add_paragraph(document, f'   - 漂移率从悲观转向乐观，可使20%盈利概率提升20-30%')
    add_paragraph(document, f'   - 漂移率从悲观转向乐观，可使20%盈利概率提升20-30个%')
    add_paragraph(document, '')

    add_paragraph(document, f'3. 波动率对盈利概率呈负相关')
    add_paragraph(document, f'   - 波动率降低25%，保本概率可提升约8-12%')
    add_paragraph(document, f'   - 波动率降低25%，保本概率可提升约8-12个%')
    add_paragraph(document, '')
    ...
    add_paragraph(document, f'   - 对于高波动率股票，要求更低的溢价率（更深的折价）以补偿风险')
    ...
    add_title(document, '5.1 模拟方法', level=2)
    ...
    add_title(document, '5.2 模拟参数', level=2)
    ...
    # ==================== 6.3 波动率 × 溢价率热力图 ====================
    add_paragraph(document, '')
    add_title(document, '6.3 波动率 × 溢价率热力图', level=2)
    ...
    try:
        ...
        add_paragraph(document, '图表 6.1: 盈利概率热力图 - 波动率 × 溢价率')
    ...
    # ==================== 6.4 漂移率 × 溢价率热力图 ====================
    add_paragraph(document, '')
    add_title(document, '6.4 漂移率 × 溢价率热力图', level=2)
    ...
    try:
        ...
        add_paragraph(document, '图表 6.2: 盈利概率热力图 - 漂移率 × 溢价率')
        ...
        add_paragraph(document, '• 黄色区域（盈利概率40-70%）：中等风险收益比，需谨慎评估')
        add_paragraph(document, '• 红色区域（盈利概率<40%）：负漂移 + 低溢价率（或正溢价率），高风险区域')
        ...
        add_paragraph(document, f'• 漂移率: {current_drift*100:+.2f}%')
        add_paragraph(document, f'• 溢价率: {current_discount*100:.2f}%')
    ...
    # ==================== 6.5 情景综合分析表 ====================
    add_paragraph(document, '')
    add_title(document, '6.5 情景综合分析表', level=2)
    ...
    scenarios_config.append({
        ...
        'volatility': max(0.25, industry_vol_120d * 0.8),  # 较低波动率
        'drift': max(0.15, industry_return_120d * 1.5),  # 较高漂移率
    ...
    scenarios_config.append({
        ...
        'volatility': industry_vol_120d,  # 中档波动率
        'drift': industry_return_120d,  # 中档漂移率
    ...
    scenarios_config.append({
        ...
        'volatility': min(0.45, industry_vol_120d * 1.2),  # 较高波动率
        'drift': min(-0.10, industry_return_120d * 0.5),  # 较低漂移率
        'premium_rate': 0.00  # 0%溢价率（平价）
    })

    # 情景5-7: 基于行业PE分位数的情景（如果有PE数据）
    pe_scenarios = []
    if 'stock_pe_data' in locals() and 'industry_pe_data' in locals():
        try:
            # 计算行业PE分位数
            ...
            scenarios_config.append({
                'name': '情景4（PE 75%分位数）',
                'description': f'行业PE 75%分位数({pe_75:.1f}倍)，溢价率-20%（深折价）',
            ...
            scenarios_config.append({
                'name': '情景5（PE 50%分位数）',
                'description': f'行业PE 50%分位数({pe_50:.1f}倍)，溢价率-10%（折价）',
                'volatility': industry_vol_120d,
                'drift': industry_return_120d,
                'premium_rate': -0.10,
                'pe_based': True
            ...
            scenarios_config.append({
                'name': '情景6（PE 25%分位数）',
                'description': f'行业PE 25%分位数({pe_25:.1f}倍)，溢价率0%（平价）',
                'volatility': industry_vol_120d,
                'drift': industry_return_120d,
                'premium_rate': 0.00,
                'pe_based': True
            ...
            pe_scenarios = ['情景4（PE 75%分位数）', '情景5（PE 50%分位数）', '情景6（PE 25%分位数）']
        except Exception as e:
    ...
    if comprehensive_results:
        ...
        for i, result in enumerate(comprehensive_results):
            ...
            # 生成推荐等级
            if category == '乐观':
                recommendation = '理想投资机会 ⭐⭐⭐'
            elif category == '中性':
                recommendation = '稳健投资 ⭐⭐'
            else:
                recommendation = '高风险 [X]'
        ...
        comprehensive_headers = ['情景类型', '波动率', '漂移率', '溢价率', '发行价(元)', '盈利概率(%)', '收益率中位数(%)', '95% VaR(%)', '风险评级', '投资建议']
    ...
    add_paragraph(document, '')
    add_paragraph(document, '三、压力测试评分体系说明', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '四、综合风险等级评定', bold=True)
    add_paragraph(document, '')
