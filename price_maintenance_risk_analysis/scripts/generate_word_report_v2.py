            ...
            if prices_list and len(prices_list) >= 100:
                ...
                if arima_result['model_fitted']:
                    try:
                        ...
                        # 图1: 对数收益率历史 + 拟合值 + 预测
                        ax1.plot(historical_index, log_returns.values, 'o-', alpha=0.5, markersize=3,
                                label='历史对数收益率', color='#3498db', linewidth=1)

                        # 绘制拟合值（in-sample prediction）
                        fitted_values = fitted_model.fittedvalues
                        fitted_index = range(len(fitted_values))
                        ax1.plot(fitted_index, fitted_values, '-', linewidth=1.5,
                                label='ARIMA拟合值', color='#e74c3c', alpha=0.8)

                        # 绘制预测值
                        ax1.plot(forecast_index, forecast_series.values, '--', linewidth=2,
                                label='ARIMA预测值', color='#27ae60', alpha=0.8)

                        # 添加历史均值参考线
                        mean_return = log_returns.mean()
                        ax1.axhline(y=mean_return, color='gray', linestyle=':', linewidth=1.5,
                                   label=f'历史均值: {mean_return:.4f}', alpha=0.6)

                        ax1.set_xlabel('交易日', fontproperties=font_prop, fontsize=12)
                        ax1.set_ylabel('对数收益率', fontproperties=font_prop, fontsize=12)
                        ax1.set_title('ARIMA(1,1,1)模型 - 对数收益率拟合与预测',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ax1.legend(prop=font_prop, fontsize=10, loc='upper left')
                        ax1.grid(True, alpha=0.3)
                        ...
                        cumulative_fitted = fitted_values.cumsum()

                        # 预测的累积收益率：从最后一个历史累积值开始累加预测值
                        last_cumulative = cumulative_returns.iloc[-1]
                        cumulative_forecast = pd.Series([last_cumulative] + list(forecast_series.cumsum().values + last_cumulative))

                        ax2.plot(historical_index, cumulative_returns.values, '-',
                                label='历史累积收益率', color='#3498db', linewidth=2, alpha=0.7)
                        ax2.plot(range(len(cumulative_fitted)), cumulative_fitted.values,
                                '--', label='ARIMA拟合+预测累积收益率', color='#27ae60', linewidth=2, alpha=0.8)
                        ...
                        ax2.set_xlabel('交易日', fontproperties=font_prop, fontsize=12)
                        ax2.set_ylabel('累积对数收益率', fontproperties=font_prop, fontsize=12)
                        ax2.set_title('ARIMA(1,1,1)模型 - 累积收益率拟合与预测',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ax2.legend(prop=font_prop, fontsize=10, loc='upper left')
                        ax2.grid(True, alpha=0.3)
                        ...
                        # 保存图表
                        arima_chart_path = os.path.join(IMAGES_DIR, '05_03_arima_fit_forecast.png')
                        plt.savefig(arima_chart_path, dpi=150, bbox_inches='tight')
                        plt.close()

                        add_paragraph(document, '')
                        add_paragraph(document, '图表 5.3.1: ARIMA模型拟合与预测曲线')
                        add_image(document, arima_chart_path, width=Inches(6))
            ...
            if 'forecaster' in locals():
                ...
                if garch_result['model_fitted']:
                    ...
                    try:
                        ...
                        returns = forecaster.prices.pct_change().dropna() * 100
                        conditional_vol = garch_result['conditional_variances'] ** 0.5  # 条件标准差
                        ...
                        # 添加长期平均波动率线
                        long_run_vol = np.sqrt(garch_result['omega'] / (1 - garch_result['persistence']))
                        ax.axhline(y=long_run_vol, color='green', linestyle='--', linewidth=1.5,
                                  label=f'长期平均波动率: {long_run_vol:.2f}%', alpha=0.7)
                        ...
                        add_paragraph(document, '• 绿色虚线：长期平均波动率（波动率回归的目标）')
