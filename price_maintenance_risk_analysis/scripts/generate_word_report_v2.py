...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    market_table_data = [
        ...
        ['季度区间收益率(60日)', f'{((1 + market_data.get("annual_return_60d", 0)) ** (60/365.0) - 1)*100:+.2f}%'],
    ...
    if actual_data_years > 0 and 'historical_fcf_data' in project_params:
        ...
        for i in range(min(actual_data_years, len(historical_fcf))):
            fcfs.append(historical_fcf[-(i+1)]['fcf'] * 100000000)  # 转回元，从最近开始

        # 剩余年数使用预测
        for i in range(projection_years):
            fcf_proj = fcfs[-1] * (1 + fcf_growth_example) ** (i + 1)
    ...
    for vol in volatilities:
        ...
        d = (np.log(ma20 / project_params['issue_price']) - lockup_drift) / lockup_vol
        ...
        expected_price = ma20 * np.exp(lockup_drift + lockup_vol**2 / 2)
    ...
    sensitivity_chart_paths = generate_sensitivity_charts_split(
        ...
        ma20, project_params['issue_price'], IMAGES_DIR)
    ...
    add_paragraph(document, f'• MA20价格: {ma20:.2f} 元/股（作为基准）')
    add_paragraph(document, '')
    ...
    scenario_chart_paths = generate_discount_scenario_charts_split(
        ma20, project_params['current_price'], risk_params['volatility'],
    ...
    current_discount = (project_params['issue_price'] - ma20) / ma20 * 100
    add_paragraph(document, f'• 当前发行价: {project_params["issue_price"]:.2f} 元')
    add_paragraph(document, f'• MA20基准价: {ma20:.2f} 元')
    ...
    try:
        ...
        add_paragraph(document, '图表 6.4: 优质情景TOP10 (盈利概率>50%)')
