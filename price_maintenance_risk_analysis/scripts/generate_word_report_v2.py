            ...
            if 'forecaster' in locals():
                ...
                if garch_result['model_fitted']:
                    ...
                    try:
                        ...
                        returns = forecaster.prices.pct_change().dropna() * 100
                        conditional_vol = garch_result['conditional_variances'] ** 0.5  # 条件标准差（日波动率，百分比）

                        # 绘制收益率和条件波动率
                        ax.plot(returns.index, returns.values, 'o-', alpha=0.3, markersize=2,
                               label='收益率', color='#3498db', linewidth=0.5)
                        ax.plot(conditional_vol.index, conditional_vol.values, '-',
                               label='条件波动率（GARCH拟合）', color='#e74c3c', linewidth=1.5)

                        # 计算长期平均波动率（日波动率和年化波动率）
                        long_run_vol_daily = np.sqrt(garch_result['omega'] / (1 - garch_result['persistence']))
                        long_run_vol_annual = long_run_vol_daily * np.sqrt(250)  # 年化

                        # 添加长期平均波动率线（日波动率）
                        ax.axhline(y=long_run_vol_daily, color='green', linestyle='--', linewidth=1.5,
                                  label=f'长期平均波动率（日）: {long_run_vol_daily:.2f}%', alpha=0.7)

                        # 添加±2倍条件波动率带
                        ax.fill_between(conditional_vol.index,
                                        conditional_vol.values * (-2),
                                        conditional_vol.values * 2,
                                        alpha=0.2, color='gray', label='±2倍条件波动率')

                        ax.set_xlabel('时间', fontproperties=font_prop, fontsize=12)
                        ax.set_ylabel('百分比 (%)', fontproperties=font_prop, fontsize=12)
                        ax.set_title('GARCH模型 - 条件波动率拟合与预测',
                                      fontproperties=font_prop, fontsize=14, fontweight='bold')
                        ...
                        add_paragraph(document, '')
                        add_paragraph(document, '📊 图表解读：', bold=True)
                        add_paragraph(document, '• 蓝色点：实际收益率（波动较大）')
                        add_paragraph(document, '• 红色线：GARCH模型拟合的条件波动率（反映波动率的变化规律）')
                        add_paragraph(document, f'• 绿色虚线：长期平均波动率（日）: {long_run_vol_daily:.2f}%')
                        add_paragraph(document, f'  → 年化为: {long_run_vol_annual:.2f}%（日波动率 × √250）')
                        add_paragraph(document, '• 灰色带：±2倍条件波动率区间（约95%置信区间）')
                        add_paragraph(document, f'• 未来半年预测波动率: {garch_result["forecast_volatility"]*100:.2f}%（年化）')
                        add_paragraph(document, '')
                        add_paragraph(document, '💡 说明：', bold=True)
                        add_paragraph(document, f'• 图表中显示的是日波动率（如{long_run_vol_daily:.2f}%）')
                        add_paragraph(document, f'• 预测结果使用年化波动率（如{garch_result["forecast_volatility"]*100:.2f}%）')
                        add_paragraph(document, '• 转换关系：年化波动率 = 日波动率 × √250（年化系数）')
                        add_paragraph(document, f'• 两者对比：长期平均{long_run_vol_annual:.2f}% vs 预测{garch_result["forecast_volatility"]*100:.2f}%')
