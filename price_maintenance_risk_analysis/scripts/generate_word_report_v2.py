...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    if '120日' in multi_window_mc_results:
        ...
        mc_profit_prob = mc_120_result['profit_prob']
        mc_mean_return = mc_120_result['mean_return']
        mc_median_return = mc_120_result['median_return']
        mc_percentile_5 = mc_120_result['percentile_5']
        mc_percentile_1 = mc_120_result['percentile_1']
        ...
        mc_eval = "风险较高" if mc_profit_prob < 50 else "风险适中" if mc_profit_prob < 70 else "风险较低"

        # 模拟结果表格
        monte_carlo_results = [
            ['模拟窗口', '120日（半年）', '基于历史120日波动率和收益率'],
            ['模拟次数', '10,000次', '蒙特卡洛模拟路径数'],
            ['盈利概率', f'{mc_profit_prob:.1f}%', '模拟中盈利场景占比'],
            ['预期年化收益', f'{mc_mean_return*100:.2f}%', '平均年化收益率'],
            ['中位数年化收益', f'{mc_median_return*100:.2f}%', '中位数年化收益率（更稳健）'],
            ['5% VaR（最坏5%）', f'{mc_percentile_5*100:.2f}%', '95%置信水平下的最大损失'],
            ['1% VaR（最坏1%）', f'{mc_percentile_1*100:.2f}%', '99%置信水平下的最大损失'],
            ['风险评估', mc_eval, '基于盈利概率综合判断'],
        ]
        ...
        add_table_data(document, ['模拟指标', '数值', '说明'], monte_carlo_results)

        add_paragraph(document, '')
        add_paragraph(document, '📋 模拟参数：', bold=True)
        add_table_data(document, ['参数', '数值', '说明'], monte_carlo_params)

        # 参数解读
        add_paragraph(document, '')
        add_paragraph(document, '💡 参数解读：', bold=True)
        add_paragraph(document, f'• 历史漂移率{mc_drift_120d*100:.2f}%：基于最近120日股价计算的年化收益率，反映历史趋势')
        add_paragraph(document, f'• 历史波动率{mc_volatility_120d*100:.2f}%：基于最近120日股价计算的年化波动率，反映历史波动程度')
        add_paragraph(document, f'• 锁定期{lockup_months}个月：定增后的持股期限，影响资金占用时间')
        add_paragraph(document, f'• 发行价{project_params["issue_price"]:.2f}元：定增认购价格，影响初始成本')
        add_paragraph(document, f'• 当前价{project_params["current_price"]:.2f}元：市场最新价格，影响盈亏')
        add_paragraph(document, f'• 溢价率{premium_rate*100:+.2f}%：发行价相对当前价的溢价/折价，正数表示溢价发行，负数表示折价发行')
        add_paragraph(document, '')
        add_paragraph(document, '📊 结果分析：', bold=True)
        if mc_profit_prob < 30:
            add_paragraph(document, f'• ⚠️ 盈利概率仅{mc_profit_prob:.1f}%（{mc_eval}），主要原因是：')
            if premium_rate > 0:
                add_paragraph(document, f'   - 溢价发行（溢价率{premium_rate*100:+.1f}%）增加了成本')
            if mc_drift_120d < 0:
                add_paragraph(document, f'   - 历史漂移率为负（{mc_drift_120d*100:.2f}%），股价呈下跌趋势')
            add_paragraph(document, f'   - 波动率较高（{mc_volatility_120d*100:.2f}%），增加了收益不确定性')
        elif mc_profit_prob > 70:
            add_paragraph(document, f'• ✅ 盈利概率{mc_profit_prob:.1f}%（{mc_eval}），主要利好：')
            if premium_rate < 0:
                add_paragraph(document, f'   - 折价发行（溢价率{premium_rate*100:+.1f}%）提供了安全边际')
            if mc_drift_120d > 0:
                add_paragraph(document, f'   - 历史漂移率为正（{mc_drift_120d*100:.2f}%），股价呈上涨趋势')
        else:
            add_paragraph(document, f'• 盈利概率{mc_profit_prob:.1f}%（{mc_eval}），风险收益平衡')
