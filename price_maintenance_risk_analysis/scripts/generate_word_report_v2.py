...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    add_paragraph(document, '通过对比个股与行业的PE历史分位数趋势，可以更清晰地判断当前估值处于历史哪个水平。')
    add_paragraph(document, '')
    ...
    try:
        ...
        # 获取个股历史PE数据（最近2年）
        stock_pe_data = analyzer.get_stock_pe_history(stock_code, days=730)

        # 获取行业历史PE数据
        industry_name, industry_code, industry_pe_data = analyzer.get_industry_pe_history(stock_code, days=730)
        ...
        if stock_pe_data is not None and industry_pe_data is not None:
            ...
            add_table_data(document, pe_history_table)
            ...
            chart_path = analyzer.generate_pe_trend_chart(
    ...
    if 'historical_fcf_data' in project_params and project_params['historical_fcf_data']:
        ...
        # 计算历史FCF增长率
        fcf_values = [d['fcf'] for d in historical_fcf['data'] if d['fcf'] > 0]
        print(f"   正值FCF年数: {len(fcf_values)}")
        if len(fcf_values) >= 2:
            fcf_growth_rates = []
            for i in range(1, len(fcf_values)):
                rate = (fcf_values[i] - fcf_values[i-1]) / fcf_values[i-1]
                fcf_growth_rates.append(rate)

            calculated_fcf_cagr = np.mean(fcf_growth_rates) if fcf_growth_rates else 0.10
            # 限制增长率在合理范围
            calculated_fcf_cagr = max(min(calculated_fcf_cagr, 0.30), -0.05)

            print(f"   历史FCF增长率: {calculated_fcf_cagr*100:.2f}%")
    ...
    if actual_data_years > 0 and 'historical_fcf_data' in project_params:
        ...
        # 计算历史FCF增长率（只计算正FCF年份的同比增长率）
        import numpy as np
        growth_rates = []
        for i in range(len(historical_fcf) - 1):
            fcf_current = historical_fcf[i]['fcf']
            fcf_next = historical_fcf[i+1]['fcf']
            if fcf_current > 0 and fcf_next > 0:  # 只计算正FCF年份
                growth_rate = (fcf_next - fcf_current) / fcf_current
        ...
        if growth_rates:
            ...
            avg_growth_rate = 0.05  # 默认5%
            median_growth_rate = 0.05

        # 使用平均增长率，但限制在合理范围（3%-8%）
        fcf_growth_example = max(0.03, min(0.08, avg_growth_rate))
        ...
        add_paragraph(document, f'• 正FCF年份平均增长率: {avg_growth_rate*100:.2f}%')
        add_paragraph(document, f'• 中位数增长率: {median_growth_rate*100:.2f}%')
        add_paragraph(document, f'• 采用预测增长率: {fcf_growth_example*100:.1f}%（基于历史平均，限制在3%-8%区间）')
    ...
    add_table_data(document, extreme_scenario_table)

    add_paragraph(document, '')
    add_paragraph(document, '二、极端情景模拟结果', bold=True)
    add_paragraph(document, '')

    extreme_results_table = [
        ['指标', '正常情景', '极端情景（三重打击）', '差异'],
    ...
    add_table_data(document, extreme_results_table)
    ...
    add_table_data(document, summary_table)
    ...
    # 创建参数说明表格
    var_params_table = []
    var_params_table.append(['参数', '60日窗口', '120日窗口', '250日窗口'])
    var_params_table.append(['数据窗口', '60日（季度）', '120日（半年线）', '250日（年线）'])
    var_params_table.append(['年化波动率', f'{var_results["60日"]["vol"]*100:.2f}%', f'{var_results["120日"]["vol"]*100:.2f}%', f'{var_results["250日"]["vol"]*100:.2f}%'])
    var_params_table.append(['年化漂移率', f'{var_results["60日"]["drift"]*100:+.2f}%', f'{var_results["120日"]["drift"]*100:+.2f}%', f'{var_results["250日"]["drift"]*100:+.2f}%'])
    var_params_table.append(['锁定期', f'{lockup_months}个月', f'{lockup_months}个月', f'{lockup_months}个月'])
    var_params_table.append(['模拟次数', '5,000次', '5,000次', '5,000次'])
    ...
    var_params_table.append(['发行价格', f'{project_params["issue_price"]:.2f}元/股', f'{project_params["issue_price"]:.2f}元/股', f'{project_params["issue_price"]:.2f}元/股'])
    var_params_table.append(['当前价格', f'{project_params["current_price"]:.2f}元/股', f'{project_params["current_price"]:.2f}元/股', f'{project_params["current_price"]:.2f}元/股'])
    var_params_table.append(['折价/溢价率', f'{discount_var:+.2f}% {discount_note}', f'{discount_var:+.2f}% {discount_note}', f'{discount_var:+.2f}% {discount_note}'])

    add_table_data(document, var_params_table)
    ...
    var_comparison_table.append(['窗口期', '95% VaR', '99% VaR', '95% CVaR', '99% CVaR', '风险等级'])
    ...
    add_table_data(document, var_comparison_table)
    ...
    # 总结三个窗口期的VaR
    var_summary_table = []
    var_summary_table.append(['窗口期', '95% VaR', '风险特征', '适用场景'])
    var_summary_table.append([
        '60日',
    ...
    add_table_data(document, var_summary_table)
    ...
    summary_data = [
        ...
        ['波动率(250日)', f'{mc_volatility*100:.1f}%'],
    ...
    add_paragraph(document, '')
    add_paragraph(document, '💡 指标说明：')
    add_paragraph(document, '• 蒙特卡洛模拟参数：基于250日窗口（年线），年化漂移率-31.52%，年化波动率42.64%')
