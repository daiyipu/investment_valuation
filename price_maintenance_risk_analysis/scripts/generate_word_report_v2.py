...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    if not skip_time_series:
        ...
        # 添加对高波动率下伊藤引理修正项的说明
        drift_adjusted = predicted_drift - 0.5 * predicted_vol**2
        add_paragraph(document, '')
        add_paragraph(document, '💡 重要说明：高波动率对漂移率的影响', bold=True)
        add_paragraph(document, f'• 原始预测漂移率: {predicted_drift*100:.2f}%（ARIMA预测）')
        add_paragraph(document, f'• 波动率修正项: -0.5 × σ² = {-0.5 * predicted_vol**2 * 100:.2f}%')
        add_paragraph(document, f'• 修正后漂移率: {drift_adjusted*100:.2f}%（用于GBM模拟）')

        if drift_adjusted < 0:
            add_paragraph(document, f'⚠️ 由于修正项过大（{-0.5 * predicted_vol**2 * 100:.2f}%），修正后漂移率为负')
            add_paragraph(document, f'⚠️ 这解释了为什么中位数收益率为负：高波动率抵消了正漂移率的影响')
            add_paragraph(document, f'💡 实际投资中，高波动率本身就是一种风险，需要更高的期望收益来补偿')
