...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    if 'stock_pe_data' in locals() and 'industry_pe_data' in locals() and stock_pe_data is not None and industry_pe_data is not None:
        try:
            ...
            # 比值 = 行业PE分位数 / 标的个股PE
            # 比值 > 1：个股PE低于行业，估值便宜，未来收益率应该更高
            # 比值 < 1：个股PE高于行业，估值贵，未来收益率应该更低
            ...
            # 使用比值作为漂移率的调整因子
            # 基础漂移率 × 比值 = 调整后的漂移率
            base_drift = industry_return_120d

            drift_pe_75 = base_drift * ratio_75
            drift_pe_50 = base_drift * ratio_50
            drift_pe_25 = base_drift * ratio_25
            ...
            print(f"   基础漂移率（行业120日）: {base_drift*100:+.2f}%")
