...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
            ...
            if prices_list and len(prices_list) >= 100:
                ...
                if arima_result['model_fitted']:
                    ...
                    try:
                        ...
                        # 绘制价格曲线（半透明，作为参考）
                        ax1_right.plot(historical_index, prices_series.values, '-',
                        ...
                        # 历史价格
                        ax2.plot(historical_index, prices_series.values, '-',
    ...
    add_paragraph(document, '• 数值越大表示风险越高')
    add_paragraph(document, '')
    ...
    for window_name in ['60日', '120日', '250日']:
        if window_name in multi_window_mc_results:
            ...
            # VaR = |负分位数|（表示损失）
            # percentile_10 = 10%分位数（左侧尾部最坏10%）= VaR 90%
            # percentile_5 = 5%分位数（左侧尾部最坏5%）= VaR 95%
            # percentile_1 = 1%分位数（左侧尾部最坏1%）= VaR 99%
            var_90 = abs(mc_result['percentile_10'])
            var_95 = abs(mc_result['percentile_5'])
            var_99 = abs(mc_result['percentile_1'])

            # CVaR直接使用5.2节的计算结果
            cvar_90 = abs(mc_result['cvar_90'])
            cvar_95 = abs(mc_result['cvar_95'])
            cvar_99 = abs(mc_result['cvar_99'])
    ...
    if has_percentile_data:
        ...
        add_paragraph(document, '⚠️ 历史分位数数据不可用，无法进行历史对比分析')
            '较大' if stock_market_cap > industry_market_cap else '较小'
        ])

    add_table_data(document, ['指标', '标的个股', '行业平均', '差异', '评价'], industry_comparison_data)
    add_paragraph(document, '')
    ...
    if has_industry_percentile and 'pe' in industry_stats_val:
        # 获取PE分位数数据
        pe_p25 = industry_stats_val['pe'].get('p25', 0)
        pe_p50 = industry_stats_val['pe'].get('p50', 0)
        pe_p75 = industry_stats_val['pe'].get('p75', 0)
    ...
    if '120日' in multi_window_mc_results:
        ...
        try:
            ...
            add_table_data(document, comparison_data)
    ...
    try:
        ...
        # 获取上海A股和深圳市场最近一天的数据
        df_sh_latest = pro.daily_info(trade_date=end_date, exchange='SH',
                                       fields='trade_date,ts_name,total_mv,float_mv,amount,vol,tr')
        df_sz_latest = pro.daily_info(trade_date=end_date, exchange='SZ',
                                       fields='trade_date,ts_name,total_mv,float_mv,amount,vol,tr')

        # 提取上海A股数据
        sh_a_latest = df_sh_latest[df_sh_latest['ts_name'] == '上海A股']
        if len(sh_a_latest) > 0:
            sh_turnover = sh_a_latest.iloc[0]['tr']  # 上海有tr字段
            sh_mv = sh_a_latest.iloc[0]['total_mv']
            sh_float_mv = sh_a_latest.iloc[0]['float_mv']
        else:
            sh_turnover = 1.5
            sh_mv = 500000
            sh_float_mv = 450000
        ...
        # 提取深圳市场数据（使用自计算的换手率）
        sz_market_latest = df_sz_latest[df_sz_latest['ts_name'] == '深圳市场']
        if len(sz_market_latest) > 0:
            # 深交所tr字段可能不准，自己计算：换手率 = 成交额/流通市值 * 100
            sz_amount = sz_market_latest.iloc[0]['amount']
            sz_float_mv = sz_market_latest.iloc[0]['float_mv']
            sz_turnover = (sz_amount / sz_float_mv) * 100 if sz_float_mv > 0 else 3.0
            sz_mv = sz_market_latest.iloc[0]['total_mv']
        else:
            sz_turnover = 3.0
            sz_mv = 400000
            sz_float_mv = 360000
        ...
        # 按市值加权计算综合换手率
        total_mv = sh_mv + sz_mv
        weighted_turnover = (sh_turnover * sh_mv + sz_turnover * sz_mv) / total_mv

        print(f"✅ 成功获取市场换手率数据：")
        print(f"   上海A股换手率={sh_turnover:.2f}%，市值={sh_mv:.2f}亿")
        print(f"   深圳市场换手率={sz_turnover:.2f}%（自计算），市值={sz_mv:.2f}亿")
        print(f"   加权换手率={weighted_turnover:.2f}%")

        # 获取最近120个交易日的换手率数据以计算历史分位数
        print(f"正在获取历史换手率数据（最近120个交易日）...")

        historical_turnovers = []
        current_date_hist = datetime.now()
        count_hist = 0
        max_days_hist = 365

        while count_hist < 120 and max_days_hist > 0:
            date_str_hist = current_date_hist.strftime('%Y%m%d')

            try:
                # 获取上海A股数据
                df_sh_hist = pro.daily_info(trade_date=date_str_hist, exchange='SH',
                                            fields='trade_date,ts_name,total_mv,float_mv,tr')
                sh_a_hist = df_sh_hist[df_sh_hist['ts_name'] == '上海A股']

                # 获取深圳市场数据
                df_sz_hist = pro.daily_info(trade_date=date_str_hist, exchange='SZ',
                                            fields='trade_date,ts_name,total_mv,float_mv,amount')
                sz_mkt_hist = df_sz_hist[df_sz_hist['ts_name'] == '深圳市场']

                if len(sh_a_hist) > 0 and len(sz_mkt_hist) > 0:
                    # 上海使用tr字段
                    sh_tr_hist = sh_a_hist.iloc[0]['tr']
                    sh_mv_hist = sh_a_hist.iloc[0]['total_mv']

                    # 深圳自计算换手率
                    sz_amount_hist = sz_mkt_hist.iloc[0]['amount']
                    sz_float_mv_hist = sz_mkt_hist.iloc[0]['float_mv']
                    sz_tr_hist = (sz_amount_hist / sz_float_mv_hist) * 100 if sz_float_mv_hist > 0 else 3.0
                    sz_mv_hist = sz_mkt_hist.iloc[0]['total_mv']

                    # 按市值加权
                    total_mv_hist = sh_mv_hist + sz_mv_hist
                    weighted_tr_hist = (sh_tr_hist * sh_mv_hist + sz_tr_hist * sz_mv_hist) / total_mv_hist

                    historical_turnovers.append(weighted_tr_hist)
                    count_hist += 1
        ...
        # 计算历史分位数
        if len(historical_turnovers) >= 60:
            current_turnover_percentile = (pd.Series(historical_turnovers) < weighted_turnover).mean() * 100
            print(f"✅ 成功获取{len(historical_turnovers)}天历史数据，当前分位数={current_turnover_percentile:.1f}%")
        else:
            print(f"⚠️ 仅获取{len(historical_turnovers)}天数据，使用默认分位数50%")
        ...
        market_activity_level = "平稳"
        market_activity_score = 60
        market_activity_desc = "数据获取失败，使用默认值"
        current_percentile = 2.0
    ...
    macro_env_data = [
        ...
        ['二级市场活跃度', market_activity_level, f'{market_activity_score}', '40%', f'{market_activity_score * 0.4:.1f}',
         f'{market_activity_desc}，基于120日交易日换手率计算'],
    ]

    add_table_data(document, macro_env_data)

    # 评估说明
    add_paragraph(document, '')
    add_paragraph(document, '💡 评估说明：', bold=True)
    add_paragraph(document, '• 评估周期：最近120个交易日（约6个月）')
    ...
    add_paragraph(document, f'• 当前换手率：{total_turnover*100:.2f}%（120日平均）')
    add_paragraph(document, f'• 历史分位数：{current_percentile:.1f}%（最近120日）')
    add_paragraph(document, '• 数据来源：上证指数(60%) + 深证A指(35%) + 北交所(5%)')
    add_paragraph(document, '')
    ...
    for result in comprehensive_results:
        if 'scenario' in result and 'median_return' in result and 'profit_prob' in result:
            scenario_name = result['scenario']['name']
            median_return = result['median_return'] * 100  # 转为百分比
            profit_prob = result['profit_prob']  # 已经是百分比

            # 获取情景的溢价率
            scenario_premium = result.get('premium_rate', 0)

            # 获取情景的VaR（使用percentile_5作为95% VaR）
            var_95 = abs(result.get('percentile_5', median_return)) * 100

            scenario_options.append({
                'name': scenario_name,
                'median_return': median_return,
                'profit_prob': profit_prob,
                'premium_rate': scenario_premium,
                'var_95': var_95,
                'issue_price': current_price_eval * (1 + scenario_premium / 100)
            })
    ...
    if '120日' in multi_window_mc_results:
        ...
        scenario_options.append({
            'name': '历史参数模拟（120日）',
            'median_return': mc_120_result['median_return'] * 100,
            'profit_prob': mc_120_result['profit_prob'],
            'premium_rate': discount_premium * 100,  # 当前溢价率
            'var_95': abs(mc_120_result.get('percentile_5', 0)) * 100,
    ...
    if qualified_scenarios:
        ...
        # 没有符合条件的方案，使用模型反推最高溢价率
        add_paragraph(document, '⚠️ 情景分析中未找到完全符合所有4个筛选条件的方案')
        add_paragraph(document, '')
        add_paragraph(document, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
        add_paragraph(document, '备选方案：基于目标收益率反推最高溢价率', bold=True)
        add_paragraph(document, '')
        ...
        add_paragraph(document, '当情景分析中没有符合条件的方案时，使用以下逻辑反推最高可接受溢价率：')
        add_paragraph(document, '')
        ...
        add_table_data(document, pricing_options)
