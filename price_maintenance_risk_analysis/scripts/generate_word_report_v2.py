...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    try:
        ...
        # 上证指数
        try:
            sh_index_data = ak.index_daily(symbol="sh000001")
            sh_index_data['date'] = pd.to_datetime(sh_index_data['date'])
            sh_index_data = sh_index_data.sort_values('date')
            sh_recent_120 = sh_index_data.tail(120)
            sh_avg_amount = sh_recent_120['amount'].mean()  # 成交额（万元）
            sh_latest_amount = sh_index_data.iloc[-1]['amount']
            sh_market_cap = sh_index_data.iloc[-1].get('market_cap', sh_avg_amount * 100)  # 总市值，如果没有就用成交额估算
            # 计算换手率 = 成交额 / 总市值
            sh_turnover = (sh_avg_amount / sh_market_cap) if sh_market_cap > 0 else 0.02
        except Exception as e:
            print(f"⚠️ 获取上证指数数据失败: {e}")
            sh_turnover = 0.02
            sh_latest_amount = 0

        # 深证A指（399107.SZ）- 代表深市所有A股
        try:
            sz_index_data = ak.index_daily(symbol="sz399107")
            sz_index_data['date'] = pd.to_datetime(sz_index_data['date'])
            sz_index_data = sz_index_data.sort_values('date')
            sz_recent_120 = sz_index_data.tail(120)
            sz_avg_amount = sz_recent_120['amount'].mean()
            sz_latest_amount = sz_index_data.iloc[-1]['amount']
            sz_market_cap = sz_index_data.iloc[-1].get('market_cap', sz_avg_amount * 100)
            sz_turnover = (sz_avg_amount / sz_market_cap) if sz_market_cap > 0 else 0.025
        except Exception as e:
            print(f"⚠️ 获取深证A指数据失败: {e}")
            sz_turnover = 0.025
            sz_latest_amount = 0

        # 北交所指数（如果没有就用默认值）
        try:
            bse_index_data = ak.index_daily(symbol="bj899001")
            bse_index_data['date'] = pd.to_datetime(bse_index_data['date'])
            bse_index_data = bse_index_data.sort_values('date')
            bse_recent_120 = bse_index_data.tail(120)
            bse_avg_amount = bse_recent_120['amount'].mean()
            bse_latest_amount = bse_index_data.iloc[-1]['amount']
            bse_market_cap = bse_index_data.iloc[-1].get('market_cap', bse_avg_amount * 50)
            bse_turnover = (bse_avg_amount / bse_market_cap) if bse_market_cap > 0 else 0.01
        except Exception as e:
            print(f"⚠️ 获取北交所指数数据失败: {e}")
            bse_turnover = 0.01
            bse_latest_amount = 0

        # 计算综合换手率（加权平均）
        total_turnover = (sh_turnover * 0.6 + sz_turnover * 0.35 + bse_turnover * 0.05)

        # 计算历史分位数（基于120日数据）
        all_turnovers = []
        for i in range(len(sh_recent_120)):
            sh_day_amount = sh_recent_120.iloc[i]['amount']
            sz_day_amount = sz_recent_120.iloc[i]['amount'] if len(sz_recent_120) > i else 0
            bse_day_amount = bse_recent_120.iloc[i]['amount'] if len(bse_recent_120) > i else 0
            day_turnover = (sh_day_amount / sh_market_cap * 0.6 +
                           sz_day_amount / sz_market_cap * 0.35 +
                           bse_day_amount / bse_market_cap * 0.05) if all([sh_market_cap, sz_market_cap, bse_market_cap]) else total_turnover
            all_turnovers.append(day_turnover)

        # 计算当前换手率的历史分位数
        current_percentile = (pd.Series(all_turnovers) < total_turnover).mean() * 100

    except Exception as e:
        print(f"⚠️ 计算二级市场活跃度失败: {e}")
        total_turnover = 0.02
    ...
    if current_percentile < 20:
        ...
        market_activity_desc = f"历史分位数20%-40%（当前{current_percentile:.1f}%）"
    elif current_percentile < 60:
        market_activity_level = "平稳"
        market_activity_score = 60
        market_activity_desc = f"历史分位数40%-60%（当前{current_percentile:.1f}%）"
    elif current_percentile < 80:
        market_activity_level = "较活跃"
        market_activity_score = 80
        market_activity_desc = f"历史分位数60%-80%（当前{current_percentile:.1f}%）"
    else:
        market_activity_level = "活跃"
        market_activity_score = 100
        market_activity_desc = f"历史分位数>80%（当前{current_percentile:.1f}%）"
