...
def generate_discount_scenario_charts_split(base_price, current_price, volatility, drift, lockup_period, save_dir):
    ...
    ax.axhline(y=base_price, color='green', linestyle='--', linewidth=2,
               label=f'MA20基准 ({base_price:.2f}元)', alpha=0.7)
    ...
    # 选择一个代表性的折价率（如15%）来展示盈利幅度
    representative_discount = 15
    discount_15_data = next((d for d in discount_data if d['rate'] == representative_discount), None)

    if discount_15_data:
        profit_targets_display = ['0%', '5%', '10%', '15%', '20%', '30%', '50%']
        probs = [discount_15_data.get(f'prob_{int(t[:-1])}pct', 0) for t in profit_targets_display]

        colors_gradient = plt.cm.RdYlGn(np.linspace(0.2, 0.8, len(profit_targets_display)))
        bars = ax.bar(profit_targets_display, probs, color=colors_gradient, alpha=0.8, edgecolor='black', linewidth=2)

        ax.set_xlabel('盈利目标', fontproperties=font_prop, fontsize=14)
        ax.set_ylabel('达成概率 (%)', fontproperties=font_prop, fontsize=14)
        ax.set_title(f'折价发行({representative_discount}%) - 盈利幅度敏感性分析',
        ...
        for bar, prob in zip(bars, probs):
            if prob > 5:  # 只在概率足够高时显示标签
                ax.text(bar.get_x() + bar.get_width()/2, prob + 1,
                        f"{prob:.1f}%", ha='center', va='bottom', fontsize=11, fontweight='bold')

        # 添加参考线
        ax.axhline(y=50, color='gray', linestyle='--', alpha=0.5, label='50%概率线')
        ax.legend(prop=font_prop, fontsize=12)
        ...
        plt.tight_layout()
        chart_path = os.path.join(save_dir, 'scenario_discount_profit_magnitude_split.png')
        plt.savefig(chart_path, dpi=150, bbox_inches='tight')
        plt.close()
        chart_paths.append(chart_path)
    ...
    # 选择一个代表性的溢价率（如15%）来展示盈利幅度
    representative_premium = 15
    premium_15_data = next((p for p in premium_data if p['rate'] == representative_premium), None)

    if premium_15_data:
        profit_targets_display = ['0%', '5%', '10%', '15%', '20%', '30%', '50%']
        probs = [premium_15_data.get(f'prob_{int(t[:-1])}pct', 0) for t in profit_targets_display]

        colors_gradient = plt.cm.Reds(np.linspace(0.4, 0.9, len(profit_targets_display)))
        bars = ax.bar(profit_targets_display, probs, color=colors_gradient, alpha=0.8, edgecolor='black', linewidth=2)

        ax.set_xlabel('盈利目标', fontproperties=font_prop, fontsize=14)
        ax.set_ylabel('达成概率 (%)', fontproperties=font_prop, fontsize=14)
        ax.set_title(f'溢价发行({representative_premium}%) - 盈利幅度敏感性分析',
                     fontproperties=font_prop, fontsize=16, fontweight='bold')
        ax.set_ylim(0, 100)
        ax.grid(True, alpha=0.3, linestyle='--', axis='y')
        ...
        # 在柱子上添加数值标签
        for bar, prob in zip(bars, probs):
            if prob > 1:  # 溢价发行概率较低，降低阈值
                ax.text(bar.get_x() + bar.get_width()/2, prob + 0.5,
                        f"{prob:.1f}%", ha='center', va='bottom', fontsize=11, fontweight='bold')
        ...
        plt.tight_layout()
        chart_path = os.path.join(save_dir, 'scenario_premium_profit_magnitude_split.png')
        plt.savefig(chart_path, dpi=150, bbox_inches='tight')
        plt.close()
        chart_paths.append(chart_path)

    # 图7：不同发行价下的盈利目标对比（热力图）
    fig, ax = plt.subplots(figsize=(14, 8))

    # 创建热力图数据：行=发行价情景，列=盈利目标
    all_scenarios = discount_data + premium_data
    scenario_labels = [f"{d['rate']}%折价" if 'rate' in d else f"{p['rate']}%溢价"
                       for d, p in zip(discount_data, premium_data)]

    heatmap_data = []
    for scenario in all_scenarios:
        row_data = [scenario.get(f'prob_{int(t[:-1])}pct', 0) for t in profit_targets_display]
    ...
    ax.set_xticks(range(len(profit_targets_display)))
    ax.set_xticklabels(profit_targets_display, fontproperties=font_prop, fontsize=12)
    ax.set_yticks(range(len(scenario_labels)))
    ax.set_yticklabels(scenario_labels, fontproperties=font_prop, fontsize=11)

    ax.set_xlabel('盈利目标', fontproperties=font_prop, fontsize=14)
    ax.set_ylabel('发行价情景', fontproperties=font_prop, fontsize=14)
    ax.set_title('发行价敏感性：盈利目标达成概率热力图',
                 fontproperties=font_prop, fontsize=16, fontweight='bold')

    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label('达成概率 (%)', fontproperties=font_prop, fontsize=12)

    # 在热力图上添加数值标签
    for i in range(len(scenario_labels)):
        for j in range(len(profit_targets_display)):
            value = heatmap_data[i, j]
            if value >= 5:  # 只在概率>=5%时显示
    ...
    plt.tight_layout()
    chart_path = os.path.join(save_dir, 'scenario_profit_magnitude_heatmap_split.png')
    plt.savefig(chart_path, dpi=150, bbox_inches='tight')
    plt.close()
    chart_paths.append(chart_path)
...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    print(f"计算得出发行数量: {issue_shares:,} 股（融资金额: {project_params['financing_amount']/100000000:.2f}亿元 ÷ 发行价: {project_params['issue_price']:.2f}元/股）")
    ...
    project_data = [
        ...
        ['融资金额', f'{project_params["financing_amount"] / 100000000:.2f} 亿元'],
        ['当前价格', f'{project_params["current_price"]:.2f} 元/股'],
    ...
    if os.path.exists(indices_data_file):
        ...
        try:
            ...
            add_paragraph(document, '⚠️ 注：以下数据为历史数据，年化收益率计算方法可能与最新标准存在差异。')
            ...
            add_paragraph(document, '• 250日（年线）年化收益率采用复利公式计算：(1 + 期间收益率)^(365/250) - 1')
            add_paragraph(document, '• 若部分指数收益率偏高（如创业板90%+），可能反映了市场在该时间段的大幅上涨')
            add_paragraph(document, '• 建议以沪深300为基准参考，其年化收益率约30%较为合理')
    ...
    add_paragraph(document, '本章节分析光弘科技所属的申万三级行业"消费电子零部件及组装"的行业指数表现，为项目风险评估提供行业环境参考。')
    ...
    try:
        ...
        if os.path.exists(industry_data_file):
            ...
            # 行业波动率
            add_paragraph(document, '')
            add_paragraph(document, '⚠️ 行业波动率：')
            ...
            # 行业技术位置
            add_paragraph(document, '')
            add_paragraph(document, '📊 行业技术位置：')
            add_paragraph(document, f"• MA20: {industry_data.get('ma_20', 0):.2f}")
            add_paragraph(document, f"• MA60: {industry_data.get('ma_60', 0):.2f}")
            add_paragraph(document, f"• MA120: {industry_data.get('ma_120', 0):.2f}")
            add_paragraph(document, f"• MA250: {industry_data.get('ma_250', 0):.2f}")

            # 行业胜率
            add_paragraph(document, '')
            add_paragraph(document, f"• 60日胜率: {industry_data.get('win_rate_60d', 0)*100:.1f}%")
            ...
            # 个股与行业对比
            add_paragraph(document, '')
            add_paragraph(document, '📊 个股与行业对比（60日窗口）：')
            ...
            comparison_headers = ['指标', '光弘科技', '行业指数', '差异']
            comparison_data = [
                ['波动率', f"{stock_volatility*100:.2f}%", f"{industry_volatility*100:.2f}%", f"{(stock_volatility-industry_volatility)*100:+.2f}%"],
                ['年化收益率', f"{stock_return*100:+.2f}%", f"{industry_return*100:+.2f}%", f"{(stock_return-industry_return)*100:+.2f}%"],
            ]
            add_table_data(document, comparison_headers, comparison_data)

            # 分析结论
            add_paragraph(document, '')
            add_paragraph(document, '💡 行业分析结论：')
    ...
    add_title(document, '2.4 估值回归情景分析', level=2)
    ...
    add_paragraph(document, '')
    add_paragraph(document, '详细分析请参见：第七章 压力测试 → 7.1 PE回归压力测试')
    ...
    calculated_fcf_cagr = None  # 初始化FCF复合增长率变量
    ...
    if 'historical_fcf_data' in project_params and project_params['historical_fcf_data']:
        ...
        if len(positive_fcf_data) >= 2:
            ...
            # CAGR = (最终值 / 初始值)^(1/年数) - 1
            calculated_fcf_cagr = (last_fcf_val / first_fcf_val) ** (1 / years_count) - 1

            # 限制增长率在合理范围（-5%到30%）
            calculated_fcf_cagr = max(-0.05, min(0.30, calculated_fcf_cagr))

            print(f"   历史FCF CAGR: {calculated_fcf_cagr*100:.2f}%（从{first_year_data['year']}年{first_fcf_val:.2f}亿到{last_year_data['year']}年{last_fcf_val:.2f}亿，{years_count}年）")
        else:
            calculated_fcf_cagr = None
    ...
    if calculated_fcf_cagr is not None:
        fcf_growth_example = max(calculated_fcf_cagr, 0.05)  # 至少5%，避免过于保守
        if fcf_growth_example > 0.30:  # 限制最高30%
            fcf_growth_example = 0.30

        if historical_fcf_years > 0:
            fcf_growth_source = f"基于历史{historical_fcf_years}年FCF数据"
        elif historical_incomes is not None:
            fcf_growth_source = f"基于历史{len(historical_incomes)-1}年净利润CAGR"
    ...
    dcf_process_data = [
        ...
        ['最新FCF', f'{base_fcf/100000000:.2f} 亿元'],
    ...
    if actual_data_years > 0 and 'historical_fcf_data' in project_params:
        ...
        if len(positive_fcf_years) >= 2:
            ...
            # CAGR = (最终值 / 初始值)^(1/年数) - 1
            cagr_fcf = (last_fcf / first_fcf) ** (1 / years_span) - 1

            # 限制增长率在合理范围（-5% 到 15%）
            cagr_fcf = max(-0.05, min(0.15, cagr_fcf))
        else:
            cagr_fcf = 0.05  # 默认5%

        # 使用CAGR增长率
        fcf_growth_example = cagr_fcf
        ...
        add_paragraph(document, f'• 历史CAGR: {cagr_fcf*100:.2f}%（从{first_fcf_year["year"]}年{first_fcf:.2f}亿到{last_fcf_year["year"]}年{last_fcf:.2f}亿，跨度{years_span}年）')
        add_paragraph(document, f'• 采用预测增长率: {fcf_growth_example*100:.1f}%（基于历史CAGR，限制在-5%至15%区间）')
    ...
    try:
        ...
        if ts_token:
            ...
            if not df_prices.empty and len(df_prices) > 180:
                ...
                vol_120 = time_window_results["volatility"][3] if len(time_window_results["volatility"]) > 3 else time_window_results["volatility"][2]
                vol_250 = time_window_results["volatility"][-1]
                ...
                ret_120 = time_window_results["annual_return"][3] if len(time_window_results["annual_return"]) > 3 else time_window_results["annual_return"][2]
    ...
    add_paragraph(document, '')
    add_paragraph(document, '发行类型定义（基于120日均线MA120）：')
    add_paragraph(document, '• 折价发行：发行价 < MA120，有安全边际，盈利阈值 = 发行价')
    add_paragraph(document, '• 溢价发行：发行价 > MA120，无安全边际，盈利阈值 = max(MA120, 发行价×1.02)')
    add_paragraph(document, '')
    ...
    # 添加拆分的图表
    add_paragraph(document, '')
    add_paragraph(document, '图表 4.4: 折价发行情景 - 盈利概率')
    add_image(document, scenario_chart_paths[0], width=Inches(6.5))
    add_paragraph(document, '')

    add_paragraph(document, '图表 4.5: 溢价发行情景 - 盈利概率')
    add_image(document, scenario_chart_paths[1], width=Inches(6.5))
    add_paragraph(document, '')

    add_paragraph(document, '图表 4.6: 预期收益率对比')
    add_image(document, scenario_chart_paths[2], width=Inches(6.5))
    add_paragraph(document, '')

    add_paragraph(document, '图表 4.7: 发行价格对比')
    add_image(document, scenario_chart_paths[3], width=Inches(6.5))
    add_paragraph(document, '')

    # 新增：盈利幅度敏感性分析图表
    add_paragraph(document, '')
    add_title(document, '4.2.1 盈利幅度敏感性分析', level=3)

    add_paragraph(document, '除盈利概率外，投资者更关注"能赚多少"。本节分析不同盈利目标下的达成概率。')
    add_paragraph(document, '')

    if len(scenario_chart_paths) > 4 and os.path.exists(scenario_chart_paths[4]):
        add_paragraph(document, '图表 4.8: 折价发行(15%) - 盈利幅度敏感性分析')
        add_image(document, scenario_chart_paths[4], width=Inches(6.5))
        add_paragraph(document, '')

    if len(scenario_chart_paths) > 5 and os.path.exists(scenario_chart_paths[5]):
        add_paragraph(document, '图表 4.9: 溢价发行(15%) - 盈利幅度敏感性分析')
        add_image(document, scenario_chart_paths[5], width=Inches(6.5))
        add_paragraph(document, '')

    if len(scenario_chart_paths) > 6 and os.path.exists(scenario_chart_paths[6]):
        add_paragraph(document, '图表 4.10: 发行价敏感性 - 盈利目标达成概率热力图')
        add_image(document, scenario_chart_paths[6], width=Inches(6.5))
        add_paragraph(document, '')

    # 盈利幅度分析说明
    add_paragraph(document, '')
    add_paragraph(document, '💡 盈利幅度分析说明：')
    add_paragraph(document, '• 横轴：盈利目标（如5%表示股价上涨5%）')
    add_paragraph(document, '• 纵轴：达成该盈利目标的概率')
    add_paragraph(document, '• 折价发行：安全边际高，即使是高盈利目标（如+50%）也有一定达成概率')
    add_paragraph(document, '• 溢价发行：无安全边际，即使是保本（0%）也面临挑战')
    add_paragraph(document, '• 热力图：展示所有发行价情景下，不同盈利目标的达成概率')
    add_paragraph(document, '')

    # 添加参数说明
    add_paragraph(document, '')
    add_paragraph(document, '💡 图表参数说明：')
    add_paragraph(document, '• 数据窗口：120日（半年线，统一分析基准）')
    add_paragraph(document, f'• 年化波动率：{risk_params["volatility"]*100:.2f}%')
    add_paragraph(document, f'• 年化漂移率：{risk_params["drift"]*100:.2f}%')
    add_paragraph(document, '• 锁定期：6个月')
    add_paragraph(document, '• 盈利概率计算方法：基于正态分布模型的累积分布函数（CDF）')
    add_paragraph(document, '  - 计算公式：P(盈利) = 1 - Φ(-z_score)')
    add_paragraph(document, '  - 其中 z_score = (锁定期漂移率 - 要求收益率) / 锁定期波动率')
    add_paragraph(document, '• 注意：此处的盈利概率是理论模型计算值，非蒙特卡洛模拟的平均值或中位数')
    add_paragraph(document, '• 敏感性分析方法：单一变量变化，其他参数保持不变')
    add_paragraph(document, '')
    ...
    add_paragraph(document, f'• MA120基准价: {ma120:.2f} 元')
    ...
    for drift, scenario_name in drift_scenarios:
        ...
        # 计算基础盈利概率（0%，即保本）
        d = (np.log(ma120 / project_params['issue_price']) - lockup_drift) / lockup_vol
        prob = stats.norm.cdf(d) * 100

        # 计算期望收益率
        expected_price = ma120 * np.exp(lockup_drift + lockup_vol**2 / 2)
        expected_return = (expected_price - project_params['issue_price']) / project_params['issue_price'] * 100

        # 计算不同盈利目标的概率
        profit_probs = []
        for target_pct in profit_targets_pct:
            target_price = project_params['issue_price'] * (1 + target_pct/100)
            d_target = (np.log(target_price / project_params['issue_price']) - lockup_drift) / lockup_vol
            prob_target = stats.norm.cdf(d_target) * 100
    ...
    add_paragraph(document, '• 漂移率为正时，达成20%盈利目标的概率可能超过50%，投资价值凸显')
    add_paragraph(document, '')
    ...
    add_paragraph(document, '• 漂移率每提升10个百分点，盈利概率约提升15-25个百分点')
    ...
    add_paragraph(document, f'• 在当前120日窗口波动率下，项目盈利概率受波动率影响显著')
    add_paragraph(document, f'• 保持漂移率和锁定期不变，波动率每增加10个百分点，盈利概率下降约15-20个百分点')
    add_paragraph(document, f'• 建议结合4.1节的时间窗口分析，综合评估不同窗口期的波动风险')

    # ==================== 4.5 锁定期敏感性分析 ====================
    add_paragraph(document, '')
    add_title(document, '4.5 锁定期敏感性分析', level=2)
    ...
    add_paragraph(document, '')
    add_paragraph(document, '图表 4.15: 锁定期敏感性分析 - 盈亏平衡价格')
    add_image(document, lockup_chart1_path, width=Inches(6.5))
    add_paragraph(document, '')
    add_paragraph(document, '图表 4.16: 锁定期敏感性分析 - 需要涨幅')
    add_image(document, lockup_chart2_path, width=Inches(6.5))

    # ==================== 4.6 参数敏感性排序（龙卷风图） ====================
    add_paragraph(document, '')
    add_title(document, '4.6 参数敏感性排序（龙卷风图）', level=2)
    ...
    add_paragraph(document, '图表 4.17: 参数敏感性排序（龙卷风图）')
    ...
    # ==================== 4.7 敏感性分析综合排序 ====================
    add_paragraph(document, '')
    add_title(document, '4.7 敏感性分析综合排序', level=2)

    add_paragraph(document, '本节对前面4.2-4.6节的敏感性分析结果进行综合排序和对比分析。')
    add_paragraph(document, '通过对比不同参数情景下的盈利概率和盈利幅度，识别最优和最差情景。')
    add_paragraph(document, '')
    add_paragraph(document, '💡 排序说明：')
    add_paragraph(document, '• 所有分析均基于120日窗口数据')
    add_paragraph(document, '• 盈利概率：达到保本（0%收益）的概率')
    add_paragraph(document, '• 盈利幅度：达到20%收益率的概率（代表高收益目标）')
    add_paragraph(document, '• 排序帮助投资者识别最具吸引力和最需警惕的情景')
    add_paragraph(document, '')

    # 综合排序数据
    # 收集所有敏感性分析情景的盈利概率和盈利幅度数据
    ranking_data = []

    # 获取基础参数
    base_vol = market_data.get('volatility_120d', risk_params.get('volatility', 0.30))
    base_drift = market_data.get('annual_return_120d', risk_params.get('drift', 0.08))
    lockup_months = project_params['lockup_period']

    # 1. 发行价折扣情景排序
    discount_rates = [-30, -25, -20, -15, -10, -5, 0, 5, 10, 15, 20]
    for d in discount_rates:
        issue_price_scenario = ma120 * (1 + d/100)
        lockup_drift = base_drift * (lockup_months / 12)
        lockup_vol = base_vol * np.sqrt(lockup_months / 12)

        # 盈利概率（保本）
        z = (np.log(ma120 / issue_price_scenario) - lockup_drift) / lockup_vol
        prob_profit = stats.norm.cdf(z) * 100

        # 盈利幅度（20%收益目标）
        target_price = issue_price_scenario * 1.20
        required_return = (target_price - ma120) / ma120
        z_20 = (lockup_drift - required_return) / lockup_vol
        prob_20pct = (1 - stats.norm.cdf(-z_20)) * 100
        ...
        ranking_data.append({
            ...
            'expected_return': (ma120 * np.exp(lockup_drift + lockup_vol**2 / 2) - issue_price_scenario) / issue_price_scenario * 100
        })

    # 2. 漂移率情景排序
    drift_scenarios_ranking = [
        (-0.10, "极度悲观"),
        (-0.05, "悲观"),
        (0.00, "中性"),
        (0.05, "轻度乐观"),
    ...
    for drift, scenario_name in drift_scenarios_ranking:
        lockup_drift = drift * (lockup_months / 12)
        lockup_vol = base_vol * np.sqrt(lockup_months / 12)

        # 盈利概率
        z = (np.log(ma120 / project_params['issue_price']) - lockup_drift) / lockup_vol
        prob_profit = stats.norm.cdf(z) * 100

        # 盈利幅度（20%）
        target_price = project_params['issue_price'] * 1.20
        required_return = (target_price - ma120) / ma120
        z_20 = (lockup_drift - required_return) / lockup_vol
        prob_20pct = (1 - stats.norm.cdf(-z_20)) * 100
        ...
        ranking_data.append({
            ...
            'prob_profit': prob_profit,
            'prob_20pct': prob_20pct,
            'expected_return': (ma120 * np.exp(lockup_drift + lockup_vol**2 / 2) - project_params['issue_price']) / project_params['issue_price'] * 100
        })

    # 3. 波动率情景排序
    vol_levels = [base_vol * 0.5, base_vol * 0.75, base_vol, base_vol * 1.25, base_vol * 1.5]
    ...
    # 按盈利概率排序
    ranking_data_sorted_by_prob = sorted(ranking_data, key=lambda x: x['prob_profit'], reverse=True)

    # 按盈利幅度排序
    ranking_data_sorted_by_magnitude = sorted(ranking_data, key=lambda x: x['prob_20pct'], reverse=True)

    # 添加排序表格
    add_paragraph(document, '')
    add_paragraph(document, '4.7.1 按盈利概率排序（保本概率）', bold=True)
    add_paragraph(document, '')

    # Top 10 and Bottom 5 by profit probability
    top_10_prob = ranking_data_sorted_by_prob[:10]
    bottom_5_prob = ranking_data_sorted_by_prob[-5:]

    prob_table_data = []
    for i, item in enumerate(top_10_prob, 1):
        prob_table_data.append([
            str(i),
            item['category'],
            item['scenario'],
            f'{item["prob_profit"]:.1f}%',
            f'{item["prob_20pct"]:.1f}%',
            f'{item["expected_return"]:+.1f}%'
        ])

    add_paragraph(document, '📊 表4.7.1: 盈利概率排名 TOP 10')
    add_table_data(document, ['排名', '参数类型', '情景', '保本概率', '20%盈利概率', '期望收益率'], prob_table_data)

    add_paragraph(document, '')
    add_paragraph(document, '💡 关键发现：')
    add_paragraph(document, f'• 最优情景：{top_10_prob[0]["category"]} - {top_10_prob[0]["scenario"]}，保本概率达 {top_10_prob[0]["prob_profit"]:.1f}%')
    add_paragraph(document, f'• 当前参数位于第 {next(i for i, item in enumerate(ranking_data_sorted_by_prob, 1) if item["category"] == "波动率" and "当前" in item["scenario"])} 位（基于120日波动率）')
    add_paragraph(document, '')

    # 盈利幅度排序
    add_paragraph(document, '')
    add_paragraph(document, '4.7.2 按盈利幅度排序（20%收益目标达成概率）', bold=True)
    add_paragraph(document, '')

    top_10_magnitude = ranking_data_sorted_by_magnitude[:10]

    magnitude_table_data = []
    for i, item in enumerate(top_10_magnitude, 1):
        magnitude_table_data.append([
            str(i),
            item['category'],
            item['scenario'],
            f'{item["prob_profit"]:.1f}%',
    ...
    add_paragraph(document, '📊 表4.7.2: 盈利幅度排名 TOP 10（20%收益目标）')
    add_table_data(document, ['排名', '参数类型', '情景', '保本概率', '20%盈利概率', '期望收益率'], magnitude_table_data)

    add_paragraph(document, '')
    add_paragraph(document, '💡 关键发现：')
    add_paragraph(document, f'• 高收益最优情景：{top_10_magnitude[0]["category"]} - {top_10_magnitude[0]["scenario"]}，20%盈利概率达 {top_10_magnitude[0]["prob_20pct"]:.1f}%')
    add_paragraph(document, '• 折价发行的情景在盈利幅度排序中占据前列，体现了安全边际的重要性')
    add_paragraph(document, '• 漂移率提升对盈利幅度的改善效果显著，体现了长期趋势的关键作用')
    add_paragraph(document, '')

    # 综合结论
    add_paragraph(document, '')
    add_paragraph(document, '4.7.3 综合分析结论', bold=True)
    add_paragraph(document, '')
    add_paragraph(document, '基于120日窗口的敏感性分析综合结论：')
    add_paragraph(document, '')

    # 计算当前参数的排名
    current_vol_120d = market_data.get('volatility_120d', risk_params.get('volatility', 0.30))
    current_drift_120d = market_data.get('annual_return_120d', risk_params.get('drift', 0.08))
    current_discount_ranking = next((i for i, item in enumerate(ranking_data_sorted_by_prob, 1)
                                     if item["category"] == "发行价折扣"), 0)

    add_paragraph(document, f'1. 发行价折扣是影响盈利概率最敏感的因素')
    add_paragraph(document, f'   - 折价率每增加5个百分点，保本概率平均提升10-15个百分点')
    add_paragraph(document, f'   - 深度折价（-20%以下）的情景在盈利概率和盈利幅度排序中均位居前列')
    add_paragraph(document, '')

    add_paragraph(document, f'2. 漂移率对盈利幅度影响显著')
    add_paragraph(document, f'   - 当前120日窗口漂移率为{current_drift_120d*100:+.2f}%，属于{"乐观" if current_drift_120d > 0.05 else "中性" if current_drift_120d > 0 else "悲观"}情景')
    add_paragraph(document, f'   - 漂移率从悲观转向乐观，可使20%盈利概率提升20-30个百分点')
    add_paragraph(document, '')

    add_paragraph(document, f'3. 波动率对盈利概率呈负相关')
    add_paragraph(document, f'   - 当前120日窗口波动率为{current_vol_120d*100:.2f}%，波动率{"较高" if current_vol_120d > 0.35 else "适中" if current_vol_120d > 0.25 else "较低"}')
    add_paragraph(document, f'   - 波动率降低25%，保本概率可提升约8-12个百分点')
    add_paragraph(document, '')

    add_paragraph(document, f'4. 投资建议')
    add_paragraph(document, f'   - 优先选择深度折价的定增项目，确保足够的安全边际')
    add_paragraph(document, f'   - 关注标的股的长期趋势（漂移率），避免下降趋势中的项目')
    add_paragraph(document, f'   - 对于高波动率股票，要求更高的折价率以补偿风险')
    add_paragraph(document, f'   - 综合考虑盈利概率和盈利幅度，选择平衡最优的方案')
    add_paragraph(document, '')
    ...
    mc_params = [
        ...
        ['发行价格', f'{project_params["issue_price"]:.2f} 元/股'],
        ['定增折价率', f'{discount_rate:+.2f}% {discount_basis}'],
        [''],
        ['60日窗口（季度）参数', ''],
    ...
    add_paragraph(document, '')
    add_paragraph(document, '图表 5.6: 蒙特卡洛模拟结果 - 关键统计指标')
    add_image(document, mc_chart_paths[4])

    add_paragraph(document, '')
    add_paragraph(document, '图表 5.7: 蒙特卡洛模拟结果 - 概率分布曲线')
    ...
    window_headers = ['窗口期', '波动率', '历史年化收益率', '折价/溢价率', '模拟预期年化收益', '中位数收益', '盈利概率', '5% VaR', '95% VaR']
    window_table_data = []

    # 为每个窗口期，计算不同折价/溢价率下的模拟结果
    # 折价率：-20%, -15%, -10%, -5%（负值表示发行价低于市场价，即折价）
    # 溢价率：0%, 5%, 10%, 15%, 20%（正值表示发行价高于市场价，即溢价）
    ...
    for window_name in ['60日', '120日', '250日']:
        if window_name in multi_window_mc_results:
            ...
            # 显示当前项目的折价/溢价率（对所有窗口期都相同，因为这是项目属性）
            window_table_data.append([
                window_name,
                f"{r['volatility']*100:.2f}%",
                f"{r['drift']*100:.2f}%",  # 历史年化收益率（蒙特卡洛输入参数）
                f"{current_premium_discount:+.2f}%",  # 当前项目的折价/溢价率（负值=折价，正值=溢价）
            ...
            for rate in discount_scenarios:
                ...
                # 根据折价/溢价率显示不同的标签
                if rate < 0:
                    rate_label = f"折价率{abs(rate):.0f}%"
                elif rate == 0:
                    rate_label = "平价"
                else:
                    rate_label = f"溢价率{rate:.0f}%"

                window_table_data.append([
                    f"  └ {rate_label}",
                    "",  # 波动率留空
                    "",  # 历史收益率留空
                    f"{rate:+.0f}%",  # 折价/溢价率（负值=折价，正值=溢价）
    ...
    add_paragraph(document, '• 折价/溢价率：')
    add_paragraph(document, '  - 折价率（负值）：发行价低于市场价，如-20%表示发行价是市场价的80%（深度折价）')
    add_paragraph(document, '  - 平价（0%）：发行价等于市场价')
    add_paragraph(document, '  - 溢价率（正值）：发行价高于市场价，如+10%表示发行价是市场价的110%（溢价发行）')
    ...
    try:
        ...
        add_paragraph(document, '')
        add_paragraph(document, '投资建议：')
        add_paragraph(document, '• 优先选择折价率不低于-10%的定增项目，确保足够安全边际')
    ...
    try:
        ...
        add_paragraph(document, '• 绿色区域（盈利概率>70%）：正漂移 + 低溢价率（高折价），投资安全边际充足')
        add_paragraph(document, '• 黄色区域（盈利概率40-70%）：中等风险收益比，需谨慎评估')
    ...
    # 当前项目实际情况（折价/溢价率相对于MA20计算）
    current_discount = (project_params['issue_price'] - ma20) / ma20  # 负值表示折价，正值表示溢价
    current_vol = market_data.get('volatility_60d', risk_params.get('volatility', 0.30))
    ...
    comprehensive_headers = ['情景类型', '波动率', '期间收益率', '溢价率', '发行价(元)', '投资建议']
    ...
    add_paragraph(document, '• 当前项目定位在基准情景，需结合具体溢价率和市场表现评估（负值表示折价）')
    add_paragraph(document, '• 谨慎对待悲观情景和极端情景，风险较高')
    add_paragraph(document, '• 建议要求至少-15%溢价率（即15%折价）以获取足够安全边际')
    ...
    try:
        ...
        add_paragraph(document, '')
        add_paragraph(document, '图表 6.5: 不同波动率下的情景对比')
        add_image(document, multi_dim_chart_paths[1], width=Inches(6.5))
        add_paragraph(document, '')

        add_paragraph(document, '图表 6.6: 优质情景TOP10 (盈利概率>50%)')
    ...
    add_paragraph(document, '一、压力情景定义', bold=True)
    add_paragraph(document, '')
    ...
    # 二、压力测试结果
    add_paragraph(document, '')
    add_paragraph(document, '二、压力测试结果', bold=True)
    add_paragraph(document, '')
    ...
    # 三、经济面压力测试结论
    add_paragraph(document, '')
    add_paragraph(document, '三、经济面压力测试结论', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '一、极端情景组合定义', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '二、极端情景模拟结果', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '三、极端情景分析', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '一、压力测试全景汇总', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '二、最坏风险情景分析', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '三、综合风险等级评定', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '四、压力测试核心结论', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '五、风险控制建议', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, f'• VaR数值范围：95% VaR在{var_95_min*100:.2f}%至{var_95_max*100:.2f}%之间，差异{var_95_range*100:.2f}个百分点')
    ...
    add_paragraph(document, '一、多窗口期VaR综合评估', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '二、VaR风险度量汇总', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '三、综合风险评级', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '四、风险控制建议', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '五、VaR分析的局限性', bold=True)
    add_paragraph(document, '')
    ...
    add_paragraph(document, '基于多维度量化指标，对项目进行综合风险评分。')
    add_paragraph(document, '评分维度包括盈利概率、发行折价、锁定期、波动率和VaR等关键风险指标。')
    add_paragraph(document, '')
    ...
    # ==================== 9.3 最终报价建议 ====================
    add_paragraph(document, '')
    add_title(document, '9.3 最终报价建议', level=2)
    ...
    # ==================== 十、风控建议与风险提示 ====================
    add_title(document, '十、风控建议与风险提示', level=1)

    add_paragraph(document, '本章节从风险控制角度，给出盈亏平衡分析、风控建议和全面的风险提示。')
    add_paragraph(document, '基于保守原则，确保投资决策在合理风险可控范围内进行。')
    add_paragraph(document, '')

    # ==================== 10.1 盈亏平衡价格分析 ====================
    add_title(document, '10.1 盈亏平衡价格分析', level=2)

    add_paragraph(document, '分析在不同期望年化收益率要求下，解禁时需要达到的盈亏平衡价格。')
    add_paragraph(document, '通过计算不同收益率目标下的盈亏平衡价格，评估项目的安全边际，为风控决策提供支撑。')
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '图表 10.1: 盈亏平衡价格敏感性曲线')
    ...
    add_paragraph(document, '基于120日窗口（半年线）的蒙特卡洛模拟结果，汇总核心风险指标：')
    add_paragraph(document, '')

    # 处理盈利概率和预期收益率的格式（可能是小数或百分比形式）
    ...
    # 统一使用120日窗口参数（与敏感性分析保持一致）
    mc_volatility_120d = market_data.get('volatility_120d', market_data.get('volatility', 0.30))
    mc_drift_120d = market_data.get('annual_return_120d', market_data.get('drift', 0.08))

    summary_data = [
        ['风险评分', f'{total_score}/100'],
        ['盈利概率', f'{profit_prob_display:.1f}%'],
        ['发行类型', f'{issue_type}'],
        ['安全边际/溢价率', f'{discount_premium:.2f}%'],
    ...
    add_paragraph(document, '')
    add_paragraph(document, '💡 指标说明：')
    add_paragraph(document, '• 蒙特卡洛模拟参数：基于120日窗口（半年线），年化漂移率{:+.2f}%，年化波动率{:.1f}%'.format(mc_drift_120d*100, mc_volatility_120d*100))
    add_paragraph(document, '• 预期收益率：蒙特卡洛模拟平均收益率，考虑了极端损失的影响')
    add_paragraph(document, '• 盈利概率：10,000次模拟中，到期价高于发行价的比例')
    add_paragraph(document, '• VaR：基于120日窗口参数计算的95%置信水平下的最大可能损失')

    add_paragraph(document, '')

    # ==================== 10.3 风控建议 ====================
    add_title(document, '10.3 风控建议', level=2)

    add_paragraph(document, '基于盈亏平衡价格分析、风险评分和多维度情景分析，从风险控制角度给出以下建议：')
    add_paragraph(document, '')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '三、综合风控建议（保守原则）')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '四、风控策略建议')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '五、压力测试与VaR风险提示')
    ...
    add_paragraph(document, '')
    add_paragraph(document, '六、多重方案选项建议')
    ...
    add_title(document, '10.4 主要风险提示', level=2)

    add_paragraph(document, '基于120日窗口（半年线）的多维度风险分析，提示以下主要风险：')
    add_paragraph(document, '')

    # 一、市场风险
    add_paragraph(document, '一、市场风险（基于120日窗口）')
    mc_volatility_120d = market_data.get('volatility_120d', market_data.get('volatility', 0.30))
    mc_drift_120d = market_data.get('annual_return_120d', market_data.get('drift', 0.08))
    ...
    # 二、流动性风险
    add_paragraph(document, '二、流动性风险')
    add_paragraph(document, f'• 锁定期风险：{project_params["lockup_period"]}个月锁定期内无法交易，需承担期间价格波动')
    add_paragraph(document, f'• 解禁冲击：解禁后可能面临抛压，导致实际变现价格低于理论价格')
    add_paragraph(document, '')

    # 三、VaR在险价值风险（多窗口期对比）
    add_paragraph(document, '三、VaR在险价值风险（多窗口期对比）')

    # 获取多窗口期VaR数据
    var_95_120d = var_95 * 100  # 120日窗口

    # 如果有多窗口期VaR结果，展示对比
    var_summary_text = f'• 120日窗口：95%置信水平下最大可能亏损{var_95_120d:.1f}%'
    var_summary_text += f'，99%置信水平下最大可能亏损{var_99*100:.1f}%' if 'var_99' in locals() else ''
    add_paragraph(document, var_summary_text)

    add_paragraph(document, '• VaR解读：在{:.0f}%的置信水平下，投资损失不超过上述百分比。但需注意，VaR基于历史数据，极端市场情况下可能失效'.format(95 if 'var_99' not in locals() else 99))
    add_paragraph(document, '• 尾部风险：历史数据显示，小概率极端事件（黑天鹅）可能导致损失超过VaR预测值')
    add_paragraph(document, '')

    # 四、压力测试风险
    add_paragraph(document, '四、压力测试风险提示')
    add_paragraph(document, '• PE回归风险：若标的PE回归至行业Q1分位数（低估值区间），股价可能面临下行压力')
    add_paragraph(document, '• 极端情景风险：历史危机情景（2008金融危机、2020疫情等）显示，极端情况下股价可能下跌40%-60%')
    add_paragraph(document, '• 三重打击风险：深度溢价发行（20%）+ 高波动率（1.5倍）+ 负向漂移率（-10%）同时发生时，亏损概率和亏损幅度显著增加')
    add_paragraph(document, '• 敏感性风险：发行价对盈利概率和盈利幅度影响最大，其次为波动率和漂移率')
    add_paragraph(document, '')

    # 五、估值风险
    add_paragraph(document, '五、估值风险')
    add_paragraph(document, f'• DCF估值风险：DCF内在价值{intrinsic_value:.2f}元/股基于多个假设（WACC={project_params.get("wacc", 0.10)*100:.1f}%，永续增长率={project_params.get("perpetual_growth", 0.02)*100:.1f}%），实际业绩可能偏离预测')
    add_paragraph(document, '• 相对估值风险：PE/PS/PB相对估值基于行业平均水平，行业景气度变化可能导致估值体系重构')
    add_paragraph(document, '')

    # 六、发行定价风险
    add_paragraph(document, '六、发行定价风险')
    current_discount = (project_params['current_price'] - project_params['issue_price']) / project_params['current_price'] * 100

    if current_discount > 0:
        add_paragraph(document, f'• 折价风险：当前折价率为{current_discount:.2f}%，若折价率不足（<10%），安全边际有限')
    else:
        add_paragraph(document, f'• 溢价风险：当前溢价率为{abs(current_discount):.2f}%，溢价发行无安全边际，需重点关注公司成长性')

    add_paragraph(document, '• 发行定价偏离：若发行价显著高于盈亏平衡价格，将大幅降低盈利概率和预期收益')
    add_paragraph(document, '')

    # 七、行业与公司特定风险
    add_paragraph(document, '七、行业与公司特定风险')
    add_paragraph(document, '• 行业政策风险：需关注行业监管政策变化，如医疗、教育、互联网等行业的历史政策风险')
    add_paragraph(document, '• 竞争格局风险：行业竞争加剧可能导致毛利率下降，影响公司盈利能力')
    add_paragraph(document, '• 业绩暴雷风险：需关注公司业绩预告、审计报告等，防范财务造假或业绩大幅下滑风险')
    add_paragraph(document, '')

    # 八、多重方案风险提示
    add_paragraph(document, '八、多重方案风险提示（基于120日窗口）')
    add_paragraph(document, '根据不同的报价方案，需关注以下风险：')
    add_paragraph(document, '')

    # 保守方案风险
    add_paragraph(document, '🛡️ 保守型方案（15%目标收益）：')
    add_paragraph(document, '   • 优势：折价率较高，安全边际充足，盈利概率较高')
    add_paragraph(document, '   • 风险：可能因报价过低而错配机会，或无法获得足额配售')
    add_paragraph(document, '')

    # 平衡方案风险
    add_paragraph(document, '⚖️ 平衡型方案（20%目标收益）：')
    add_paragraph(document, '   • 优势：收益与风险相对平衡，符合一般投资要求')
    add_paragraph(document, '   • 风险：对市场波动敏感，需密切关注120日窗口指标变化')
    add_paragraph(document, '')

    # 积极方案风险
    add_paragraph(document, '🚀 积极型方案（25%目标收益）：')
    add_paragraph(document, '   • 优势：可参与更多项目，提高资金使用效率')
    add_paragraph(document, '   • 风险：折价率较低或溢价发行，安全边际有限，需承受较高波动风险')
    add_paragraph(document, '')

    # 九、综合风险等级
    add_paragraph(document, '九、综合风险等级评定')
    if total_score >= 80:
        risk_level = "较低风险 🟢"
        risk_description = "整体风险可控，但仍需关注市场波动和公司基本面变化"
    elif total_score >= 60:
        risk_level = "中等风险 🟡"
        risk_description = "风险与收益相对平衡，建议适度参与并控制仓位"
    else:
        risk_level = "较高风险 🔴"
        risk_description = "整体风险较高，建议谨慎参与或要求更高折价率"

    add_paragraph(document, f'• 综合风险评分：{total_score}/100 分')
    add_paragraph(document, f'• 风险等级：{risk_level}')
    add_paragraph(document, f'• 风险描述：{risk_description}')
    add_paragraph(document, '')

    # 十、特别风险提示
    add_paragraph(document, '十、特别风险提示')
    add_paragraph(document, '⚠️ 本报告基于120日窗口历史数据和多情景模拟，不构成投资承诺')
    add_paragraph(document, '⚠️ 历史数据不代表未来表现，实际收益可能因市场环境变化而偏离预期')
    add_paragraph(document, '⚠️ 极端市场事件（黑天鹅）可能造成超过VaR预测的损失')
    add_paragraph(document, '⚠️ 投资决策应结合最新市场情况、公司公告和投资者自身风险承受能力')
    add_paragraph(document, '⚠️ 建议建立动态监控机制，定期评估120日窗口指标变化，及时调整投资策略')

    add_title(document, '10.5 免责声明', level=2)
    ...
    add_paragraph(document, '')
    add_paragraph(document, '说明：本表按波动率分块展示，便于根据实际市场波动率选择参考情景。建议参考创业板180日波动率（约36%）确定目标波动率区间。')
    add_paragraph(document, '')
    ...
    for vol_name, vol_min, vol_max in vol_ranges:
        ...
        # 在区间内按预期年化收益率和盈利概率排序（收益率优先）
        scenarios_sorted = sorted(scenarios_in_range,
                                 key=lambda x: (x['mean_return'], x['profit_prob']),
                                 reverse=True)
    ...
    add_paragraph(document, '• 排序方式：按波动率分块后，在每个区块内按预期年化收益率和盈利概率从高到低排序')
    ...
    add_paragraph(document, '• 在相同波动率区间内，优先选择收益率和盈利概率较高的情景')
    add_paragraph(document, '• 结合当前项目实际参数，找到最接近的情景作为参考基准')
