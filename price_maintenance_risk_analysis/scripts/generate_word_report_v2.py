...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    try:
        ...
        if stock_pe_data is not None and industry_pe_data is not None:
            ...
            stock_pe_percentile = float((stock_pe_data['pe_ttm'] < stock_pe_current).sum() / len(stock_pe_data) * 100)
            ...
            sw_index_pe_percentile = float((industry_pe_data['pe_ttm'] < sw_index_pe_current).sum() / len(industry_pe_data) * 100)
            ...
            try:
                ...
                if peer_pe_histories:
                    ...
                    # iloc返回可能包含Series，需要用.values[0]提取
