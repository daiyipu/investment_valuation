...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    # ==================== 9.1.1 个股与行业对比 ====================
    add_title(document, '9.1.1 个股与行业对比', level=3)

    # 获取个股和行业数据
    stock_pe = market_data.get('pe_ratio', 0)
    stock_pb = market_data.get('pb_ratio', 0)
    industry_pe = industry_stats_val.get('pe', {}).get('current', 0) if 'industry_stats_val' in locals() else 0
    industry_pb = industry_stats_val.get('pb', {}).get('current', 0) if 'industry_stats_val' in locals() else 0

    # 计算比较
    pe_comparison = ((stock_pe - industry_pe) / industry_pe * 100) if industry_pe > 0 else 0
    pb_comparison = ((stock_pb - industry_pb) / industry_pb * 100) if industry_pb > 0 else 0

    # 估值评价
    pe_eval = "低估" if pe_comparison < -10 else "合理" if pe_comparison < 10 else "高估"
    pb_eval = "低估" if pb_comparison < -10 else "合理" if pb_comparison < 10 else "高估"

    industry_comparison_data = [
        ['市盈率(PE)', f'{stock_pe:.2f}倍', f'{industry_pe:.2f}倍', f'{pe_comparison:+.1f}%', pe_eval],
        ['市净率(PB)', f'{stock_pb:.2f}倍', f'{industry_pb:.2f}倍', f'{pb_comparison:+.1f}%', pb_eval],
    ]

    # 添加市值比较（如果有）
    if 'market_cap' in market_data and 'market_cap' in industry_stats_val:
        stock_market_cap = market_data['market_cap']
        industry_market_cap = industry_stats_val['market_cap']
        market_cap_comparison = ((stock_market_cap - industry_market_cap) / industry_market_cap * 100) if industry_market_cap > 0 else 0
        industry_comparison_data.append([
            '市值',
            f'{stock_market_cap/100000000:.2f}亿元',
            f'{industry_market_cap/100000000:.2f}亿元',
            f'{market_cap_comparison:+.1f}%',
    ...
    # 获取PE分位数数据
    pe_p25 = industry_stats_val.get('pe', {}).get('p25', 0) if 'industry_stats_val' in locals() else 0
    pe_p50 = industry_stats_val.get('pe', {}).get('p50', 0) if 'industry_stats_val' in locals() else 0
    pe_p75 = industry_stats_val.get('pe', {}).get('p75', 0) if 'industry_stats_val' in locals() else 0

    # 计算在不同分位数下的目标价
    eps_rel = market_data.get('eps', 0)
    target_price_p25 = eps_rel * pe_p25 if eps_rel > 0 and pe_p25 > 0 else 0
    target_price_p50 = eps_rel * pe_p50 if eps_rel > 0 and pe_p50 > 0 else 0
    target_price_p75 = eps_rel * pe_p75 if eps_rel > 0 and pe_p75 > 0 else 0
