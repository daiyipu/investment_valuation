...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    try:
        ...
        # 获取最近5年（约1200个交易日）的换手率数据以计算历史分位数
        print(f"正在获取历史换手率数据（最近5年）...")

        historical_turnovers = []
        current_date_hist = datetime.now()
        count_hist = 0
        max_days_hist = 365 * 5  # 5年

        while count_hist < 1200 and max_days_hist > 0:  # 目标1200个交易日
        ...
        # 计算历史分位数
        if len(historical_turnovers) >= 600:  # 至少有600天数据（约2.4年）
            current_turnover_percentile = (pd.Series(historical_turnovers) < weighted_turnover).mean() * 100
            print(f"✅ 成功获取{len(historical_turnovers)}天历史数据（约{len(historical_turnovers)/240:.1f}年），当前分位数={current_turnover_percentile:.1f}%")
        else:
