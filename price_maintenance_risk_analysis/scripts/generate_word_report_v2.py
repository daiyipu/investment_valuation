...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    if qualified_scenarios:
        ...
        # 没有符合条件的方案
        add_paragraph(document, '⚠️ 未找到完全符合所有4个筛选条件的方案')
        add_paragraph(document, '')
        add_paragraph(document, '建议：')
        add_paragraph(document, '• 适当放宽筛选条件（如降低收益率要求或提高VaR阈值）')
        add_paragraph(document, '• 重新评估项目风险收益特征')
        add_paragraph(document, '• 考虑其他投资机会')

        # 显示最接近的方案（部分符合条件）
        add_paragraph(document, '')
        add_paragraph(document, '最接近条件的方案（仅供参考）：', bold=True)

        # 按满足条件数量排序
        scenario_options.sort(key=lambda x: sum([
            x['premium_rate'] <= -5,
            x['median_return'] > 8,
            x['var_95'] < 60,
            x['profit_prob'] > 50
        ...
        if scenario_options:
            closest = scenario_options[0]
            satisfied_count = sum([
                closest['premium_rate'] <= -5,
                closest['median_return'] > 8,
                closest['var_95'] < 60,
                closest['profit_prob'] > 50
            ])

            closest_data = [
                ['方案名称', closest['name'], f'满足{satisfied_count}/4个条件'],
                ['建议报价', f"{closest['issue_price']:.2f}元/股", f"溢价率{closest['premium_rate']:+.2f}%"],
                ['中位数收益率', f"{closest['median_return']:.2f}%", '✅' if closest['median_return'] > 8 else '❌'],
                ['95% VaR', f"{closest['var_95']:.2f}%", '✅' if closest['var_95'] < 60 else '❌'],
                ['盈利概率', f"{closest['profit_prob']:.1f}%", '✅' if closest['profit_prob'] > 50 else '❌'],
            ]

            add_table_data(document, ['指标', '数值', '满足条件'], closest_data)
