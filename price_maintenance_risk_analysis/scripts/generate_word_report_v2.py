...
def generate_sensitivity_charts_split(volatilities, profit_probs, current_vol, ma20, issue_price, save_dir):
    ...
    for d in drift_range_new:
        ...
        for discount in discount_range:
            issue_price_hm = ma20 * (1 - discount/100)
            lockup_drift = d * (6/12)
    ...
    current_discount_for_hm = ((ma20 - issue_price) / ma20) * 100  # 计算当前折价率
...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    if issue_price < ma20:
        issue_type = "折价发行"
        discount_premium = (ma20 - issue_price) / ma20 * 100
    else:
        issue_type = "溢价发行"
    ...
    # 计算当前发行价的折扣
    current_discount = (ma20 - project_params['issue_price']) / ma20 * 100
