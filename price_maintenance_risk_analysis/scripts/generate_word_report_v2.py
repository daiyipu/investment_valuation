...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    # ==================== 9.1.8 综合评分 ====================
    add_title(document, '9.1.8 综合风险评分', level=3)
    ...
    summary_data = [
        ['综合风险评分', f'{total_score}/100', risk_level_eval, '基于多维度量化评估'],
        ['盈利概率', f'{profit_prob_display:.1f}%', profit_level_eval, '蒙特卡洛模拟盈利概率'],
        ...
        ['预期年化收益率', f'{mean_return_display:.2f}%', '较高' if mean_return_display > 10 else '中等' if mean_return_display > 0 else '负收益', '模拟平均年化收益率'],
        ['95% VaR', f'{var_95*100:.2f}%', '风险可接受' if var_95 < 0.25 else '风险较高', '95%置信水平下的最大损失'],
        ...
        ['DCF内在价值', f'{intrinsic_value:.2f}元/股', dcf_eval, '基于自由现金流折现'],
    ]
    add_table_data(document, ['评分类别', '分值/指标', '评估', '说明'], summary_data)
