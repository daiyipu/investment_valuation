...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    for result in comprehensive_results:
        if 'scenario' in result:
            ...
            elif '指数乐观' in scenario_name:
                index_optimistic_result = result
    ...
    if current_scenario_result:
        scenario_data.append([
            '当前情景',
            f'{current_scenario_result["profit_prob"]*100:.1f}%',
    ...
    if index_neutral_result:
        scenario_data.append([
            '指数中性情景',
            f'{index_neutral_result["profit_prob"]*100:.1f}%',
    ...
    if industry_neutral_result:
        scenario_data.append([
            '行业中性情景',
            f'{industry_neutral_result["profit_prob"]*100:.1f}%',
            f'{industry_neutral_result["median_return"]*100:.2f}%',
            '基于所属行业历史数据'
        ])

    if index_optimistic_result:
        scenario_data.append([
            '指数乐观情景',
            f'{index_optimistic_result["profit_prob"]*100:.1f}%',
            f'{index_optimistic_result["median_return"]*100:.2f}%',
            '最佳情景（仅供参考）'
        ])
    ...
    for result in comprehensive_results:
        if 'scenario' in result and '极端' in result['scenario']['name']:
            extreme_stress_result = result
            break

    if extreme_stress_result:
        stress_data = [
            ['极端情景（三重打击）', f'{extreme_stress_result["profit_prob"]*100:.1f}%', f'{extreme_stress_result["median_return"]*100:.2f}%', '股价下跌20% + 波动率2.5倍 + 漂移率恶化'],
        ]

        # 与当前情景对比
        if current_scenario_result:
            profit_prob_diff = extreme_stress_result["profit_prob"] - current_scenario_result["profit_prob"]
            median_return_diff = extreme_stress_result["median_return"] - current_scenario_result["median_return"]

            stress_data.append([
                '与当前情景差异',
                f'{profit_prob_diff*100:+.1f}个百分点',
    ...
    # 使用120日窗口的VaR结果
    if '120日' in var_results:
        var_120_result = var_results['120日']
        var_90_value = var_120_result['var_90']
        var_95_value = var_120_result['var_95']
        var_99_value = var_120_result['var_99']
        cvar_95_value = var_120_result['cvar_95']
        cvar_99_value = var_120_result['cvar_99']

        var_risk_level = "低风险" if var_95_value < 0.15 else "中等风险" if var_95_value < 0.25 else "高风险"

        var_summary_data = [
            ['90%置信', f'{var_90_value*100:.2f}%', f'{var_120_result.get("cvar_90", var_90_value*1.3)*100:.2f}%', '10%概率损失超过此值'],
            ['95%置信', f'{var_95_value*100:.2f}%', f'{cvar_95_value*100:.2f}%', '5%概率损失超过此值'],
            ['99%置信', f'{var_99_value*100:.2f}%', f'{cvar_99_value*100:.2f}%', '1%概率损失超过此值'],
            ['', '', '', ''],
            ['数据窗口', '120日（半年）', '', '推荐窗口期'],
