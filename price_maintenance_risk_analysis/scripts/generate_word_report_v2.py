...
def generate_monte_carlo_charts_split(final_prices, issue_price, current_price, save_dir):
    ...
    ax.text(0.1, 0.5, stats_text, transform=ax.transAxes,
            fontsize=16, verticalalignment='center', fontfamily='monospace',
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.3))
...
def generate_report(stock_code='300735.SZ', output_file='定增风险分析报告.docx'):
    ...
    add_paragraph(document, '五、相对估值分析')
    add_paragraph(document, '六、蒙特卡洛模拟')
    add_paragraph(document, '七、VaR风险度量')
    add_paragraph(document, '八、综合评估')
    add_paragraph(document, '九、投资建议与风险提示')
    ...
    # 添加拆分的敏感性分析图表
    add_paragraph(document, '')
    add_paragraph(document, '图表 1: 敏感性分析 - 波动率与盈利概率')
    add_image(document, sensitivity_chart_paths[0], width=Inches(6.5))

    add_paragraph(document, '')
    add_paragraph(document, '图表 1.1: 敏感性分析 - 发行价折扣与盈利概率')
    add_image(document, sensitivity_chart_paths[1], width=Inches(6.5))

    add_paragraph(document, '')
    add_paragraph(document, '图表 1.2: 敏感性分析 - 盈利概率热力图')
    add_image(document, sensitivity_chart_paths[2], width=Inches(6.5))
    ...
    add_title(document, '4.1 DCF估值方法说明', level=2)
            ...
            for days_back in range(1, 15):
                ...
                try:
                    ...
                    if not df_balancesheet.empty:
                        ...
                        print(f"   现金及现金等价物: {cash_equivalents:.2f} 亿元")
                        if net_debt > 0:
    ...
    # 生成并插入 DCF 热力图
    add_title(document, '4.2 估值敏感性分析', level=2)
    ...
    add_title(document, '4.3 估值参数与过程', level=2)
    ...
    equity_value = enterprise_value - (total_debt / 100000000)
    intrinsic_value = equity_value / (total_shares / 100000000)
    ...
    add_title(document, '4.4 估值结论', level=2)
    ...
    add_title(document, '5.1 估值指标对比', level=2)
    ...
    # 添加同行公司名单
    add_paragraph(document, '')
    add_title(document, '5.1.1 同行公司名单', level=2)
    ...
    add_title(document, '5.2 估值偏离度分析', level=2)
    ...
    add_title(document, '5.3 估值回归情景分析', level=2)
    ...
    # ==================== 六、蒙特卡洛模拟 ====================
    add_title(document, '六、蒙特卡洛模拟', level=1)
    ...
    # ==================== 六、VaR风险度量 ====================
    add_title(document, '七、VaR风险度量', level=1)
    ...
    # ==================== 七、综合评估 ====================
    add_title(document, '八、综合评估', level=1)
    ...
    # ==================== 八、投资建议与风险提示 ====================
    add_title(document, '九、投资建议与风险提示', level=1)
