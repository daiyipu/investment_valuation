...
class ReportFetcher:
    ...
    def fetch_industry_reports(self, industry_keyword: str, max_count: int = 5) -> List[dict]:
        """获取行业研报（用关键词列表匹配标题和行业字段）"""
        begin_time = (datetime.now() - timedelta(days=self.date_range_months * 30)).strftime('%Y-%m-%d')

        # 从行业名称生成搜索关键词
        keywords = self._expand_keywords(industry_keyword)
    ...
    def _expand_keywords(industry_name: str) -> list:
        ...
        """
        # 基础关键词：按逗号分割的行业名
        keywords = [k.strip() for k in industry_name.split(',') if k.strip()] if industry_name else []

        # 行业名称到搜索关键词的扩展映射
        expansions = {
            '电力设备': ['电力', '电气', '电力设备', '新能源', '电网', '电池', '线缆', '电缆'],
            '电气设备': ['电力', '电气', '电力设备', '新能源', '电网', '电池', '线缆', '电缆'],
            '电网设备': ['电网', '电力', '输变电', '配电', '电缆'],
            '线缆': ['线缆', '电缆', '电力', '电力设备'],
            '电池': ['电池', '锂电', '储能', '新能源'],
            '汽车': ['汽车', '新能源车', '电动'],
            '医药': ['医药', '生物', '医疗'],
        ...
        for key, words in expansions.items():
            if any(key in k for k in keywords):
                keywords.extend(words)
                break

        return list(set(keywords)) if keywords else [industry_name]
